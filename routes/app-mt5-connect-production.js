...
const { clearOtherAccountsOnPortSlot, upsertAccountForPortSlot } = require('../lib/mt5PortAccount');
...
let runtimeColumnsReady = false;

async function ensureRuntimeColumns() {
  if (runtimeColumnsReady) return;
  ...
  try {
    ...
    const r = await client.query(`
      ...
      WHERE LOWER(COALESCE(p.status,'available')) IN ('available','free','idle')
        ...
        AND (p.port_no BETWEEN 101 AND 120 OR p.port_no BETWEEN 1 AND 20)
        AND NOT EXISTS (
          SELECT 1
...
/** บัญชีเดิมที่ retry login ได้ — ไม่ reuse FolderPort เก่า (จอง folder ว่างใหม่) */
async function findRetryAccountForLogin(userId, mt5Login, serverName) {
  const r = await query(
    `
    SELECT id AS account_id, port_slot, status
    FROM vps_system.mt5_accounts
    WHERE user_id = $1
      AND mt5_login = $2
      AND COALESCE(server_name, mt5_server, '') = $3
      AND LOWER(COALESCE(status, '')) IN ('failed', 'ready', 'connecting', 'starting', 'checking')
    ORDER BY updated_at DESC
    LIMIT 1
  `,
...
async function getReusablePortSlotAccount(userId, slot, mt5Login, serverName) {
  ...
  if (!userId || !s || !login || !server) return null;

  const r = await query(
    `
    SELECT id, mt5_login, status, port_slot, port_id, vps_id, assigned_port_no
    FROM vps_system.mt5_accounts
    WHERE user_id=$1
      AND port_slot=$2
      AND mt5_login=$3
      AND COALESCE(server_name, mt5_server, '')=$4
      AND LOWER(COALESCE(status, '')) IN ('connected', 'ready', 'failed', 'checking', 'connecting', 'starting')
    ORDER BY updated_at DESC, id DESC
    LIMIT 1
  `,
    [userId, s, login, server]
  ).catch(() => ({ rows: [] }));
...
async function handleMt5ConnectProduction(req, res) {
  ...
  try {
    ...
    const acc = await upsertAccountForPortSlot(query, {
      userId,
      portSlot,
      mt5Login,
      mt5Password,
...
async function handleMt5ConnectStatusProduction(req, res) {
  try {
    const poll = String(req.query.poll || '').trim() === '1';
    if (!poll) {
      await ensureRuntimeColumns();
    }
    const userId = Number((req.user || req.session?.user)?.id || 0);
    ...
    const data = await getConnectStatusForUser(userId, accountId, { poll }).catch(() => null);
    if (!data) {
