...
function computeAkPreset(bot, capital, tradeLevel, manualLot, lotMin, lotMax) {
...
function computePresetForBot(bot, capital, tradeLevel, manualLot, lotMin, lotMax, packageDefaultLot) {
  ...
  if (kind === 'ak') {
    return computeAkPreset(bot, capital, tradeLevel, manualLot, lotMin, lotMax);
  }
...
function defaultCapitalForBot(bot, equity = 0) {
  ...
  if (kind === 'quantum') return Math.max(eq, BOT_MIN_CAPITAL['QUANTUM-QUEEN-MT5-3.65']);
  if (kind === 'queen') return Math.max(eq, BOT_MIN_CAPITAL['QUEEN-SNIPER-AI-V1.0']);
  if (eq > 0) return eq;
...
function capitalHintForBot(bot) {
  ...
  if (kind === 'queen') {
    return `ขั้นต่ำ ${minCap.toLocaleString()} USD · ทุน 10,000 ต่อ +0.01 Lot (แก้ทุนได้)`;
  }
  if (kind === 'quantum') {
    return `ขั้นต่ำ ${minCap.toLocaleString()} USD · บอทคำนวณอัตโนมัติ (M15)`;
  }
