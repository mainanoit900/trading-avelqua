...
async function gatherForecastContext(userId, accountId) {
...
async function saveForecast(userId, accountId, login, payload, model) {
  ...
  await query(
    ...
    `,
    [userId, accountId, login, FORECAST_HORIZON, payload, model, expiresAt]
  );
}

async function loadCachedForecast(userId, accountId) {
  const rows = await query(
    `
    SELECT payload, model, generated_at, expires_at
  ...
  return {
    ...
    stale: expiresAt > 0 && expiresAt < Date.now()
  };
}

async function fetchForecastForAccount(userId, accountId, { refresh = false } = {}) {
  ...
  const cached = await loadCachedForecast(uid, aid);
  ...
  if (refresh && cooldownLeftMs > 0 && cached?.payload) {
    return {
      ok: true,
      accounts,
      accountId: aid,
  ...
  const context = await gatherForecastContext(uid, aid);
  if (!context) return { ok: false, message: 'account_not_found' };

  const apiKey = await resolveOpenAiKey();
  const model = process.env.OPENAI_INTEL_MODEL || process.env.OPENAI_MODEL || 'gpt-4.1-mini';
  const forecast = await runPythonForecast(context, apiKey, model);
  await saveForecast(uid, aid, context.mt5_login, forecast, model);
