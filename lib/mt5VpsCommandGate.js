...
function createVpsCommandGate(opts) {
  ...
  function staggerMs() {
    const fallback = num(opts.defaultStaggerMs, 3000);
    return Math.max(1500, num(process.env[staggerEnv], fallback));
  }
  ...
  async function countInFlight(vpsId, db = null) {
  ...
  async function acquire(vpsId, db = null) {
    const nid = num(vpsId, 0);
    if (!nid) return { waitedMs: 0, lockKey: null };

    const lockKey = `lock:vps:${nid}:${lockSuffix}`;
    const start = Date.now();
    const maxWait = maxWaitMs();
    const poll = pollMs();

    while (Date.now() - start < maxWait) {
      if ((await countInFlight(nid, db)) > 0) {
        await sleep(poll);
        continue;
      }
