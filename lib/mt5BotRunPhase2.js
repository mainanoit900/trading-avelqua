...
function folderPathForAccount(account) {
  ...
  const folderNo = adminPortNoFromSystem(portNo) || portNo;
  if (!folderNo) return '';
  return `C:\\MT5_PORTS\\${nodeCode}-PORT-${String(folderNo).padStart(2, '0')}`;
}

async function queuePreRunPortCleanup({ vpsId, portId, runPayload, client = null }) {
  const payload = runPayload && typeof runPayload === 'object' ? runPayload : {};
  const portNo = num(
    payload.port || payload.portNumber || payload.folderPort || payload.vpsPortNumber || payload.port_no
  );
  if (!portNo || !num(vpsId)) return { stopCommandId: 0 };

  const folderPath = clean(payload.folderPath || payload.folder_path || payload.vpsFolderPath);
  const mt5Login = clean(payload.mt5Login || payload.login);
  const accountId = num(payload.accountId || payload.account_id);

  const stopIns = await insertPendingAgentCommand({
    vpsId: Number(vpsId),
    nodeId: Number(vpsId),
    ...
    payload: {
      ...
      commandType: 'stop_mt5_bot',
      purposeType: 'pre_run_cleanup',
      purpose: 'pre_run_cleanup',
      port: portNo,
      portNumber: portNo,
      portNo,
      port_no: portNo,
      folderPort: portNo,
      vpsPortNumber: portNo,
      folderPath,
      folder_path: folderPath,
      vpsFolderPath: folderPath,
      mt5Login,
      expectedMt5Login: mt5Login,
  ...
  return { stopCommandId: Number(stopIns?.id || 0) };
}

function botRunSplitPhasesEnabled() {
  return String(process.env.MT5_BOT_RUN_SPLIT_PHASES || '1').trim() !== '0';
}

async function queueBotRunCommands({ attemptId, vpsId, portId, runPayload, client = null }) {
  // Cancel any pending login_exit_mt5 before bot run — prevents it from killing the new bot MT5
  if (vpsId && portId) {
    await query(`
      UPDATE vps_system.vps_agent_commands
      SET status='cancelled', result_message='cancelled_for_bot_run',
          finished_at=COALESCE(finished_at,NOW()), updated_at=NOW()
      WHERE (vps_id=$1 OR node_id=$1) AND port_id=$2
        AND command_type IN ('login_exit_mt5', 'stop_mt5')
        AND LOWER(COALESCE(status,'')) IN ('pending','queued','picked','processing','running')
        AND COALESCE(payload->>'purpose','') IN ('post_connect_exit','login_only_exit')
    `, [Number(vpsId), Number(portId)]).catch(() => {});
  }

  // ไม่คิว stop แยก — run_bot_command จัดการ close positions + kill + launch เอง (กันเปิด MT5 ซ้ำจาก mt5.initialize)
  ...
  const launchIns = await insertPendingAgentCommand({
    vpsId: Number(vpsId),
    nodeId: Number(vpsId),
    portId: portId != null ? Number(portId) : null,
    commandType: 'run_mt5_bot',
    payload: {
      ...basePayload,
      botPhase: 'launch',
      purpose: 'bot_run_launch'
    },
    client
  });

  const algoIns = await insertPendingAgentCommand({
    vpsId: Number(vpsId),
    nodeId: Number(vpsId),
    portId: portId != null ? Number(portId) : null,
    commandType: 'enable_mt5_algo',
    payload: {
      ...basePayload,
      purpose: 'bot_run_enable_algo'
    },
    client
  });

  const finalizeIns = await insertPendingAgentCommand({
    vpsId: Number(vpsId),
    nodeId: Number(vpsId),
    portId: portId != null ? Number(portId) : null,
    commandType: 'run_mt5_bot',
  ...
  return {
    loginCommandId: 0,
    runCommandId: Number(finalizeIns?.id || 0),
    launchCommandId: Number(launchIns?.id || 0),
    algoCommandId: Number(algoIns?.id || 0),
    finalizeCommandId: Number(finalizeIns?.id || 0)
  };
}

async function assertNoRecentBotRunAttempt(accountId, windowSec = 12) {
  ...
  await query(
    ...
    SET status = 'expired',
        last_message = COALESCE(last_message, 'expired_stale_bot_run'),
        updated_at = NOW()
    WHERE account_id = $1
      AND purpose_type = 'bot_run'
      AND status IN ('queued', 'starting', 'checking')
  ...
  const existingAttempt = await query(
    ...
    LIMIT 1
    `,
    [aid, String(Math.max(5, Number(windowSec) || 12))]
  ).catch(() => ({ rows: [] }));
