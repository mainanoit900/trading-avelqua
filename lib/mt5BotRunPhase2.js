...
async function queueBotRunCommands({
  attemptId,
  vpsId,
  portId,
  loginPayload,
  ...
  const loginIns = await insertPendingAgentCommand({
    vpsId: Number(vpsId),
    nodeId: Number(vpsId),
    ...
    payload: {
      ...loginPayload,
      attemptId: String(attemptId || loginPayload?.attemptId || ''),
      purposeType: 'bot_run',
      keepMt5Open: true,
      purpose: 'bot_run_login',
      botCode: 'LOGIN_ONLY'
    },
    client
  });
  ...
  return {
    loginCommandId: Number(loginIns?.id || 0),
...
async function startBotRunForAccount({ userId, accountId, botCode, runOptions = {} }) {
  ...
  try {
    ...
    if (attemptId && queued.loginCommandId) {
      await query(
        `
        ...
        WHERE attempt_id = $1
      `,
        [attemptId, queued.loginCommandId]
      ).catch(() => {});
    }
    ...
    await query(
      ...
      SET status = 'connecting',
          last_login_message = 'กำลังเปิด MT5 เพื่อรัน BOT...',
          updated_at = NOW()
      WHERE id = $1
    `,
      [aid]
