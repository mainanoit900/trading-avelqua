...
async function tryCachedEquityFastConnect({ userId, mt5Login, mt5Password, serverName, portSlot }) {
  ...
  const slot = Number(portSlot || 0) || Number(cached.port_slot || 0) || null;
  ...
  try {
    let acc = await query(
      ...
      SET
        ...
        last_balance=COALESCE($6, last_balance),
        last_equity=COALESCE($7, last_equity),
    ...
    await query(
      ...
      VALUES
      ($1,$2,$3,NULL,NULL,$4,NULL,NULL,$5,$6,'login_only','connected',TRUE,TRUE,TRUE,$7,$8,'cached_equity',$9,NOW(),NOW(),NOW())
    `,
      [attemptId, accountId, uid, slot, login, server, balance, equity, msg]
    ).catch(() => {});
    ...
    await query(
      ...
      VALUES ($1,$2,NULL,NULL,NULL,$3,$4,'connected',$5)
    `,
