...
async function tryCachedEquityFastConnect({ userId, mt5Login, mt5Password, serverName, portSlot }) {
  ...
  `,
    [slot, password, server, accountName, msg, balance, equity, canonicalId, uid]
  ).catch(() => ({ rows: [] }));
    ...
    `,
      ...
      return query(
        ...
        SET
          ...
          assigned_port_no=NULL,
          windows_port_no=NULL,
          last_balance=COALESCE($4, last_balance),
          last_equity=COALESCE($5, last_equity),
          connected_at=COALESCE(connected_at, NOW()),
          updated_at=NOW()
        WHERE id=$6
          AND user_id=$7
        RETURNING id, last_equity, last_balance
      `,
        [slot, password, msg, balance, equity, reviveId, uid]
      );
    });
  }
