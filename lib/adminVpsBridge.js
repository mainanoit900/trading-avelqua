...
/** ส่งคำสั่งปิด MT5 ไป Agent (ใช้เมื่อ login ผิด / ผู้ใช้กดปิด MT5 บน VPS) */
async function queueForceStopMt5(systemVpsId, portNo, folderPath, reason = 'force_stop') {
  ...
  const portNos = systemPortNosForAdmin(no);
  const targets = portNos.length ? portNos : [no];
  ...
  for (const pno of targets) {
    const payload = {
      ...
      port_no: pno,
      portNumber: pno,
      portSlot: adminPortNoFromSystem(pno) || pno,
      folder_path: folder,
      folderPath: folder,
    ...
    await queueSystemAgentCommand(vpsId, 'stop_mt5', payload, null).catch(() => null);
  }
