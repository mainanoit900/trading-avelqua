...
/** สำหรับจอง PORT login — ผ่อนกว่า health 3 นาที (Agent heartbeat อาจห่างชั่วคราว) */
const VPS_LOGIN_NODE_SEEN_MS = Number(process.env.VPS_LOGIN_NODE_SEEN_MS || 10 * 60 * 1000);
const VPS_LOGIN_HEALTH_SEEN_MS = Number(process.env.VPS_LOGIN_HEALTH_SEEN_MS || 5 * 60 * 1000);
/** MT5 รันค้างบน VPS แต่ไม่มี BOT — แสดง "ว่าง" หลังไม่มี activity นานกว่านี้ */
const ORPHAN_MT5_IDLE_MS = Number(process.env.ADMIN_ORPHAN_MT5_IDLE_MS || 40_000);
 ...
 */
function portHasRecentBotActivity(botActivity, portNo) {
  const slot = normalizePortSlot(portNo);
  if (!slot || !botActivity) return false;
  const act = botActivity[slot] || botActivity[portNo] || botActivity[100 + slot];
  if (!act) return false;
  if (act.hasActiveBot === true) return true;
  const last = Number(act.lastActivityMs || 0);
  return last > 0 && Date.now() - last <= ORPHAN_MT5_IDLE_MS;
}

function resolveAdminPortMt5State({ live, dbUse, adminDisabled, botActivity }) {
  if (adminDisabled) {
  ...
  const portNo = normalizePortSlot(live?.port_number || dbUse?.port_number || 0);

  // บัญชี connected ในระบบ — อย่าแสดง "ว่าง" เพราะ health ค้างหลัง MT5 ถูก kill ชั่วคราว
  if (hasConnected && strict !== true) {
    return {
      inUse: true,
      orphanRunning: false,
      mt5Login: dbUse.mt5_login || live?.mt5_login || null,
      agentState: strict,
      usageSource: strict === false ? 'connected_stale_agent' : 'connected'
    };
  }
  ...
  if (agentRunning) {
    ...
    // MT5 เปิดค้างบน VPS — แสดง "MT5 ค้าง" เฉพาะเมื่อมี BOT ทำงาน/มี activity ภายใน 40 วินาที
    if (portHasRecentBotActivity(botActivity, portNo)) {
      return {
        inUse: false,
        orphanRunning: true,
        mt5Login: live?.mt5_login || null,
        agentState: true,
        usageSource: 'orphan_mt5_bot'
      };
    }
    return {
      inUse: false,
      ...
      mt5Login: null,
      agentState: true,
      usageSource: 'orphan_idle_expired',
      shouldClearOrphan: true
    };
  }

  if (strict === false && !hasConnected) {
...
function hasFreshPortHealthSignal(liveMap) {
  if (!liveMap || typeof liveMap !== 'object') return false;
  const now = Date.now();
  for (const live of Object.values(liveMap)) {
    const ts = live?.updated_at ? new Date(live.updated_at).getTime() : 0;
    if (ts && now - ts <= VPS_LOGIN_HEALTH_SEEN_MS) return true;
  }
  return false;
}

/** VPS ผ่านเงื่อนไขจาก /admin/vps/:id/edit (cpu_alarm, ram_alarm, ping_alarm) */
function isVpsNodeEligibleForLogin(nodeRow, opts = {}) {
  ...
  if (opts.freshPortHealth === true) return true;
  const seen = nodeRow.last_seen_at ? new Date(nodeRow.last_seen_at).getTime() : 0;
  if (seen && Date.now() - seen > VPS_LOGIN_NODE_SEEN_MS) return false;
  return true;
}
...
/** BOT / คำสั่ง Agent ล่าสุดต่อ PORT — ใช้ตัดสิน "MT5 ค้าง" vs "ว่าง" */
async function fetchPortBotActivityMap(adminNodeId) {
  const { systemVpsId } = await resolveSystemVpsId(adminNodeId);
  const map = {};
  if (!systemVpsId) return map;

  const touch = (portNo, tsMs, activeBot = false) => {
    const slot = normalizePortSlot(portNo);
    if (!slot || !tsMs) return;
    const prev = map[slot] || { hasActiveBot: false, lastActivityMs: 0 };
    map[slot] = {
      hasActiveBot: prev.hasActiveBot || activeBot,
      lastActivityMs: Math.max(prev.lastActivityMs || 0, tsMs)
    };
  };
  ...
  const bots = await query(
    `
    SELECT
      COALESCE(bi.assigned_port_no, ma.assigned_port_no, ma.windows_port_no, ma.port_slot) AS port_no,
      bi.updated_at,
      bi.started_at
    FROM vps_system.bot_instances bi
    LEFT JOIN vps_system.mt5_accounts ma ON ma.id = bi.mt5_account_id
    WHERE bi.vps_id = $1
      AND LOWER(COALESCE(bi.status, '')) IN ('running', 'pending', 'starting', 'restarting')
  `,
    [systemVpsId]
  ).catch(() => ({ rows: [] }));
  ...
  const cmds = await query(
    ...
    WHERE (vps_id = $1 OR node_id = $1)
      ...
      AND command_type IN (
        'run_mt5_bot', 'run_mt5', 'run_bot', 'login_mt5', 'connect_mt5',
        'sync_mt5_account', 'account_snapshot', 'read_account_metrics'
      )
  `,
    [systemVpsId]
  ).catch(() => ({ rows: [] }));
  ...
  for (const row of cmds.rows || []) {
    ...
    const pno =
      ...
      pl.vpsPortNumber;
    const ts = Math.max(
      row.finished_at ? new Date(row.finished_at).getTime() : 0,
      row.updated_at ? new Date(row.updated_at).getTime() : 0
    );
    touch(pno, ts, String(pl.action || '').toLowerCase() === 'run_bot');
  }

  return map;
}

/** ปิด MT5 ทันทีหลัง login ผิด — ไม่ dedupe (กันค้าง「โฟลเดอร์ไม่ว่าง」) */
async function queueForceStopMt5(systemVpsId, portNo, folderPath, reason = 'login_auth_failed') {
  const vpsId = Number(systemVpsId || 0);
  const slot = normalizePortSlot(portNo);
  if (!vpsId || !slot) return false;

  await query(
    `
    UPDATE vps_system.vps_agent_commands
    SET status='cancelled',
        error='cancelled: force stop after login fail',
        finished_at=NOW(),
        updated_at=NOW()
    WHERE (vps_id = $1 OR node_id = $1)
      ...
      AND (
        (payload->>'port')::text IN ($2::text, $3::text)
        OR (payload->>'port_no')::text IN ($2::text, $3::text)
        OR (payload->>'portNumber')::text IN ($2::text, $3::text)
        OR (payload->>'portSlot')::text IN ($2::text, $3::text)
      )
  `,
    [vpsId, String(slot), String(100 + slot)]
  ).catch(() => {});
...
async function queueOrphanIdleStop(systemVpsId, portNo, folderPath, reason = 'orphan_idle_40s') {
  const vpsId = Number(systemVpsId || 0);
  const slot = normalizePortSlot(portNo);
  if (!vpsId || !slot) return false;

  if (reason === 'login_auth_failed' || reason === 'login_failed_cleanup') {
    return queueForceStopMt5(vpsId, slot, folderPath, reason);
  }
  ...
  const recent = await query(
    ...
    FROM vps_system.vps_agent_commands
    WHERE (vps_id = $1 OR node_id = $1)
      AND command_type IN ('stop_mt5', 'kill_mt5')
      AND LOWER(COALESCE(status, '')) IN ('pending', 'processing', 'picked', 'running', 'success', 'done')
...
/** ปล่อย PORT ฝั่งลูกค้า + admin หลังกดลบบน /app/mt5 */
async function releaseUserPortCompletely({
  systemVpsId,
  adminNodeId,
  portNo,
  folderPath,
  ...
  const slot = adminPortNoFromSystem(portNo);
  if (!vpsId && !aid) return;

  if (vpsId && slot) {
    await clearPortHealthRunning(vpsId, slot);
    await query(
      `
      UPDATE vps_system.vps_ports
      SET status='available', mt5_login=NULL, current_mt5_login=NULL,
          locked_by_user_id=NULL, locked_until=NULL, process_id=NULL,
          last_error=NULL, updated_at=NOW()
      WHERE vps_id=$1 AND port_no = ANY($2::int[])
    `,
      [vpsId, systemPortNosForAdmin(slot)]
    ).catch(() => {});
  }

  if (aid && slot) {
    await reconcilePortIdleWhenAgentFree(aid, slot, folderPath || '');
    await setAdminAllocationStatus(aid, slot, 'free');
  }

  if (vpsId && slot && folderPath) {
    await queueOrphanIdleStop(vpsId, slot, folderPath, 'user_delete_port').catch(() => false);
  }
}
...
module.exports = {
  ...
  hasFreshPortHealthSignal,
  VPS_LOGIN_NODE_SEEN_MS,
  VPS_LOGIN_HEALTH_SEEN_MS,
