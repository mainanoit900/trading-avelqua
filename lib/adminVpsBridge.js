...
/** สถานะ "ใช้งาน" ตาม /admin/vps/:id/ports — เฉพาะเมื่อ Agent ยืนยันว่า MT5 รันจริง */
function isAdminPortInUse(portRow, liveMap = {}, dbUsageMap = {}, portNo = null) {
  const no = Number(portNo || parsePortNumber(portRow) || 0);
  if (!no || isPortAdminDisabled(portRow)) return false;
  const live = liveMap[no] || {};
  return isAgentMt5Running(live) === true;
}
