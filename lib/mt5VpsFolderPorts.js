...
/**
 * สถานะ Folder PORT บน Windows VPS — อ้างอิง /admin/vps/:id/ports + Agent vps_port_health
 */
...
async function listFolderPortsOnNode(adminNodeId) {
  ...
  const r = await query(
    ...
    SELECT
      ...
      p.port_name,
      p.port_number,
      COALESCE(NULLIF(TRIM(p.folder_path), ''), NULLIF(TRIM(p.base_path), ''), '') AS folder_path,
  ...
  return (r.rows || []).map((row) => {
    ...
    const available = isAdminPortAvailableForLogin(row, liveMap, dbMap);
    return {
      adminNodeId: nid,
      nodeName,
      portNumber: portNo,
...
/** เลือก Folder PORT ที่ตรง package slot (เลข PORT 1 → folder PORT 01) */
function pickFolderPortForSlot(folderPorts, slot, nodes) {
  const s = Number(slot || 0);
  if (!s) return null;
  const online = (nodes || []).filter((n) => n.agentOnline);
  const onlineIds = new Set(online.map((n) => n.adminNodeId));

  const matchAvailable = (folderPorts || []).filter(
    (fp) =>
      fp.portNumber === s &&
      fp.available &&
      onlineIds.has(fp.adminNodeId)
  );
  if (matchAvailable.length) {
    matchAvailable.sort((a, b) => a.adminNodeId - b.adminNodeId);
    return matchAvailable[0];
  }

  const matchAny = (folderPorts || []).filter(
    (fp) => fp.portNumber === s && onlineIds.has(fp.adminNodeId)
  );
  if (matchAny.length) {
    matchAny.sort((a, b) => (a.available === b.available ? 0 : a.available ? -1 : 1));
    return matchAny[0];
  }

  const anyAvail = (folderPorts || []).filter((fp) => fp.available && onlineIds.has(fp.adminNodeId));
  if (anyAvail.length) {
    anyAvail.sort(
      (a, b) =>
        Math.abs(a.portNumber - s) - Math.abs(b.portNumber - s) || a.adminNodeId - b.adminNodeId
    );
    return anyAvail[0];
  }
...
async function enrichPackagePortsForUi(ports, accounts) {
  ...
  return {
    ...
    ports: (ports || []).map((p) => {
      ...
      const accFolder = acc?.folder_path || acc?.assigned_folder || null;
      let sublabel = p.sublabel || 'ยังไม่เชื่อมต่อ';
      if (folder && !p.accountId) {
        sublabel =
          `${folder.nodeName} · โฟลเดอร์ PORT ${String(folder.portNumber).padStart(2, '0')}` +
          (folder.folderPath ? ` · ${folder.folderPath}` : '') +
          ` · ${folder.folderState}`;
      } else if (folder && p.accountId) {
        sublabel =
          `Login: ${p.mt5_login || acc?.mt5_login || '—'}` +
          (folder.nodeName ? ` · ${folder.nodeName}` : '') +
          (accFolder || folder.folderPath ? ` · ${accFolder || folder.folderPath}` : '');
      }
      const canPick =
        p.canPick !== false &&
        (!folder || folder.available || !!p.accountId) &&
        (folder ? folder.vpsOnline !== false : onlineCount > 0);
      return {
        ...p,
        sublabel,
        canPick,
        folderPortNo: folder?.portNumber ?? null,
        ...
        folderAvailable: folder?.available ?? null,
