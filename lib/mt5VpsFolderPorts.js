  ...
  return {
    ...
    ports: (ports || []).map((p) => {
      ...
      if (folder && !p.accountId) {
        sublabel = `${shortVpsLabel(folder.nodeName)} · P${String(folder.portNumber).padStart(2, '0')} · ${
          folder.available ? 'ว่าง' : folder.folderState || 'ไม่ว่าง'
        }`;
      } else if (folder && p.accountId) {
        sublabel = `Login ${p.mt5_login || acc?.mt5_login || '—'} · ${shortVpsLabel(folder.nodeName)}`;
      }
      const canPick =
        p.canPick !== false &&
