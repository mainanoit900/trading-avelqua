...
async function listVpsNodesStatus() {
  const r = await query(
    ...
    WHERE COALESCE(n.agent_enabled, TRUE) = TRUE
      AND LOWER(COALESCE(n.status, '')) NOT IN (
        'maintenance', 'disabled', 'deleted', 'inactive', 'off', 'retired'
      )
    ORDER BY n.id ASC
    `,
    []
...
function pickFolderPortByNumber(folderPorts, portNo, nodes) {
  const n = Number(portNo || 0);
  if (!n) return null;
  return pickFolderPortForSlot(folderPorts, n, nodes);
}
...
async function enrichPackagePortsForUi(ports, accounts) {
  ...
  const loginOwnerSlot = new Map();
  for (const a of accounts || []) {
    const slot = Number(a.port_slot || 0);
    const login = String(a.mt5_login || '').trim();
    if (slot > 0 && login) loginOwnerSlot.set(login, slot);
  }
  ...
  return {
    ...
    ports: (ports || []).map((p) => {
      ...
      const physicalPortNo = acc
        ? Number(acc.assigned_port_no || acc.windows_port_no || slot)
        : slot;
      const folder =
        pickFolderPortByNumber(folderPorts, physicalPortNo, nodes) ||
        pickFolderPortForSlot(folderPorts, slot, nodes);
      ...
      const folderLogin = String(folder?.mt5Login || '').trim() || null;
      const ownerSlot = folderLogin ? loginOwnerSlot.get(folderLogin) : null;
      const folderUsedByOtherSlot = !acc && ownerSlot && ownerSlot !== slot;

      const canPick =
        !p.accountId &&
        p.canPick !== false &&
        !folderUsedByOtherSlot &&
      ...
      return {
        ...
        folderPortNo: physicalPortNo || (folder ? folder.portNumber : null),
        folderPath: folder?.folderPath || accFolder || null,
        folderMismatch: acc ? physicalPortNo !== slot : folder && folder.portNumber !== slot,
        vpsNodeName: folder?.nodeName || null,
        vpsOnline: folder?.vpsOnline ?? (onlineCount > 0),
        folderRunning: folderUsedByOtherSlot ? false : (folder?.running ?? false),
        folderMt5Login: folderUsedByOtherSlot ? null : folderLogin,
        folderOwnedBySlot: folderUsedByOtherSlot ? ownerSlot : null,
        folderAvailable: folderUsedByOtherSlot
          ? true
          : folder?.running === true
            ? false
            : folder?.available ?? null,
        folderState: folderUsedByOtherSlot
          ? `Folder P${String(physicalPortNo).padStart(2, '0')} ใช้โดย PORT ${ownerSlot}`
          : folder?.folderState || (onlineCount ? 'รอเลือก' : 'ไม่มี VPS online'),
