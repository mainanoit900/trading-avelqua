 ...
 * package slot เป็นเพียงช่องของผู้ใช้; ตอน login ระบบควรหา FolderPort ว่างให้อัตโนมัติ
 */
...
/** ใช้จับคู่ FolderPort เดิมของ account ที่ผูกเลข port ไว้แล้วเท่านั้น */
...
function pickAnyFreeFolderPort(folderPorts, nodes) {
  const onlineIds = new Set(
    (nodes || []).filter((n) => n.agentOnline).map((n) => n.adminNodeId)
  );
  const rows = (folderPorts || []).filter(
    (fp) => onlineIds.has(fp.adminNodeId) && fp.available === true && fp.running !== true
  );
  if (!rows.length) return null;
  rows.sort(
    (a, b) =>
      Number(a.portNumber || 0) - Number(b.portNumber || 0) ||
      Number(a.adminNodeId || 0) - Number(b.adminNodeId || 0)
  );
  return rows[0];
}
...
async function enrichPackagePortsForUi(ports, accounts) {
  ...
  const autoFolder = pickAnyFreeFolderPort(folderPorts, nodes);
  ...
  return {
    ...
    ports: (ports || []).map((p) => {
      ...
      const accPortNo = Number(acc?.assigned_port_no || acc?.windows_port_no || 0);
      const boundFolder =
        (accPortNo
          ? (folderPorts || []).find((fp) => {
              const fpNo = Number(fp.portNumber || 0);
              return fpNo === accPortNo || adminPortToSystemPortNo(fpNo) === accPortNo;
            })
          : null) ||
        null;
      const folder = acc ? boundFolder : autoFolder;
      const accFolder = acc?.folder_path || boundFolder?.folderPath || null;
      ...
      return {
        ...
        folderPortNo: acc ? (accPortNo || folder?.portNumber || null) : null,
        folderPath: acc ? (accFolder || folder?.folderPath || null) : null,
        ...
        folderAvailable: acc ? (folder?.running === true ? false : folder?.available ?? null) : !!autoFolder,
        folderState: acc
          ? folder?.folderState || (onlineCount ? 'รอเลือก' : 'ไม่มี VPS online')
          : autoFolder
            ? 'ระบบจะค้นหา FolderPort ว่างให้อัตโนมัติ'
            : (onlineCount ? 'ไม่มี FolderPort ว่าง' : 'ไม่มี VPS online'),
        adminPortsUrl: (folder || autoFolder)?.adminPortsUrl || null
      };
    })
  };
}
