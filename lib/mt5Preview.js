...
const previewSaveAt = new Map();

async function ensureMt5PreviewColumns() {
...
function saveMt5Preview(accountId, base64Jpeg) {
  ...
  const now = Date.now();
  const last = previewSaveAt.get(id) || 0;
  if (now - last < 4000) {
    return previewPublicPath(id);
  }
...
async function patchAccountMt5Preview(accountId, { status, message, windowTitle, previewB64 } = {}) {
  const id = Number(accountId || 0);
  if (!id) return '';
  ...
  const previewSqlWithCols = st
    ? `UPDATE vps_system.mt5_accounts SET status=$2, last_error=NULL, last_login_message=$3,
         ...
         mt5_preview_path=COALESCE(NULLIF($5, ''), mt5_preview_path), updated_at=NOW() WHERE id=$1`
    : `UPDATE vps_system.mt5_accounts SET last_login_message=$2,
         mt5_window_title=COALESCE(NULLIF($3, ''), mt5_window_title),
         mt5_preview_path=COALESCE(NULLIF($4, ''), mt5_preview_path), updated_at=NOW() WHERE id=$1`;
