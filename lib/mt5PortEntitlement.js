...
function computePortEntitlement(packageMaxPorts, extraPortRows, packageGroup = '') {
  ...
  const maxExtraSlots = Math.max(0, cap - includedPorts);
  ...
  return {
    ...
    permanentExtra,
    totalPorts,
    /** จำนวนช่องที่ซื้อเพิ่มได้ (ไม่รวม 1 PORT ที่มากับแพ็กเกจ) */
    maxExtraPurchases: maxExtraSlots,
    canAddTemporary: cap > 0 && rawTotal < cap,
