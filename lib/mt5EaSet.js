...
function normalizeSessions(eaTimeProfile) {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    out.push({
      use: row?.use != null ? !!row.use : !!def.use,
...
function buildAkEaSetContent({
  ...
  const useTimeFilter = eaTimeProfile?.useTimeFilter !== false;
  const sessions = normalizeSessions(eaTimeProfile);

  const lines = [
    '; Avelqua — AK-SNIPER (web overrides only)',
    `; bot=${botCode || ''} level=${tradeLevel || ''} capital=${capital || 0}`,
...
function buildEaSetPayloadFields({
  ...
  const setFileName = eaSetFileName(botCode, tradeLevel, capitalUsed);
