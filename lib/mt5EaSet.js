...
/** QUANTUM-QUEEN: ส่ง BaseLot + LotPerCapital + CapitalAmount จากทุนที่ตั้งบนเว็บ */
function buildQuantumEaSetContent({ botCode, lot, capital }) {
  const lotVal = num(lot, 0.01);
  const capitalVal = num(capital, 2000);
  const lines = [
    '; Avelqua — QUANTUM-QUEEN (web: lot + capital)',
    `; bot=${botCode || ''} capital=${capitalVal}`,
    `; generated=${new Date().toISOString()}`,
    '',
    setLine('BaseLot', lotVal, { default: 0.01, min: 0.01, max: 100, step: 0.01 }),
    setLine('LotPerCapital', lotVal, { default: 0.01, min: 0.001, max: 10, step: 0.001 }),
    setLine('CapitalAmount', capitalVal, { default: 2000, min: 100, max: 10000000, step: 100 }),
    ''
  ];
  return lines.join('\r\n');
}
...
function buildEaSetContent(opts = {}) {
  ...
  if (kind === 'quantum') return buildQuantumEaSetContent(opts);
...
function buildEaSetPayloadFields({
  ...
  if (kind === 'sniper_ai') {
    return {
      ...
      eaSetContent: '',
      eaSetPaths: [],
      eaAttachHint: 'แนบ EA SNIPER-AI-TRADING บนกราฟ XAUUSD M5 — ไม่ต้องตั้งค่า (อัตโนมัติ 100%)'
    };
  }
  ...
  const attachName = kind === 'queen' ? 'QUEEN-SNIPER-AI-V1.0' : kind === 'quantum' ? 'Quantum-Queen-MT5-3.65' : botCode;
  const periodHint = 'M15';
