...
// คำสั่งที่ให้ส่งต่อให้ ADMIN ตอบเอง (ไม่ตอบกลับอัตโนมัติจากบอท)
const ADMIN_MANUAL_REPLY_KEYS = new Set([
  'วิธีใช้งาน',
  'ลงทะเบียน',
  'แจ้งสรุปผล',
  'เช็คพอร์ต',
...
function shouldHandOffToAdmin(raw, lower) {
  const text = String(raw || '').trim();
  const l = String(lower || '').trim();
  if (!text) return false;
  if (ADMIN_MANUAL_REPLY_KEYS.has(text) || ADMIN_MANUAL_REPLY_KEYS.has(l)) return true;
  if (/^(?:register|ลงทะเบียน)\b/i.test(text)) return true;

  // รองรับกรณี user พิมพ์ต่อท้าย เช่น "ช่วยเหลือครับ", "เช็คพอร์ตให้หน่อย"
  return (
    text.includes('วิธีใช้งาน') ||
    text.includes('ลงทะเบียน') ||
    text.includes('แจ้งสรุปผล') ||
    text.includes('เช็คพอร์ต') ||
...
async function handleLineInput(lineUserId, text, replyToken = null) {
  ...
  if (shouldHandOffToAdmin(raw, lower)) {
    // ปิด flow อัตโนมัติค้างไว้ เพื่อให้ ADMIN รับช่วงตอบเองทั้งหมด
    await setPendingAction(lineUserId, null);
    return;
  }
