...
function mapLiveInstanceRow(row) {
  ...
  let balance = positiveMoney(row.mt5_balance) ?? positiveMoney(row.account_last_balance) ?? 0;
  let equity = positiveMoney(row.mt5_equity) ?? positiveMoney(row.account_last_equity) ?? 0;
  if (hasSnapshot) {
    equity = snapshotEq;
