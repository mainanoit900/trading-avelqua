...
/** จุดกราฟ Equity ใน DB */
const CHART_SNAPSHOT_MINUTES = Math.max(
  5,
  Number(process.env.MT5_EQUITY_SNAPSHOT_MINUTES || 30)
);
...
function mapLiveInstanceRow(row) {
  ...
  return {
    ...
    chart_snapshot_minutes: CHART_SNAPSHOT_MINUTES,
...
async function fetchLiveDashboardInstances(userId, { limit = 10, offset = 0 } = {}) {
  ...
  return {
    ...
    chartSnapshotMinutes: CHART_SNAPSHOT_MINUTES,
    metricsIntervalMinutes: CHART_SNAPSHOT_MINUTES
  };
}
