...
/** running ใน UI = VPS ยืนยันคำสั่งสำเร็จ + EA/Algo พร้อมเทรดจริง */
function isBotOperationallyRunning(row) {
  ...
  if (!DONE_CMD.includes(cmd)) return false;
  ...
  if (!ea || ea === 'starting' || ea === 'pending' || ea === 'attach_required' || ea === 'ready') {
    return false;
  }
