...
function resolvePackagePortNo(row) {
  const portUsed = num(row?.port_used, 0);
  if (portUsed >= 1 && portUsed <= 50) return portUsed;
  const payload = parseRunPayload(row);
  const slot = num(payload?.portSlot, 0);
  if (slot >= 1 && slot <= 50) return slot;
  const assigned = num(row?.assigned_port_no, 0);
  if (assigned >= 101 && assigned <= 150) return assigned - 100;
  if (assigned >= 1 && assigned <= 50) return assigned;
  return assigned || null;
}

function formatVpsPortLabel(row) {
  const payload = parseRunPayload(row);
  const fromPayload = String(payload.vpsPortName || payload.vps_port_name || '').trim();
  if (fromPayload) return fromPayload;
  const nodeCode = String(row?.node_code || 'VPS-WIN-01').trim() || 'VPS-WIN-01';
  const folderNo =
    adminPortNoFromSystem(num(row?.assigned_port_no, 0))
    || resolvePackagePortNo(row)
    || num(row?.port_used, 0);
  if (!folderNo) return nodeCode;
  return `${nodeCode}-PORT-${String(folderNo).padStart(2, '0')}`;
}

function enrichDashboardRow(row) {
  const payload = parseRunPayload(row);
  const configSnapshot =
    payload.runConfigSnapshot && typeof payload.runConfigSnapshot === 'object'
      ? payload.runConfigSnapshot
      : null;
  let summary =
    configSnapshot
    || (payload.runSummary && typeof payload.runSummary === 'object' ? payload.runSummary : null);
  const runTimeMode = String(
    payload.runTimeMode
    || payload.run_time_mode
    || summary?.runTimeMode
    || configSnapshot?.runTimeMode
    || 'auto'
  ).trim().toLowerCase();
  ...
  if (summary) {
    ...
    displayLotText = String(configSnapshot?.lotText || '').trim() || formatRunSummaryLot(summary);
  ...
  return {
    ...
    display_port: resolvePackagePortNo(row),
...
function isLiveMetricsFresh(row) {
  ...
  const acctAge = row?.account_last_seen_at
    ? pingAgeSec({ last_agent_ping: row.account_last_seen_at })
    : null;
  return (
    (age != null && age <= LIVE_METRICS_FRESH_SEC)
    || (acctAge != null && acctAge <= LIVE_METRICS_FRESH_SEC)
  );
}

function resolveStartEquity(row, equity, balance) {
  let startEq = positiveMoney(row.start_equity);
  const eq = positiveMoney(equity) ?? 0;
  const bal = positiveMoney(balance) ?? 0;
  const cap = num(row.capital_used, 0);
  if (startEq != null && eq > 0 && startEq < eq * 0.05 && startEq <= cap * 2) {
    startEq = bal > 0 ? bal : eq;
  }
  if (startEq == null || startEq <= 0) startEq = bal > 0 ? bal : eq;
  return startEq;
}

/** Live: ใช้ค่าจาก agent/account ทันที (ภายใน LIVE_METRICS_FRESH_SEC) */
function mapLiveInstanceRow(row) {
  ...
  const acctBal = positiveMoney(row.account_last_balance);
  const acctEq = positiveMoney(row.account_last_equity);
  const acctFresh = row.account_last_seen_at
    && pingAgeSec({ last_agent_ping: row.account_last_seen_at }) <= LIVE_METRICS_FRESH_SEC;

  if (acctFresh && acctBal != null) {
    balance = acctBal;
    equity = acctEq ?? acctBal;
  } else if (freshLive) {
    if (balance == null) balance = acctBal ?? 0;
    if (equity == null) equity = acctEq ?? balance;
  } else {
    if (balance == null) balance = acctBal ?? 0;
    if (equity == null) equity = acctEq ?? 0;
    if (hasSnapshot) {
      equity = snapshotEq;
  ...
  const startEquity = resolveStartEquity(row, equity, balance);
  let profit = num(row.profit, NaN);
  if (!Number.isFinite(profit)) {
    profit = startEquity > 0 && equity > 0 ? equity - startEquity : equity - balance;
  }

  const metricsAt = acctFresh
    ? row.account_last_seen_at
    : freshLive
      ? (row.last_agent_ping || row.last_heartbeat || row.updated_at)
      : hasSnapshot
        ? row.last_equity_snapshot_at
        : (row.last_agent_ping || row.account_last_seen_at || row.started_at || row.updated_at);
...
function mapInstanceRow(row) {
  ...
  const payload = parseRunPayload(row);
  const stillActive = !row?.stopped_at && ACTIVE_STATUSES.includes(status);
  if (stillActive) {
    return mapLiveInstanceRow(row);
  }
  let balance = num(row.mt5_balance, 0);
  let equity = num(row.mt5_equity, 0);
  if (status === 'stopped' || status === 'failed') {
    const endedBal = num(payload.endedBalance, NaN);
    const endedEq = num(payload.endedEquity, NaN);
    if (Number.isFinite(endedBal) && endedBal > 0) balance = endedBal;
    if (Number.isFinite(endedEq) && endedEq > 0) equity = endedEq;
  }
  const startEquity = resolveStartEquity(row, equity, balance);
  let profit = num(row.profit, NaN);
  if (!Number.isFinite(profit)) {
...
function portHealthMatchSql(phAlias = 'ph', biAlias = 'bi') {
  return `${phAlias}.port_number IN (
    ${biAlias}.assigned_port_no,
    CASE WHEN ${biAlias}.assigned_port_no >= 100 THEN ${biAlias}.assigned_port_no - 100
         ELSE ${biAlias}.assigned_port_no + 100 END
  )`;
}
...
/** running ใน UI = MT5 รันจริงบน PORT (port-health / EA / live metrics) — ไม่ยก stopped กลับมา */
function isBotOperationallyRunning(row) {
  const status = String(row?.status || '').trim().toLowerCase();
  if (status === 'stopped' || status === 'failed' || status === 'deleted') return false;
  if (row?.stopped_at) return false;
  const payload = parseRunPayload(row);
  if (payload.userStopped === true || String(payload.userStopped || '').toLowerCase() === 'true') {
    return false;
  }
  if (!ACTIVE_STATUSES.includes(status) && status !== 'running') return false;

  const ea = String(row?.ea_status || '').trim().toLowerCase();
  const cmdSt = String(row?.agent_command_status || '').trim().toLowerCase();
  const hasPortHealth = isPortHealthRunningForRow(row);
  const hasLiveMoney = hasPositiveLiveMoney(row);
  const runDone = DONE_CMD.includes(cmdSt) || isRunCommandEffectivelyDone(row);
  const recentPing = hasRecentAgentPing(row);
  if (!recentPing && !hasPortHealth) return false;

  // Strict mode for dashboard: keep "connecting" until bot is truly operational.
  // 1) EA reports running and port health confirms same login.
  if (RUNNING_EA.includes(ea) && hasPortHealth) return true;

  // 2) Command finished + live metrics available + port health running.
  if (runDone && hasLiveMoney && hasPortHealth) return true;
...
const INSTANCE_SELECT = `
        ...
        a.last_seen_at AS account_last_seen_at,
        (
          SELECT el.equity
          FROM vps_system.mt5_equity_logs el
          WHERE el.instance_id = bi.id
        ...
        (
          ...
          WHERE ph.node_id = bi.vps_id
            AND ${portHealthMatchSql('ph', 'bi')}
          ORDER BY ph.updated_at DESC NULLS LAST, ph.running DESC
          LIMIT 1
...
async function reconcileStaleBotsForUser(userId) {
  ...
  const r = await query(
    ...
    JOIN vps_system.vps_port_health ph
     ...
     AND ph.port_number IN (
       bi.assigned_port_no,
       CASE WHEN bi.assigned_port_no >= 100 THEN bi.assigned_port_no - 100
            ELSE bi.assigned_port_no + 100 END
     )
     AND ph.running = TRUE
    WHERE bi.user_id = $1
      ...
      AND (
        NULLIF(COALESCE(bi.run_payload->>'loginCommandId', ''), '') IS NULL
        OR NOT EXISTS (
          SELECT 1
      ...
      AND (
        NULLIF(COALESCE(bi.run_payload->>'commandId', ''), '') IS NULL
        OR EXISTS (
          SELECT 1
          FROM vps_system.vps_agent_commands rc
          WHERE rc.id = NULLIF(COALESCE(bi.run_payload->>'commandId', ''), '')::bigint
            AND LOWER(COALESCE(rc.status, '')) IN ('success', 'processing', 'picked', 'running', 'in_progress')
        )
      )
    `,
    [uid]
  ...
  const sql = `
    ...
    SET status = $2,
        ...
        run_payload = COALESCE(run_payload, '{}'::jsonb) || jsonb_build_object(
          ...
          'endedStatus', $2,
          'userStopped', CASE WHEN $2 = 'stopped' THEN true ELSE COALESCE((run_payload->>'userStopped')::boolean, false) END,
          'userStoppedAt', CASE WHEN $2 = 'stopped' THEN to_char(NOW() AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS.MS\"Z\"') ELSE run_payload->>'userStoppedAt' END
        )
    WHERE id = $1
    RETURNING *
  `;
...
async function fetchActiveRunInstances(userId) {
  ...
  for (const row of rows.rows || []) {
    ...
    out.push({
      ...
      vps_port_label: formatVpsPortLabel(row)
    });
  }
  return out;
}
