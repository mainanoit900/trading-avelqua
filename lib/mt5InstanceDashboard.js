...
function enrichDashboardRow(row) {
  ...
  if (summary) {
    ...
    displayLot = Number.isFinite(payloadLot) ? Number(payloadLot.toFixed(2)) : num(row.lot_used, 0);
    displayFunding = Number.isFinite(payloadCap) ? Number(payloadCap.toFixed(2)) : num(row.capital_used, 0);
