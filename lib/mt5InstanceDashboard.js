...
/** running ใน DB ยังไม่พอ — ต้องมีคำสั่ง VPS สำเร็จ + heartbeat หลัง started_at */
function isBotOperationallyRunning(row) {
  const status = String(row?.status || '').trim().toLowerCase();
  if (status !== 'running') return false;

  const cmd = String(row?.agent_command_status || '').trim().toLowerCase();
  if (cmd && IN_FLIGHT_CMD.includes(cmd)) return false;
  if (cmd && !DONE_CMD.includes(cmd) && ['failed', 'error', 'cancelled'].includes(cmd)) return false;

  const startedAt = row?.started_at ? new Date(row.started_at).getTime() : 0;
  const pingRaw = row?.last_agent_ping || row?.last_heartbeat;
  const pingTs = pingRaw ? new Date(pingRaw).getTime() : 0;
  if (startedAt && (!pingTs || pingTs < startedAt - 3000)) return false;

  const ea = String(row?.ea_status || '').trim().toLowerCase();
  if (ea === 'starting' || ea === 'pending') return false;
...
function normalizeInstanceDisplayStatus(row) {
  ...
  if (status === 'running') return isBotOperationallyRunning(row) ? 'running' : 'connecting';
