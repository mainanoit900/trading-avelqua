...
/** ดึงเลข FolderPort จาก payload jsonb แล้ว normalize เป็น 101–120 */
function payloadFolderPortSql(payloadExpr = 'payload') {
  const raw = `COALESCE(
    NULLIF(${payloadExpr}->>'port', '')::int,
    NULLIF(${payloadExpr}->>'portNumber', '')::int,
    NULLIF(${payloadExpr}->>'port_no', '')::int,
    NULLIF(${payloadExpr}->>'portNo', '')::int,
    NULLIF(${payloadExpr}->>'folderPort', '')::int,
    NULLIF(${payloadExpr}->>'vpsPortNumber', '')::int,
    0
  )`;
  return `CASE WHEN (${raw}) BETWEEN 1 AND 20 THEN (${raw}) + 100 ELSE (${raw}) END`;
}
