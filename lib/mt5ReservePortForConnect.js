...
const {
  ...
  releaseStaleInflightAccountsWithoutBinding
} = require('./mt5PortSlotGuard');
const {
  FOLDER_POOL_MIN,
  FOLDER_POOL_MAX,
  normalizeSystemFolderPortNo,
  isValidPoolFolderPort
} = require('./mt5ReservedPortNo');
...
function isMisboundSlotFolder(portSlot, assignedPortNo) {
  ...
  return !isValidPoolFolderPort(actual);
}

/** SQL: บล็อกเฉพาะ slot อื่นที่ผูก folder ใน pool ถูกต้อง */
function otherSlotCorrectlyBoundFolderClause(userIdParam, slotParam) {
  void userIdParam;
  void slotParam;
  return `
  AND a.assigned_port_no BETWEEN ${FOLDER_POOL_MIN} AND ${FOLDER_POOL_MAX}
  `;
}
...
async function clearMisboundFolderBlockers(userId, targetSlot) {
  ...
  const cancelled = await query(
    ...
    WHERE user_id=$1
      ...
      AND NOT (assigned_port_no BETWEEN ${FOLDER_POOL_MIN} AND ${FOLDER_POOL_MAX})
    RETURNING id
  `,
...
async function findOccupantForFolderPort(vpsId, portNo, opts = {}) {
  ...
  const acc = await query(
    `
    SELECT id, user_id, port_slot, assigned_port_no, status, metrics_ready
    ...
    ORDER BY
      CASE
        WHEN $3::int > 0 AND user_id = $3::int AND port_slot = $4::int THEN 0
        ELSE 1
      END,
      updated_at DESC
    LIMIT 1
  `,
    [nodeId, variants, userId || 0, portSlot || 0]
  ).catch(() => ({ rows: [] }));
  if (acc.rows?.[0]) {
...
async function isSystemPortOccupied(vpsId, portNo, opts = {}) {
  ...
  if (userId > 0) {
    await releaseIdleFolderForSharing(nodeId, portNo, userId).catch(() => 0);
  }

  if (await isFolderPhysicallyActive(nodeId, portNo)) {
    return true;
  }

  // เจ้าของ PORT นี้ connected แล้ว (แม้ MT5 ปิดหลัง login) — reconnect ได้ ถ้าไม่มี user อื่นกำลังใช้ folder จริง
  if (userId > 0 && portSlot > 0) {
    const ownReady = await query(
      `
      SELECT id
      FROM vps_system.mt5_accounts
      WHERE user_id = $1
    ...
    if (ownReady.rows?.[0]) {
      const foreignOnFolder = await query(
        `
        SELECT id
        FROM vps_system.mt5_accounts
        WHERE user_id <> $1
        ...
        LIMIT 1
      `,
        [userId, nodeId, variants]
      ).catch(() => ({ rows: [] }));
      if (!foreignOnFolder.rows?.[0]) {
        return false;
      }
    }
  }

  const occupant = await findOccupantForFolderPort(nodeId, portNo, { userId, portSlot });
  ...
  const occStatus = String(occupant.status || '').toLowerCase();
  if (userId > 0 && num(occupant.user_id) !== userId) {
    if (['connecting', 'checking', 'starting'].includes(occStatus)) {
      return true;
    }
    if (['connected', 'ready'].includes(occStatus)) {
      return true;
    }
    return false;
  }

  return ['connecting', 'checking', 'starting'].includes(occStatus);
}
...
async function findUserSlotBoundPort(userId, portSlot) {
  ...
  const r = await query(
    ...
    WHERE a.user_id = $1
      ...
      AND COALESCE(a.assigned_port_no, p.port_no, 0) BETWEEN $4 AND $5
    ORDER BY
    ...
    LIMIT 1
  `,
    [uid, slot, ACTIVE_BINDING_STATUSES, FOLDER_POOL_MIN, FOLDER_POOL_MAX]
  ).catch(() => ({ rows: [] }));

  const row = r.rows?.[0];
  if (!row) return null;
  const boundNo = normalizeSystemFolderPortNo(num(row.assigned_port_no || row.port_no));
  if (!isValidPoolFolderPort(boundNo)) return null;
  return row;
}
...
async function reserveCanonicalFolderForSlot(userId, portSlot, opts = {}) {
  ...
  await releaseStaleInflightAccountsWithoutBinding().catch(() => 0);

  if (!opts.skipRepair) {
    await clearMisboundFolderBlockers(uid, slot).catch(() => 0);
  }

  const vpsOnFolder = await query(
    `
  ...
  for (const row of vpsOnFolder.rows || []) {
    await releaseIdleFolderForSharing(row.vps_id, canonicalNo, uid).catch(() => 0);
  }
  ...
  const prePortRes = await query(
    ...
    WHERE p.port_no = ANY($1::int[])
      AND LOWER(COALESCE(p.status, '')) IN ('available', 'free', 'idle', 'running', 'busy', 'used', 'locked')
  ...
  const prePort = prePortRes.rows?.[0];
  if (!prePort) {
    return {
      ok: false,
  ...
  await purgeMisboundFolderOccupants(prePort.vps_id, prePort.port_no).catch(() => 0);
  if (await isSystemPortOccupied(prePort.vps_id, prePort.port_no, { userId: uid, portSlot: slot })) {
    const folderGuard = await assertFolderPortFreeForUser(prePort.vps_id, prePort.port_no, uid);
    if (!folderGuard.ok && folderGuard.message) {
      return { ok: false, code: folderGuard.code || 'CANONICAL_FOLDER_BUSY', message: folderGuard.message };
    }
    return {
      ok: false,
      code: 'CANONICAL_FOLDER_BUSY',
  ...
  const preGuard = await assertFolderPortFreeForUser(prePort.vps_id, prePort.port_no, uid);
  if (!preGuard.ok) {
    return { ok: false, message: preGuard.message };
  }
      ...
      await client.query(
        ...
        WHERE port_no = ANY($1::int[])
          ...
          AND NOT EXISTS (
            ...
            WHERE (
                a.port_id = vps_ports.id
                OR (
                  a.vps_id = vps_ports.vps_id
                  AND a.assigned_port_no = ANY($1::int[])
                )
              )
              AND LOWER(COALESCE(a.status, '')) IN ('connecting', 'checking', 'starting')
          )
      `,
        [portNoVariants(canonicalNo)]
      ).catch(() => {});
...
async function tryReuseBoundPort(userId, portId, portSlot = 0) {
  ...
  const slot = num(portSlot);
  if (slot > 0 && !isValidPoolFolderPort(portNo)) {
    return null;
  }
