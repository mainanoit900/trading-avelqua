...
async function findOccupantForFolderPort(vpsId, portNo, opts = {}) {
  ...
  const folderSlots = variants
    .map((pno) => (pno >= 100 ? pno - 100 : pno))
    .filter((s) => s > 0);
  ...
  const acc = await query(
    ...
    WHERE (
        (vps_id = $1 AND assigned_port_no = ANY($2::int[]))
        OR (
          port_slot = ANY($3::int[])
    ...
    ORDER BY
      CASE
        WHEN $4::int > 0 AND user_id = $4::int AND port_slot = $5::int THEN 0
        WHEN vps_id = $1 AND assigned_port_no = ANY($2::int[]) THEN 1
        ELSE 2
      END,
      updated_at DESC
    LIMIT 1
  `,
    [nodeId, variants, folderSlots.length ? folderSlots : [0], userId || 0, portSlot || 0]
  ).catch(() => ({ rows: [] }));
  if (acc.rows?.[0]) {
