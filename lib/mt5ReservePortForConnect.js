...
async function findUserSlotBoundPort(userId, portSlot) {
  ...
  const r = await query(
    ...
    LIMIT 1
  `,
    [uid, slot, ACTIVE_BINDING_STATUSES]
  ).catch(() => ({ rows: [] }));
...
async function reserveCanonicalFolderForSlot(userId, portSlot, opts = {}) {
  ...
  if (blocker) {
    if (!opts.retriedRepair) {
      const repaired = await repairMisboundSlotFolderBindings(uid).catch(() => 0);
      if (repaired > 0) {
  ...
  try {
    ...
    const portRes = await client.query(
      ...
      WHERE p.port_no = $3
        AND (
          LOWER(COALESCE(p.status, '')) IN ('available', 'free', 'idle')
          OR (
            LOWER(COALESCE(p.status, '')) = 'locked'
            AND p.locked_by_user_id = $1
          )
        )
        ...
        AND NOT EXISTS (
          ...
          WHERE a.vps_id = p.vps_id
            AND a.assigned_port_no = ANY($4::int[])
            AND LOWER(COALESCE(a.status, '')) IN ('connecting', 'checking', 'connected', 'ready', 'starting')
            AND (a.user_id <> $1 OR a.port_slot IS NULL OR a.port_slot <> $2)
        )
        AND NOT EXISTS (
          SELECT 1
          FROM vps_system.bot_instances bi
          WHERE bi.vps_id = p.vps_id
            AND bi.assigned_port_no = ANY($4::int[])
            AND bi.stopped_at IS NULL
      ...
      LIMIT 1
    `,
      [uid, slot, canonicalNo, portNoVariants(canonicalNo)]
    );
