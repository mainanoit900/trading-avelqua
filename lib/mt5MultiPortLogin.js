...
const { normalizeSystemFolderPortNo, payloadFolderPortSql } = require('./mt5ReservedPortNo');
...
function queueStaggerSec(kind = 'login') {
  const loginDefault = Math.max(2, num(process.env.MT5_LOGIN_QUEUE_STAGGER_SEC, 2));
  if (kind === 'runbot') {
    return Math.max(2, num(process.env.MT5_RUN_BOT_QUEUE_STAGGER_SEC, loginDefault));
  }
  return loginDefault;
}
...
function computeJournalTimeoutSec(opts = {}) {
  const base = num(process.env.MT5_JOURNAL_TIMEOUT_SEC, 45);
  const activeLogins = Math.max(0, num(opts.activeLoginCount, 0));
  const extra = Math.min(30, Math.max(0, activeLogins - 1) * 8);
  const scaled = base + extra;
  const cap = num(process.env.MT5_JOURNAL_TIMEOUT_MAX_SEC, 75);
  return Math.max(40, Math.min(scaled, cap));
}
...
async function countInFlightLoginOnPort(vpsId, portNo, excludeAccountId = 0) {
  const nid = num(vpsId);
  const pno = normalizeSystemFolderPortNo(portNo);
  if (!nid || !pno) return 0;
  const portExpr = payloadFolderPortSql('payload');
  const r = await query(
    `
    SELECT COUNT(*)::int AS c
    FROM vps_system.vps_agent_commands
    WHERE (vps_id = $1 OR node_id = $1)
      AND LOWER(TRIM(COALESCE(command_type, ''))) IN ('login_mt5', 'connect_mt5')
      AND LOWER(TRIM(COALESCE(status, ''))) IN ('pending', 'processing', 'picked', 'running')
      AND ${portExpr} = $2
      AND ($3::bigint <= 0 OR COALESCE((payload->>'accountId')::bigint, 0) <> $3)
  `,
    [nid, pno, num(excludeAccountId)]
  ).catch(() => ({ rows: [{ c: 0 }] }));
  return num(r.rows?.[0]?.c, 0);
}

async function countInFlightRunOnPort(vpsId, portNo, excludeAccountId = 0) {
  const nid = num(vpsId);
  const pno = normalizeSystemFolderPortNo(portNo);
  if (!nid || !pno) return 0;
  const portExpr = payloadFolderPortSql('payload');
  const r = await query(
    ...
    FROM vps_system.vps_agent_commands
    WHERE (vps_id = $1 OR node_id = $1)
      AND LOWER(TRIM(COALESCE(command_type, ''))) IN ('run_mt5_bot', 'run_mt5')
      AND LOWER(TRIM(COALESCE(status, ''))) IN ('pending', 'processing', 'picked', 'running')
      AND ${portExpr} = $2
      AND ($3::bigint <= 0 OR COALESCE((payload->>'accountId')::bigint, 0) <> $3)
  `,
...
async function computeFolderPortQueueDelaySec(vpsId, accountId = 0, assignedPortNo = 0, kind = 'login') {
  const myPort = normalizeSystemFolderPortNo(assignedPortNo);
  ...
  // FolderPort ล็อกแล้ว — หน่วงเฉพาะเมื่อพอร์ตเดียวกันมีคำสั่งค้าง (รันข้ามพอร์ตพร้อมกันได้)
  if (isRunBot) {
    const samePortBusy = await countInFlightRunOnPort(vpsId, myPort, myAcc);
    return samePortBusy > 0 ? stagger : 0;
  }

  const samePortBusy = await countInFlightLoginOnPort(vpsId, myPort, myAcc);
  return samePortBusy > 0 ? stagger : 0;
}
...
function connectPollStaleLimitMs() {
  const cap = num(process.env.MT5_JOURNAL_TIMEOUT_MAX_SEC, 75);
  const minWaitSec = num(process.env.MT5_CONNECT_POLL_TIMEOUT_SEC, 120);
  return Math.max(minWaitSec, cap + 60) * 1000;
}
