...
async function computeFolderPortQueueDelaySec(vpsId, accountId = 0, assignedPortNo = 0, kind = 'login') {
  ...
  // Login คนละ FolderPort บน VPS เดียวกัน — ไม่หน่วงข้ามพอร์ต (worker แยกต่อ port แล้ว)
  if (!isRunBot) {
  ...
  let queue = await listCommandQueueByFolderPort(vpsId, {
    commandTypes: RUNBOT_COMMAND_TYPES,
    includeConnectingAccounts: false,
    includePendingBots: true
  });
