...
/** รอ Journal — เพิ่มเมื่อมี login ค้างหรือ MT5 ตัวอื่นรันอยู่บน VPS เดียวกัน */
function computeJournalTimeoutSec(opts = {}) {
  ...
  const runningMt5 = Math.max(0, num(opts.runningMt5Count, 0));
  const otherRunning = Math.max(0, runningMt5 - 1);
  const load = Math.max(activeLogins, otherRunning);
  const extra = Math.min(90, load * 30);
  const scaled = base + extra;
  const cap = num(process.env.MT5_JOURNAL_TIMEOUT_MAX_SEC, 180);
  const floor = load > 0 ? 60 : 28;
  return Math.max(floor, Math.min(scaled, cap));
}
...
/** MT5 ที่รันอยู่บน VPS (จาก health) — ใช้เพิ่ม timeout ตอน login PORT 2+ */
 ...
 */
async function computeLoginQueueDelaySec(vpsId, accountId = 0, opts = {}) {
  const running = await countRunningMt5OnVps(vpsId);
  const busy = await hasOtherLoginWorkOnVps(vpsId, accountId);
  if (running > 0 || busy) {
    const cap = opts.equityFast ? 2 : 10;
    const def = opts.equityFast ? 2 : 6;
    return Math.min(cap, num(process.env.MT5_LOGIN_QUEUE_DELAY_SEC, def));
  }
  return 0;
}
