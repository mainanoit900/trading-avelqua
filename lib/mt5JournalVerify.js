...
function parseMt5JournalOutcome(text, mt5Login, serverName, sinceMs) {
  ...
  /** May-16 last-wins + sinceMs — บรรทัดล่าสุด (ตามเวลาใน journal) เป็นผลจริง */
  let last = null;
  let lastMs = -1;
  ...
  for (const line of lines) {
    ...
    if (failRx.test(line) || failedWords.some((w) => low.includes(w))) {
      outcome = 'failed';
    } else if (low.includes('authorization on') && low.includes('failed')) {
      outcome = 'failed';
    } else if (okRx.test(line)) {
      outcome = 'success';
    } else if (
    ...
    if (outcome) {
      const orderMs = lineMs != null ? lineMs : lastMs + 1;
      if (orderMs >= lastMs) {
        lastMs = orderMs;
        last = outcome;
      }
  ...
  // May-16 fallback: success-only rescan ไม่กรองเวลา (clock skew) — อย่า override fail รอบปัจจุบัน
  if (since > 0 && last !== 'success') {
    for (const line of lines) {
      const low = line.toLowerCase();
      if (!low.includes(login.toLowerCase())) continue;
      if (!low.includes(server.toLowerCase())) continue;
      if (okRx.test(line)) return 'success';
      if (
        low.includes('authorized on') &&
        !low.includes('failed') &&
        low.includes(login.toLowerCase())
      ) {
        return 'success';
      }
    }
