...
function parseMt5JournalOutcome(text, mt5Login, serverName, sinceMs) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (
      low.includes('authorized on') &&
      !low.includes('failed') &&
      (low.includes("'") || low.includes('"') || low.includes(login.toLowerCase()))
    ) {
      return 'success';
    }
  }

  // May-16 fallback: sinceMs อาจพลาดบรรทัด success ถ้า clock skew — สแกนใหม่โดยไม่กรองเวลา
  if (since > 0) {
    for (let i = lines.length - 1; i >= 0; i--) {
      const line = lines[i];
      const low = line.toLowerCase();
      ...
      if (low.includes('authorized on') && !low.includes('failed') && low.includes(login.toLowerCase())) {
        return 'success';
      }
      if (successWords.some((w) => low.includes(w)) && !failedWords.some((w) => low.includes(w))) {
        return 'success';
      }
    }
  }
  ...
  if (blob.includes(login.toLowerCase())) {
    if (failedWords.some((w) => blob.includes(w))) return 'failed';
    if (successWords.some((w) => blob.includes(w))) return 'success';
  }

  const serverFail = parseMt5JournalServerAuthFailure(text, server, sinceMs);
  if (serverFail === 'failed') return 'failed';

  return null;
}

/** ล้มเหลวเมื่อ Journal มี authorization failed บน Server นี้ (ไม่จำกัดเลข login ตรงทุกตัว) */
function parseMt5JournalServerAuthFailure(text, serverName, sinceMs) {
  const server = String(serverName || MT5_LOCKED_SERVER).trim();
  if (!text || !server) return null;

  const since = Number(sinceMs || 0) > 0 ? Number(sinceMs) - 5000 : 0;
  const serverEsc = server.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const failRx = new RegExp(`authorization on\\s+${serverEsc}\\s+failed\\b`, 'i');
  const lines = String(text).split(/\r?\n/).filter((l) => l.trim());
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (
      failRx.test(line)
      || (low.includes('authorization on') && low.includes('failed'))
      || /failed\s*[\(\[]?\s*invalid account/i.test(low)
    ) {
      return 'failed';
    }
  }
...
/** Journal มี authorized ของ login อื่น (session เก่าค้างบน PORT) */
function parseMt5JournalForeignLogin(text, expectedLogin, serverName, sinceMs) {
  const expected = String(expectedLogin || '').trim();
  const server = String(serverName || MT5_LOCKED_SERVER).trim();
  if (!expected || !text || !server) return null;

  const since = Number(sinceMs || 0) > 0 ? Number(sinceMs) - 5000 : 0;
  const serverLow = server.toLowerCase();
  const lines = String(text).split(/\r?\n/).filter((l) => l.trim());

  for (let i = lines.length - 1; i >= 0; i--) {
    const line = lines[i];
    const low = line.toLowerCase();
    if (!low.includes(serverLow)) continue;
    if (since > 0) {
      const lineMs = parseMt5JournalLineTimeMs(line);
      if (lineMs == null) continue;
      if (lineMs < since) continue;
    }
    const m = line.match(/'(\d{5,10})'\s*:\s*authorized on\b/i);
    if (m && m[1] && m[1] !== expected) {
      return m[1];
    }
  }
  return null;
...
function resolveLoginFailUserMessage(opts = {}) {
  ...
  if (/MT5_LOGIN_AUTH_FAILED/i.test(blob)) {
    return { message: MT5_FAIL_USER_MSG, authFail: true, journalVerdict: 'failed' };
  }
  ...
  if (/open account|ค้างหน้า\s*open/i.test(blob)) {
    return {
      message:
        'MT5 ค้างหน้าเปิดบัญชี — ปิด MT5 แล้วเชื่อมต่อใหม่ หรือตรวจ Login/รหัสผ่านให้ตรง Server',
      authFail: false,
      journalVerdict: 'wizard_stuck',
    };
  }

  if (
    /MT5_JOURNAL_TIMEOUT|MT5_LOGIN_AUTH_FAILED|MT5_LOGIN_FAILED/i.test(blob) ||
    /ทันเวลา|timeout|ไม่สามารถยืนยัน\s*login|journal\s+login\s+failed|journal\s+read\s+timeout|cancelled:\s*journal/i.test(
      blob
    )
  ) {
