...
async function enforceEntitlementForUser(userId) {
  ...
  // Determine included max ports + current package group from active subscription (if any).
  ...
  // Count active extra ports that are not expired.
  // - temporary: counts regardless of group (but disappears when expires_at hits or package expired elsewhere)
  // - permanent: counts only when group matches current package group (tier-bound)
