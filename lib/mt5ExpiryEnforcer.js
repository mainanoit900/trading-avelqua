...
const forceStopAttemptCount = new Map();
const forceStopAttemptWindowUntil = new Map();

const FORCE_STOP_MAX_RETRIES = Math.max(
  1,
  Number(process.env.MT5_EXPIRY_FORCE_STOP_MAX_RETRIES || 2)
);
const FORCE_STOP_RETRY_WINDOW_SEC = Math.max(
  60,
  Number(process.env.MT5_EXPIRY_FORCE_STOP_RETRY_WINDOW_SEC || 3600)
);
...
function touchForceStopCooldown(nodeId, portNo, sec = 30) {
...
function refreshForceStopAttemptWindow(nodeId, portNo) {
  const key = portForceStopKey(nodeId, portNo);
  const now = Date.now();
  const until = forceStopAttemptWindowUntil.get(key) || 0;
  if (now > until) {
    forceStopAttemptWindowUntil.set(
      key,
      now + (FORCE_STOP_RETRY_WINDOW_SEC * 1000)
    );
    forceStopAttemptCount.set(key, 0);
  }
  return key;
}

function canIssueForceStop(nodeId, portNo) {
  const key = refreshForceStopAttemptWindow(nodeId, portNo);
  const count = Number(forceStopAttemptCount.get(key) || 0);
  return count < FORCE_STOP_MAX_RETRIES;
}

function markForceStopIssued(nodeId, portNo) {
  const key = refreshForceStopAttemptWindow(nodeId, portNo);
  const count = Number(forceStopAttemptCount.get(key) || 0);
  forceStopAttemptCount.set(key, count + 1);
}

async function hasPendingStopCommand(vpsId, portNo) {
  const vid = num(vpsId);
...
/** @deprecated use hasPendingStopCommand */
async function hasRecentOrPendingStop(vpsId, portNo) {
  return hasPendingStopCommand(vpsId, portNo);
}
...
async function findForceStopFromPendingCommands(nodeId) {
  const nid = num(nodeId);
  if (!nid) return [];
  const rows = await query(
    `
    ...
    WHERE (vps_id = $1 OR node_id = $1)
      ...
      AND status IN ('pending', 'processing')
      AND COALESCE(payload->>'reason', '') LIKE 'package_expired%'
    ORDER BY id DESC
    LIMIT 20
  `,
    [nid]
  ...
  for (const row of rows.rows || []) {
    ...
    const portNo = num(payload.port || payload.portNumber || payload.assignedPortNo);
    if (!portNo || seen.has(portNo)) continue;
    const uid = num(payload.userId || payload.user_id);
    if (uid && (await userHasActivePackage(uid))) continue;
    seen.add(portNo);
    const folderPath = String(payload.folder_path || payload.vpsFolderPath || '').trim();
    if (!folderPath) continue;
    if (isForceStopOnCooldown(nid, portNo)) continue;
    if (!canIssueForceStop(nid, portNo)) continue;
    touchForceStopCooldown(nid, portNo);
    markForceStopIssued(nid, portNo);
    kills.push({
      port: portNo,
      port_no: portNo,
      portNumber: portNo,
      folder_path: folderPath,
      vpsFolderPath: folderPath,
      mt5_login: payload.mt5Login || payload.mt5_login || null,
      userId: num(payload.userId || payload.user_id),
      reason: payload.reason || 'package_expired',
      forceKill: true,
      closeMt5: true,
      killMt5: true
    });
...
async function mergeForceStopPorts(nodeId, ...lists) {
  const seen = new Set();
  const merged = [];
  for (const list of lists) {
    for (const item of list || []) {
      const portNo = num(item.port || item.port_no || item.portNumber);
      if (!portNo || seen.has(portNo)) continue;
      seen.add(portNo);
      merged.push(item);
    }
...
const userExpirySweepAt = new Map();
...
async function sweepNodePackageExpiry(nodeId, livePorts) {
  ...
  if (now - last >= throttleMs) {
    ...
    const liveKills = Array.isArray(livePorts) && livePorts.length
      ? await findExpiredPackageKillPorts(nid, livePorts)
      : [];
    const userIds = new Set(
      liveKills.map((k) => num(k.userId)).filter(Boolean)
    );
    for (const uid of userIds) {
      const userLast = userExpirySweepAt.get(uid) || 0;
      if (now - userLast < throttleMs) continue;
      userExpirySweepAt.set(uid, now);
      if (await userHasActivePackage(uid)) continue;
      await applyPackageExpiredSideEffects(uid, 'package_expired_auto_stop').catch(() => {});
    }
  }

  const fromLive = Array.isArray(livePorts) && livePorts.length
    ? await findExpiredPackageKillPorts(nid, livePorts)
    : await findExpiredKillPortsFromPortHealth(nid);
  const fromPending = await findForceStopFromPendingCommands(nid);
  return mergeForceStopPorts(nid, fromLive, fromPending);
}
...
/** แพ็กเกจหมดอายุ: ปิด MT5/BOT + เคลียร์ FolderPort + ตัดสิทธิ์ PORT ชั่วคราว */
async function applyPackageExpiredSideEffects(userId, reason = 'package_expired_auto_stop') {
  const uid = num(userId);
  if (!uid) return { ok: false };
  const { cleanupUserOnPackageExpired } = require('./mt5PackageExpire');
  const cleanup = await cleanupUserOnPackageExpired(uid, reason).catch(() => ({
    stoppedPorts: 0,
    accountsCleared: 0
  }));
  ...
  return {
    ...
    ...stop,
    ...cleanup,
    deactivatedTemporary: tmp.deactivated
  };
}
...
async function collectKillTargetsForUser(userId) {
  ...
  const inst = await query(
    ...
    SELECT bi.id, bi.status, bi.vps_id, bi.port_id, bi.assigned_port_no,
           ...
           COALESCE(NULLIF(TRIM(COALESCE(bi.mt5_login, '')), ''), COALESCE(bi.run_payload->>'mt5Login', '')) AS mt5_login,
  ...
  // SAFETY: หลีกเลี่ยงปิด MT5 ของ user อื่นจากข้อมูล lock/port_health ที่อาจ stale
  // ใช้เฉพาะ target ที่ผูกกับ bot_instances / mt5_accounts ของ user นี้เท่านั้น
...
async function enqueueStopAllForUser(userId, reason = 'package_expired_auto_stop', closeMt5 = true) {
  ...
  for (const t of targets) {
    const expectedLogin = String(t.extra.mt5Login || '').trim();
    if (
      String(reason || '').toLowerCase().includes('package_expired')
      && !expectedLogin
    ) {
      continue;
    }
    const payload = killPayload(t.portNo, t.folderPath, reason, {
      ...
      mt5Login: expectedLogin || null,
      instanceId: t.extra.instanceId || null,
      expectedMt5Login: expectedLogin || null
    });
    if (!payload) continue;
    if (await hasPendingStopCommand(t.vpsId, t.portNo)) continue;
