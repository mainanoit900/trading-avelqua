...
async function queueReadEquityFile(vpsId, ctx, accountId, userId) {
  const { rel } = equityFilePaths(ctx);
  const portReadId = await queuePortReadFile(vpsId, ctx, accountId, userId, rel, 'equity_sync');
...
/**
 * ดึง Equity ทันทีหลัง Login สำเร็จ (รอ snapshot สั้นๆ ถ้ายังไม่มีใน DB)
 */
async function ensureEquityOnConnect(accountId, userId, loadCtx, options = {}) {
  ...
  const fetched = await fetchEquityFromVps(ctx, accountId, userId, {
    waitMs,
    skipJournal: options.skipJournal !== false
  }).catch(() => ({ ok: false }));
  if (fetched?.ok && (fetched.balance || fetched.equity)) return fetched;
  if (waitMs > 0) {
    fetchEquityFromVps(ctx, accountId, userId, { waitMs: 0, skipJournal: true }).catch(() => {});
  }
  return fetched;
}
