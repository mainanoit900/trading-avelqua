...
async function fetchEquityLight(ctx, accountId, userId, options = {}) {
  ...
  if (!metrics || (!metrics.balance && !metrics.equity)) {
    metrics = metricsFromCommandResult(recent.rows?.[0]?.result, ctx.portNo);
  }
  if (metrics && (metrics.balance || metrics.equity)) {
  ...
  const cmdId = await queueAccountSnapshot(vpsId, syncPayload).catch(() => 0);
  ...
  const row = await waitCommandResult(cmdId, Math.min(waitMs, 6000));
