...
function equityFilePaths(ctx) {
  ...
  const rels = [
    'MQL5\\Files\\avelqua_account.txt',
    'Terminal\\Common\\Files\\avelqua_account.txt'
  ];
  return { abs: base ? `${base}\\${rels[0]}` : rels[0], rel: rels[0], rels };
}
...
async function queueReadEquityFile(vpsId, ctx, accountId, userId, purpose = 'equity_sync') {
  const { rels } = equityFilePaths(ctx);
  const ids = [];
  for (const rel of rels) {
    const portReadId = await queuePortReadFile(vpsId, ctx, accountId, userId, rel, purpose || 'equity_sync');
    if (portReadId) ids.push(portReadId);
  }
  return { readId: 0, portReadId: ids[0] || 0, portReadIds: ids };
}
...
async function queueReadMt5JournalLogs(vpsId, ctx, accountId, userId, maxDays = 1) {
  ...
  for (const stamp of mt5LogDateCandidates(maxDays)) {
    ...
    relPaths.push(`Logs\\${stamp}.log`);
    // mirror จาก Agent sync ไป Common/Files
    relPaths.push('Terminal\\Common\\Files\\avelqua_journal_latest.log');
    relPaths.push('config\\journal.ini');
    relPaths.push('config\\experts.ini');
    relPaths.push('Terminal\\Common\\Files\\avelqua_journal.ini');
    relPaths.push('Terminal\\Common\\Files\\avelqua_experts.ini');
  }
...
async function ensureEquityOnConnect(accountId, userId, loadCtx, options = {}) {
  ...
  if (options.forceFresh === true) {
    return fetchPostLoginEquityOnce(accountId, userId, loadCtx, options);
  }
...
/** หลัง Login สำเร็จ — ดึง Equity ล่าสุดจาก VPS 1 ครั้ง (รอผลจริง) */
async function fetchPostLoginEquityOnce(accountId, userId, loadCtx, options = {}) {
  ...
  if (!uid) {
    ...
    uid = Number(row.rows?.[0]?.user_id || 0);
  }
  if (!uid) return { ok: false, reason: 'MISSING_USER' };

  const { loadAccountPortContext } = require('./mt5AccountPort');
  const ctx =
    typeof loadCtx === 'function'
      ? await loadCtx(accountId, uid)
      : loadCtx || (await loadAccountPortContext(accountId, uid));
  if (!ctx?.vpsId) return { ok: false, reason: 'NO_CTX' };

  const waitMs = Number(options.waitMs || 15000);
  return fetchEquityFromVps(ctx, accountId, uid, {
    waitMs,
    forceFresh: true,
    light: true,
    skipJournal: false,
    purpose: 'equity_post_login'
  });
}
