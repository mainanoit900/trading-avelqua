 ...
 */
async function findMt5LoginInUse(
  mt5Login,
  serverName,
  currentUserId,
  exceptAccountId = null,
  requestedPortSlot = null
) {
  ...
  const sameUser = Number(row.user_id) === Number(currentUserId);
  const reqSlot = Number(requestedPortSlot || 0);
  const rowSlot = Number(row.port_slot || 0);
  // ยืนยันซ้ำบน PORT แพ็กเกจเดิม (reconnect) — ไม่บล็อก
  if (sameUser && reqSlot > 0 && rowSlot === reqSlot) {
    return null;
  }
  ...
  return {
    ...row,
    sameUser,
    requestedPortSlot: reqSlot || null
  };
}
...
async function findUserActivePortInUse(currentUserId, exceptAccountId = null) {
  ...
  fallbackSql += `
    ...
    WHERE h.running = TRUE
      AND h.updated_at > NOW() - INTERVAL '45 seconds'
...
function mt5LoginInUseMessage(row) {
  ...
  if (row.sameUser) {
    const req = Number(row.requestedPortSlot || 0);
    if (req > 0 && Number(portHint) !== req) {
      return (
        `บัญชี MT5 นี้อยู่ที่ PORT ${portHint} แล้ว — ` +
        `login พร้อมกันได้คนละบัญชีบน PORT ว่าง (เช่น PORT ${req}) ` +
        `หรือลบ PORT ${portHint} ก่อน`
      );
    }
    return `บัญชี MT5 นี้กำลังใช้งานอยู่ที่ PORT ${portHint} — ไม่สามารถ Login ซ้ำข้าม PORT ได้`;
  }
