...
async function upsertAccountForPortSlot(queryFn, opts = {}) {
  ...
  // ล้าง account เก่าบน VPS folder เดิมก่อน INSERT — ไม่แตะบัญชีที่บอทยังรัน
  if (vpsId && allocPortNo) {
    const variants = portNoVariants(allocPortNo);
    if (variants.length) {
      await queryFn(
        `
        UPDATE vps_system.mt5_accounts
        SET status='expired', assigned_port_no=NULL, windows_port_no=NULL,
            vps_id=NULL, port_id=NULL,
            last_login_message='ถูกแทนที่ด้วยการเชื่อมต่อใหม่', updated_at=NOW()
        WHERE vps_id=$1 AND assigned_port_no = ANY($2::int[])
          AND LOWER(COALESCE(status,'')) IN ('connecting','checking','starting','connected','ready')
          AND user_id=$3
          AND NOT EXISTS (
            SELECT 1
            FROM vps_system.bot_instances bi
            WHERE bi.mt5_account_id = mt5_accounts.id
