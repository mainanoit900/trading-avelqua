...
const { positiveMoney, queueReadEquityFile } = require('./mt5EquitySync');
...
async function pollEquityTick() {
  ...
  try {
    ...
    for (const acc of rows.rows || []) {
      ...
      await queueReadEquityFile(vpsId, ctx, accountId, userId, 'equity_poller').catch(() => {});
      queued += 1;
    }
    if (queued > 0 && process.env.MT5_EQUITY_POLL_DEBUG === '1') {
      console.log(`[mt5-equity-poller] queued ${queued} port_read_file`);
    }
  } catch (e) {
