...
} = require('./mt5EquitySync');

const POLL_MS = Number(process.env.MT5_EQUITY_POLL_MS || 30 * 60 * 1000);
const MAX_ACCOUNTS_PER_TICK = Number(process.env.MT5_EQUITY_POLL_BATCH || 24);
const MIN_GAP_SEC = Number(process.env.MT5_EQUITY_POLL_GAP_SEC || 28 * 60);
    ...
    for (const acc of rows.rows || []) {
      ...
      if (positiveMoney(acc.last_equity)) {
        const fresh = await query(
          ...
          WHERE id=$1 AND updated_at > NOW() - INTERVAL '28 minutes'
          LIMIT 1
        `,
          [accountId]
        ).catch(() => ({ rows: [] }));
      ...
      if (!vpsTouched.has(vpsId)) {
        vpsTouched.add(vpsId);
      ...
      await queueAccountSnapshot(vpsId, syncPayload, { commandType: 'account_snapshot' }).catch(
        () => {}
      );
      queued += 1;
    }
...
async function cancelAllPendingEquityFileReads() {
  const r = await query(
    `
    ...
    SET status = 'cancelled',
        ...
        updated_at = NOW()
    WHERE command_type IN ('port_read_file', 'dashboard')
      AND (
        COALESCE(payload->>'purpose', '') IN ('equity_sync', 'equity_connect', 'equity_poller', 'equity_page_load', 'equity_refresh')
        OR COALESCE(payload->>'action', '') = 'dashboard'
      )
  ...
  if (n > 0) console.log(`[mt5-equity-poller] cancelled ${n} pending port_read_file equity commands`);
  return n;
}
...
function startMt5EquityPoller() {
  ...
  cancelAllPendingEquityFileReads().catch(() => {});
