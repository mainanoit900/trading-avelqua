...
const WINDOW_PROOF_TIMEOUT_MS = Number(process.env.MT5_CONNECT_WINDOW_PROOF_TIMEOUT_MS || 300000);
...
function isRemovedAccountStatus(status) {
  const st = String(status || '').trim().toLowerCase();
  return st === 'deleted' || st === 'cancelled' || st === 'expired';
}

/** Only the explicitly linked attempt may mutate account rows (prevents revive after delete). */
function isAttemptCurrentForAccount(attempt) {
  const currentId = clean(attempt?.current_attempt_id);
  if (!currentId || !attempt?.attempt_id) return false;
  return currentId === String(attempt.attempt_id);
}

const PROMOTABLE_ACCOUNT_STATUSES = new Set([
  'ready',
  'checking',
  'connecting',
  'starting'
...
/** Allow promote when current_attempt_id drifted but this is the latest connected attempt. */
async function canPromoteAccountFromAttempt(attempt) {
  if (!attempt?.account_id || !attempt?.attempt_id) return false;
  if (!(await accountAllowsConnectMutation(attempt.account_id))) return false;
  if (isAttemptCurrentForAccount(attempt)) return true;
  ...
  const accountStatus = String(accRes.rows?.[0]?.status || '').trim();
  if (!PROMOTABLE_ACCOUNT_STATUSES.has(accountStatus)) return false;

  const latest = await getLatestAttemptForAccount(Number(attempt.account_id)).catch(() => null);
  if (!latest?.attempt_id || String(latest.attempt_id) !== String(attempt.attempt_id)) {
    return false;
  }
  return Boolean(latest.terminal && normalizeAttemptStatus(latest.status) === 'connected');
}

async function accountAllowsConnectMutation(accountId) {
  const r = await query(
    `
    SELECT LOWER(COALESCE(status, '')) AS status
    FROM vps_system.mt5_accounts
  ...
  if (!st) return false;
  return !isRemovedAccountStatus(st);
}
...
const FINALIZE_EQUITY_SOURCES = new Set([
  ...
  'window_title',
  'login_command_window',
  'login_command_window_pending'
]);
...
function isLoginFastConnectEnabled() {
  return String(process.env.MT5_LOGIN_FAST_CONNECT || '1').trim() !== '0';
}

/** Promote to connected after journal/login proof — equity fetch runs in background. */
async function tryPromoteJournalVerifiedFast(attempt, opts = {}) {
  if (!isLoginFastConnectEnabled() || !attempt?.attempt_id) return false;
  const fresh = await getAttemptById(attempt.attempt_id).catch(() => null);
  if (!fresh || isAttemptTerminal(fresh.status)) return false;
  const journalOk = Boolean(fresh.journal_verified || opts.journalVerified);
  if (!(await hasSuccessfulLoginCommand(fresh)) && !journalOk) return false;
  if (!attemptHasVerifiedLoginProof(fresh) && !journalOk) return false;
  if (!(await canPromoteAccountFromAttempt(fresh))) return false;

  const finalMsg = clean(opts.message) || MT5_SUCCESS_MSG;
  const observed = clean(opts.observedLogin) || clean(fresh.observed_login) || clean(fresh.mt5_login);
  ...
  await updateAttemptRow(fresh.attempt_id, {
    ...
    evidenceSource: clean(opts.evidenceSource) || 'journal_fast_connect',
  ...
  await cancelPendingVerifyForAttempt(fresh).catch(() => {});
  return true;
}
...
async function tryRecoverTimedOutAttempt(attempt, accountRow = null) {
  ...
  attempt = await syncAccountWindowProofToAttempt(attempt, accountRow).catch(() => attempt);
  ...
  const expected = clean(attempt.mt5_login);
  let bal = moneyMetric(attempt.balance) ?? moneyMetric(accountRow?.last_balance);
  let eq = moneyMetric(attempt.equity) ?? moneyMetric(accountRow?.last_equity);
  if (bal === null && eq === null) {
    const hist = await loadHistoricalMetricsForAccount(attempt.account_id).catch(() => ({
      balance: null,
      equity: null,
      ready: false
    }));
...
async function createConnectAttempt({
  ...
  if (!client) await ensureMt5ConnectAttemptTables();
  const run = client?.query ? (sql, p) => client.query(sql, p) : query;
...
async function updateAccountForAttempt(attempt, patch = {}) {
  ...
  await query(
    ...
    WHERE id=$1
      ...
      AND LOWER(COALESCE(status, '')) NOT IN ('deleted', 'cancelled')
  `,
    vals
  ).catch(() => {});
}
...
async function queueBotRunRetryAfterMismatch(attempt, wrongLogin = '') {
  ...
  if (!runPayload || typeof runPayload !== 'object' || !clean(runPayload.botCode)) {
    return false;
  }
  ...
  const common = {
    ...
    vpsFolderPath: clean(attempt.folder_path) || runPayload.vpsFolderPath || undefined,
    folder_path: clean(attempt.folder_path) || runPayload.folder_path || undefined,
    folderPath: clean(attempt.folder_path) || runPayload.folderPath || undefined,
    forceKill: true,
    closeMt5: true,
    clearSession: true,
  ...
  await query(
    ...
    UPDATE vps_system.vps_agent_commands
    SET status='cancelled',
        error='cancelled: title_mismatch_retry',
        finished_at=NOW(),
        updated_at=NOW(),
        result_message='cancelled: title_mismatch_retry'
    WHERE (vps_id = $1 OR node_id = $1)
      AND COALESCE(payload->>'attemptId', '') = $2
  ...
  await queueAttemptCommand(
    ...
    'run_mt5_bot',
    {
      ...
      purpose: 'title_mismatch_retry_run',
      purposeType: 'bot_run',
      keepMt5Open: true
    },
    'title_mismatch_retry_run'
  );
  return true;
}
...
async function queueLoginRetryAfterMismatch(attempt, wrongLogin = '') {
  ...
  if (clean(attempt.purpose_type || '').toLowerCase() === 'bot_run') {
    return queueBotRunRetryAfterMismatch(attempt, wrongLogin);
  }
...
/** login_only บน VPS: ปิด MT5 ได้เมื่อดึง Equity จาก MT5 จริงแล้วเท่านั้น */
const LIVE_LOGIN_EQUITY_SOURCES = new Set([
  'login_equity_fetch',
  'snapshot',
  'connect_result_equity',
  'login_command_metrics',
...
async function shouldQueueLoginExit(attempt) {
  const purpose = clean(attempt?.purpose_type || 'login_only').toLowerCase();
  if (purpose === 'bot_run') return false;
  const vpsId = Number(attempt?.vps_id || 0);
  if (!vpsId) return false;
  const src = clean(attempt?.evidence_source).toLowerCase();
  if (src === 'cached_equity') return false;
  const metrics = resolveAttemptMetrics(attempt);
  if (!metrics.ready) return false;
  // Equity จาก MT5 จริงแล้ว — ปิดได้แม้ login_mt5 callback ยังมาไม่ทัน
  if (await hasSuccessfulLoginEquityFetch(attempt).catch(() => false)) return true;
  if (LIVE_LOGIN_EQUITY_SOURCES.has(src)) return true;
  if (await hasInflightLoginCommand(attempt)) return false;
  return false;
}
...
async function queuePostConnectExitCommands(attempt) {
  ...
  if (!(await shouldQueueLoginExit(attempt))) {
    return { ok: false, skipped: true, reason: 'LIVE_EQUITY_REQUIRED' };
  }
  const metrics = resolveAttemptMetrics(attempt);
  if (!metrics.ready) {
  ...
  if (purpose === 'login_only' && vpsId && portNo) {
    const uid = attempt.user_id ? Number(attempt.user_id) : null;
    const login = clean(attempt.mt5_login);
    if (portId) {
      await query(
        `
        ...
        SET status='locked',
            ...
            mt5_login=COALESCE(NULLIF($2, ''), mt5_login),
            current_mt5_login=COALESCE(NULLIF($2, ''), current_mt5_login),
            process_id=NULL,
            last_pid=NULL,
            updated_at=NOW()
        WHERE id=$1
    ...
    `,
      [vpsId, portNoVariants(portNo)]
    ).catch(() => {});
    const { resolveSystemVpsId, setAdminAllocationStatus } = require('./adminVpsBridge');
    const { adminNodeId } = await resolveSystemVpsId(vpsId).catch(() => ({}));
    const adminPort = portNo >= 100 ? portNo - 100 : portNo;
    if (adminNodeId && adminPort >= 1 && adminPort <= 20) {
      await setAdminAllocationStatus(adminNodeId, adminPort, 'locked').catch(() => {});
    }
  }
...
function accountWindowProofOk(accountRow, expectedLogin) {
  const expected = clean(expectedLogin || accountRow?.mt5_login);
  if (!expected || !accountRow) return false;
  const title = clean(accountRow.mt5_window_title);
  return Boolean(title && title.includes(expected));
}

function attemptHasWindowLoginProof(attempt, accountRow = null) {
  const expected = clean(attempt?.mt5_login);
  if (!expected) return false;
  if (detectWindowTitleLoginMismatch(clean(attempt?.window_title), expected)) return false;
  if (detectWindowTitleLoginMismatch(clean(accountRow?.mt5_window_title), expected)) return false;
  if (attempt?.snapshot_verified && isExactLoginMatch(expected, attempt?.observed_login)) return true;
  if (isExactLoginMatch(expected, attempt?.observed_login)) return true;
  const title = clean(attempt?.window_title);
  if (title && title.includes(expected)) return true;
  return accountWindowProofOk(accountRow, expected);
}

async function syncAccountWindowProofToAttempt(attempt, accountRow = null) {
  if (!attempt?.attempt_id || isAttemptTerminal(attempt.status)) return attempt;
  const expected = clean(attempt.mt5_login);
  if (!expected) return attempt;

  let accRow = accountRow;
  if (!accRow?.mt5_window_title && attempt.account_id) {
    const acc = await query(
      `
      SELECT mt5_window_title, last_balance, last_equity
      FROM vps_system.mt5_accounts
      WHERE id=$1
      LIMIT 1
    `,
    ...
    accRow = acc.rows?.[0] || null;
  }

  const title = clean(attempt.window_title) || clean(accRow?.mt5_window_title);
  if (!title || !title.includes(expected)) return attempt;

  const observed = loginFromWindowTitle(title, expected) || expected;
  await markAttemptProgress(attempt, {
    status: normalizeAttemptStatus(attempt.status) === 'queued' ? 'starting' : attempt.status,
    windowTitle: title,
    observedLogin: observed,
    snapshotVerified: true,
    evidenceSource: clean(attempt.evidence_source) || 'account_window_title'
  }).catch(() => {});

  return getAttemptById(attempt.attempt_id).catch(() => attempt);
}

async function loadHistoricalMetricsForAccount(accountId) {
  const id = Number(accountId || 0);
  if (!id) return { balance: null, equity: null, ready: false };
  const r = await query(
    `
    SELECT balance, equity
    FROM vps_system.mt5_connect_attempts
    WHERE account_id=$1
      AND metrics_ready=TRUE
      AND (balance IS NOT NULL OR equity IS NOT NULL)
    ORDER BY created_at DESC
    LIMIT 1
  `,
    [id]
  ).catch(() => ({ rows: [] }));
  const balance = moneyMetric(r.rows?.[0]?.balance);
  const equity = moneyMetric(r.rows?.[0]?.equity);
  return { balance, equity, ready: balance !== null || equity !== null };
}

function snapshotLoginMatches(expectedLogin, observedLogin, portProofOk = false, windowTitle = '') {
  const expected = clean(expectedLogin);
  const observed = clean(observedLogin);
  if (!expected) return false;
  if (detectWindowTitleLoginMismatch(windowTitle, expected)) return false;
  ...
  return Boolean(portProofOk && title && title.includes(expected));
}
...
async function syncAccountFromTerminalConnectedAttempt(attempt, opts = {}) {
  if (!attempt?.account_id || !attempt?.attempt_id) return false;
  if (!(await canPromoteAccountFromAttempt(attempt))) return false;
...
async function reopenAttemptForMetrics(attempt) {
  ...
  if (isAttemptCurrentForAccount(attempt)) {
    await query(
      `
      UPDATE vps_system.mt5_accounts
      SET status='checking',
...
async function requeueStuckLoginForAttempt(attempt, stuckSec = 120) {
  if (!attempt?.vps_id || !attempt?.attempt_id) return false;
  const r = await query(
    `
    SELECT id
    ...
    WHERE (vps_id = $1 OR node_id = $1)
      ...
      AND COALESCE(picked_at, started_at, updated_at, created_at)
          < NOW() - ($3::text || ' seconds')::interval
    LIMIT 1
  `,
    [Number(attempt.vps_id), String(attempt.attempt_id), String(stuckSec)]
  ).catch(() => ({ rows: [] }));
  ...
  await query(
    ...
    SET status='pending',
        ...
        finished_at=NULL,
        updated_at=NOW(),
        result_message='requeued_stuck_login_server'
    WHERE id=$1
  `,
    [Number(r.rows[0].id)]
  ).catch(() => {});
...
async function recoverAttemptPortLoginMismatch(attempt) {
  if (!attempt?.vps_id || !attempt?.assigned_port_no || !attempt?.mt5_login) return false;
  if (clean(attempt.purpose_type || '').toLowerCase() === 'bot_run') {
    const inflightRun = await query(
      `
      SELECT id
      FROM vps_system.vps_agent_commands
      WHERE (vps_id = $1 OR node_id = $1)
    ...
    if (inflightRun.rows?.[0]) return false;
  }
  const expected = clean(attempt.mt5_login);
  const attemptStartMs = attempt.connect_started_at
    ? new Date(attempt.connect_started_at).getTime()
    : attempt.created_at
      ? new Date(attempt.created_at).getTime()
      : 0;
  const wrongLogin = await detectPortHealthLoginMismatch(
    Number(attempt.vps_id),
    Number(attempt.assigned_port_no),
    expected,
    attemptStartMs
  ).catch(() => null);
  if (!wrongLogin) return false;
  if (await hasInflightMismatchLoginRetry(attempt)) return true;
  await query(
    `
    UPDATE vps_system.vps_agent_commands
    SET status='cancelled',
        error='cancelled: port_health_login_mismatch',
        finished_at=NOW(),
        updated_at=NOW(),
        result_message='cancelled: port_health_login_mismatch'
    WHERE (vps_id = $1 OR node_id = $1)
      AND COALESCE(payload->>'attemptId', '') = $2
      AND command_type IN ('login_mt5', 'connect_mt5')
      AND LOWER(COALESCE(status, '')) IN ('pending', 'queued', 'picked', 'processing', 'running')
  ...
  await handleTitleMismatchRetry(attempt, wrongLogin, expected, attempt.window_title);
  return true;
}
...
async function ensureAttemptVerificationCommands(attempt) {
  ...
  if (await recoverAttemptPortLoginMismatch(attempt).catch(() => false)) {
    return true;
  }
  ...
  const maxAgeMs =
    attemptHasWindowLoginProof(attempt) || attempt.snapshot_verified
      ? WINDOW_PROOF_TIMEOUT_MS
      : ATTEMPT_TIMEOUT_MS;
...
async function tryFinalizeWindowVerifiedAttempt(attempt, accountRow = null) {
  if (!attempt?.attempt_id) return false;
  const synced = await syncAccountWindowProofToAttempt(attempt, accountRow).catch(() => attempt);
  attempt = synced || attempt;
  const hasLoginCmd = await hasSuccessfulLoginCommand(attempt);
  const hasWindow = attemptHasWindowLoginProof(attempt, accountRow);
  if (!hasWindow) return false;
  if (!hasLoginCmd && !attempt.snapshot_verified) return false;

  let balance = moneyMetric(attempt.balance);
  let equity = moneyMetric(attempt.equity);
  if (balance === null && equity === null) {
    const acc = await query(
      `
      SELECT last_balance, last_equity, mt5_window_title
      FROM vps_system.mt5_accounts
      WHERE id=$1
      LIMIT 1
    `,
    ...
    balance = moneyMetric(acc.rows?.[0]?.last_balance);
    equity = moneyMetric(acc.rows?.[0]?.last_equity);
    // กำลัง login บน VPS — ห้ามใช้ equity เก่า ต้องดึงจาก MT5 จริง
    if (Number(attempt.vps_id || 0) > 0 && !(await hasSuccessfulLoginEquityFetch(attempt).catch(() => false))) {
      balance = null;
      equity = null;
  ...
  const titleLogin =
    loginFromWindowTitle(clean(attempt.window_title), clean(attempt.mt5_login))
    || clean(attempt.observed_login)
    || clean(attempt.mt5_login);
  if (!titleLogin || !isExactLoginMatch(attempt.mt5_login, titleLogin)) return false;

  const metrics = resolveExplicitMetrics(balance, equity);
  if (!metrics.ready) {
    await queueLoginEquityFetch(attempt).catch(() => {});
    return false;
  }
  ...
  return Boolean(
    fin
    && (
      fin.status === 'connected'
      || normalizeAttemptStatus(fin?.status) === 'connected'
      || fin.pending
    )
  );
}
...
async function finalizeAttemptConnected(attempt, {
  ...
  if (!(await accountAllowsConnectMutation(fresh.account_id))) {
  ...
  const attemptIsCurrent = await canPromoteAccountFromAttempt(fresh);
  ...
  if (attemptIsCurrent) {
    await promoteAccountConnected({
      ...
      equity: eq,
      requireMetrics: true,
      skipVpsRestore: clean(evidenceSource) === 'cached_equity'
    }).catch(() => {});
  }
  ...
  if (attemptIsCurrent) {
    ...
    if (fresh.user_id && metrics.ready) {
      try {
        const { pushFlex } = require('../services/lineService');
        const { buildSimpleNoticeFlex } = require('./lineFlexUi');
        const lineRes = await query(
          `SELECT line_user_id FROM line_subscribers WHERE user_id=$1 AND verified=TRUE AND subscribed=TRUE LIMIT 1`,
          [Number(fresh.user_id)]
        ).catch(() => ({ rows: [] }));
        const lineUserId = lineRes.rows?.[0]?.line_user_id;
        if (lineUserId) {
          const login = clean(fresh.mt5_login) || '-';
          const server = clean(fresh.server_name) || '-';
          const balText = bal != null ? `Balance: ${Number(bal).toLocaleString('th-TH', { minimumFractionDigits: 2 })}` : null;
          const eqText = eq != null ? `Equity: ${Number(eq).toLocaleString('th-TH', { minimumFractionDigits: 2 })}` : null;
          const bodyLine = [balText, eqText].filter(Boolean).join(' / ') || '-';
          ...
          await pushFlex(lineUserId, flex.altText, flex.contents).catch(() => {});
        }
      } catch (_) {}
    }
  }
...
async function finalizeAttemptFailed(attempt, message, { evidenceSource } = {}) {
  ...
  const fresh = await getAttemptById(attempt.attempt_id).catch(() => null);
  if (!fresh) return { ok: false, reason: 'ATTEMPT_NOT_FOUND' };
  if (fresh.terminal && isAttemptTerminal(fresh.status)) {
    return { ok: fresh.status === 'failed', status: fresh.status, attempt: fresh };
  }

  const failMsg = clean(message) || MT5_FAIL_USER_MSG;
  const attemptIsCurrent =
    isAttemptCurrentForAccount(fresh) && (await accountAllowsConnectMutation(fresh.account_id));
  const isBotRun = clean(fresh.purpose_type || '').toLowerCase() === 'bot_run';
  if (attemptIsCurrent) {
    await failAccountFromJournal(Number(fresh.account_id), fresh.port_id ? Number(fresh.port_id) : null, failMsg, {
      ...
      purposeType: fresh.purpose_type || 'login_only',
      preservePortSlot: isBotRun
    }).catch(() => {});
  }
  ...
  if (isBotRun) {
    await query(
      ...
      UPDATE vps_system.vps_agent_commands
      SET status='cancelled',
          error=COALESCE(NULLIF(error, ''), 'cancelled: bot run login failed'),
          finished_at=COALESCE(finished_at, NOW()),
          updated_at=NOW()
      WHERE command_type IN ('run_mt5_bot', 'run_mt5')
        AND LOWER(COALESCE(status, '')) = 'pending'
        AND (
          COALESCE(payload->>'attemptId', '') = $1
          OR COALESCE(payload->>'accountId', '') = $2
        )
    `,
      [String(fresh.attempt_id || ''), String(fresh.account_id || '')]
    ).catch(() => {});
    await query(
      `
      ...
      WHERE mt5_account_id=$1
        AND LOWER(COALESCE(status, '')) IN ('pending', 'connecting', 'starting', 'running')
        AND stopped_at IS NULL
    `,
      [Number(fresh.account_id), failMsg]
    ).catch(() => {});
  }
  ...
  if (attemptIsCurrent) {
    ...
    if (fresh.user_id) {
      try {
        const { pushFlex } = require('../services/lineService');
        const { buildErrorFlex } = require('./lineFlexUi');
        const lineRes = await query(
          `SELECT line_user_id FROM line_subscribers WHERE user_id=$1 AND verified=TRUE AND subscribed=TRUE LIMIT 1`,
          [Number(fresh.user_id)]
        ).catch(() => ({ rows: [] }));
...
async function maybeFinalizeAttempt(attemptId) {
  ...
  if (
    ...
    && (attempt.snapshot_verified || attempt.journal_verified || attemptHasWindowLoginProof(attempt))
  ) {
  ...
  if (await tryFinalizeWindowVerifiedAttempt(attempt).catch(() => false)) {
    return getAttemptById(attemptId).catch(() => null);
  }

  const syncedForTimeout = await syncAccountWindowProofToAttempt(attempt).catch(() => attempt);
  if (syncedForTimeout && syncedForTimeout !== attempt) {
    attempt = syncedForTimeout;
    if (await tryFinalizeWindowVerifiedAttempt(attempt).catch(() => false)) {
      return getAttemptById(attemptId).catch(() => null);
    }
  }
  ...
  if (createdAtMs && Date.now() - createdAtMs > (
    attemptHasWindowLoginProof(attempt) || attempt.snapshot_verified
      ? WINDOW_PROOF_TIMEOUT_MS
      : ATTEMPT_TIMEOUT_MS
  )) {
...
async function ingestConnectResultEvent(node, body) {
  ...
  const sinceMs = attempt.created_at ? new Date(attempt.created_at).getTime() : 0;
  let verdict = journalEvidence && mt5Login ? parseMt5JournalOutcome(journalEvidence, mt5Login, undefined, sinceMs) : null;
  if (!verdict && journalEvidence) {
    verdict = parseMt5JournalServerAuthFailure(journalEvidence, undefined, sinceMs);
  }
  ...
  const mergedAttempt = await markAttemptProgress(attempt, {
    ...
    snapshotVerified: windowProofOk ? true : undefined,
    rawLastEvent: body
  }).catch(() => attempt);
  ...
  if (provenConnected) {
    const loginOnlyOnVps =
      clean(mergedAttempt?.purpose_type || attempt?.purpose_type || 'login_only').toLowerCase() === 'login_only'
      && Number(mergedAttempt?.vps_id || attempt?.vps_id || 0) > 0;
    const loginCmdDone = await hasSuccessfulLoginCommand(mergedAttempt || attempt).catch(() => false);
    const equityFetchDone = await hasSuccessfulLoginEquityFetch(mergedAttempt || attempt).catch(() => false);
    const agentSentLiveEquity =
      loginCmdDone && hasMetrics && rawStatus === 'connected' && (windowOk || verdict === 'success');
    if (loginOnlyOnVps && hasMetrics && !equityFetchDone && !agentSentLiveEquity) {
      await markAttemptProgress(mergedAttempt || attempt, {
        status: 'checking',
        message: message || 'Login สำเร็จ — กำลังดึง Equity จาก MT5...',
        observedLogin: observedLogin || mt5Login,
        balance: bal,
        equity: eq,
        snapshotVerified: true,
...
async function ingestCommandResultEvent(node, { commandId, commandType, payload, ok, result, error, message }) {
  ...
  if (ctype === 'login_mt5') {
    ...
    const currentAttempt = await getAttemptById(attempt.attempt_id).catch(() => null);
    if (
      currentAttempt?.terminal
      && normalizeAttemptStatus(currentAttempt.status) === 'connected'
      && resolveAttemptMetrics(currentAttempt).ready
    ) {
      await queuePostConnectExitCommands(currentAttempt).catch(() => {});
      return { ok: true, status: 'connected', attemptId: attempt.attempt_id };
    }

    const winTitle = clean(result?.windowTitle || result?.window_title);
    const titleMismatch = detectWindowTitleLoginMismatch(winTitle, mt5Login);
    if (titleMismatch) {
      return handleTitleMismatchRetry(attempt, titleMismatch, mt5Login, winTitle);
    }

    if (result?.verificationPending === true) {
      await markAttemptProgress(attempt, {
        status: 'checking',
        message: msg || 'MT5 เปิดแล้ว กำลังรอยืนยัน login...',
        windowTitle: winTitle || undefined,
    ...
    if (loginOk || cmdJournalOk) {
      ...
      if (cmdJournalOk && (await tryPromoteJournalVerifiedFast(attempt, {
        message: msg || MT5_SUCCESS_MSG,
        evidenceSource: 'journal_fast_connect',
        observedLogin: clean(result?.observedLogin || result?.observed_login) || mt5Login
      }).catch(() => false))) {
        await queueLoginEquityFetch(attempt).catch(() => {});
        return { ok: true, status: 'connected', attemptId: attempt.attempt_id };
  ...
  if (
    ...
    let verdict = content && mt5Login ? parseMt5JournalOutcome(content, mt5Login) : null;
    if (!verdict && content) {
      const sinceMs = attempt.created_at ? new Date(attempt.created_at).getTime() : 0;
    ...
    if (verdict === 'success') {
      if (await tryPromoteJournalVerifiedFast(attempt, {
        message: 'ยืนยัน login จาก Journal แล้ว',
        evidenceSource: 'journal_fast_connect',
        observedLogin: mt5Login,
        journalVerified: true
      }).catch(() => false)) {
  ...
  if (['account_snapshot', 'sync_mt5_account', 'read_account_metrics'].includes(ctype)) {
    ...
    const portProof =
      attempt.vps_id && attempt.assigned_port_no
        ...
        : { ok: false };
    const loginMatch = snapshotLoginMatches(
      mt5Login,
      observed,
      portProof.ok,
      clean(result?.windowTitle || result?.window_title || attempt.window_title)
    );
    ...
    if (loginMatch && hasMetrics) {
      ...
      if (fin?.status === 'connected' || normalizeAttemptStatus(fin?.status) === 'connected') {
        await queuePostConnectExitCommands(
          (await getAttemptById(attempt.attempt_id).catch(() => null)) || attempt
        ).catch(() => {});
        return { ok: true, status: 'connected', attemptId: attempt.attempt_id };
      }
    }
    ...
    if (loginMatch && !hasMetrics) {
      ...
      return handleTitleMismatchRetry(attempt, observed, mt5Login, attempt.window_title);
    }
...
async function ingestPortHealthEvent(node, portData) {
  ...
  for (const attempt of r.rows || []) {
    ...
    if (conflict) {
      await markAttemptProgress(attempt, {
        status: 'checking',
        message: `พบ session เก่า (${reportedLogin}) บน PORT — กำลังล้างและ login ${expected} ใหม่...`,
        portHealthLogin: reportedLogin,
        evidenceSource: 'port_health_mismatch',
        rawLastEvent: { portHealth: portData }
      }).catch(() => {});
      if (!(await hasInflightMismatchLoginRetry(attempt))) {
        await recoverAttemptPortLoginMismatch(attempt).catch(() => {});
      }
      continue;
    }
...
async function expireStaleConnectAttempts(nodeId = null) {
  ...
  params.push(String(WINDOW_PROOF_TIMEOUT_MS));
  if (nodeId) {
  ...
  for (const attempt of r.rows || []) {
    const acc = await query(
      `SELECT mt5_window_title, mt5_login FROM vps_system.mt5_accounts WHERE id=$1 LIMIT 1`,
      [Number(attempt.account_id || 0)]
    ).catch(() => ({ rows: [] }));
    const synced = await syncAccountWindowProofToAttempt(attempt, acc.rows?.[0]).catch(() => attempt);
    const ageMs = Date.now() - new Date(synced?.created_at || attempt.created_at || Date.now()).getTime();
    const maxMs =
      attemptHasWindowLoginProof(synced || attempt, acc.rows?.[0]) || synced?.snapshot_verified
        ? WINDOW_PROOF_TIMEOUT_MS
        : ATTEMPT_TIMEOUT_MS;
    if (ageMs >= maxMs) {
      await maybeFinalizeAttempt(attempt.attempt_id).catch(() => {});
...
async function repairUserMt5AccountStatuses(userId) {
  ...
  const r = await query(
    ...
    WHERE a.user_id = $1
      AND LOWER(COALESCE(a.status, '')) IN ('checking', 'connecting', 'starting', 'ready')
      AND LOWER(COALESCE(a.status, '')) NOT IN ('deleted', 'cancelled', 'expired')
  ...
  const stuckReady = await query(
    ...
    FROM vps_system.mt5_accounts a
    JOIN LATERAL (
      SELECT attempt_id, status, terminal, balance, equity, last_message, updated_at
      FROM vps_system.mt5_connect_attempts
    ...
    WHERE a.user_id = $1
      ...
      AND LOWER(COALESCE(a.status, '')) = 'ready'
      AND at.terminal = TRUE
      AND LOWER(COALESCE(at.status, '')) = 'connected'
      AND (
        at.balance IS NOT NULL
        OR at.equity IS NOT NULL
        OR a.last_equity IS NOT NULL
        OR a.last_balance IS NOT NULL
      )
  `,
    [uid]
  ).catch(() => ({ rows: [] }));

  for (const row of stuckReady.rows || []) {
    const attempt = await getAttemptById(row.attempt_id).catch(() => null);
    if (attempt) {
      await syncAccountFromTerminalConnectedAttempt(attempt, {
        message: row.attempt_message || row.last_login_message || MT5_SUCCESS_MSG,
...
async function repairOrphanConnectAccounts(userId) {
  const uid = Number(userId || 0);
  if (!uid) return 0;
  const r = await query(
    `
...
let lastExpireStaleConnectAttemptsAt = 0;
const EXPIRE_STALE_ON_POLL_MS = 30000;

const CONNECT_STATUS_ROW_SQL = `
  SELECT
    a.id,
    a.status,
    a.last_error,
...
async function refreshConnectStatusRow(accountId) {
  const r = await query(
    `${CONNECT_STATUS_ROW_SQL} WHERE a.id=$1 LIMIT 1`,
    [Number(accountId)]
  ).catch(() => ({ rows: [] }));
  return r.rows?.[0] || null;
}

/** poll=1: ตรวจ Journal auth fail + finalize — ไม่รัน repair หนัก */
async function pollConnectAttemptOnStatusPoll(row) {
  if (!row?.id) return false;
  const accountId = Number(row.id);
  const vpsId = Number(row.vps_id || 0);
  const login = clean(row.mt5_login);
  const folderPath = clean(row.folder_path);
  const portNo = Number(row.assigned_port_no || 0);
  const portId = Number(row.port_id || 0);
  const attemptStatus = normalizeAttemptStatus(row.attempt_status || row.status);
  const accountStatus = normalizeAttemptStatus(row.status);
  if (accountStatus === 'failed' || isAttemptTerminal(attemptStatus)) return false;

  await syncJournalFromLatestCommand(accountId, vpsId, login, folderPath, portNo).catch(() => ({}));
  const applied = await tryApplyPendingJournalRead(accountId, vpsId).catch(() => false);
  if (applied) return true;

  const refreshed = await refreshConnectStatusRow(accountId).catch(() => null);
  if (refreshed && normalizeAttemptStatus(refreshed.status) === 'failed') {
    return true;
  }

  const readCountRes = await query(
    `
    SELECT COUNT(*)::int AS c
    FROM vps_system.vps_agent_commands
  ...
  const readsDone = Number(readCountRes.rows?.[0]?.c || 0);
  const journalPollFailMax = Number(process.env.MT5_JOURNAL_POLL_FAIL_MAX || 8);
  const hasWindowProof = row.snapshot_verified || attemptHasWindowLoginProof(row, row);
  const metrics = resolveAttemptMetrics(row);
  if (
    readsDone >= journalPollFailMax
    && hasWindowProof
    && !metrics.ready
    && ['checking', 'starting'].includes(attemptStatus)
  ) {
    await failAccountFromJournal(accountId, portId, MT5_FAIL_USER_MSG, {
      vpsId,
      portNo,
      folderPath,
      reason: 'journal_poll_exhausted'
    }).catch(() => {});
    return true;
  }
  ...
  if (
    ...
    await queueJournalReadVerify({ accountId, vpsId, folderPath, mt5Login: login, portNo }).catch(() => {});
    await tryApplyPendingJournalRead(accountId, vpsId).catch(() => false);
  }

  if (row.attempt_id) {
    const attemptRow = await getAttemptById(row.attempt_id).catch(() => null);
    if (attemptRow && !isAttemptTerminal(attemptRow.status)) {
      await maybeFinalizeAttempt(row.attempt_id).catch(() => {});
    }
  }
  return true;
}

async function getConnectStatusForUser(userId, accountId = 0, opts = {}) {
  const poll = opts.poll === true;
  if (!poll) {
    await ensureMt5ConnectAttemptTables();
    await expireStaleConnectAttempts().catch(() => {});
    await repairOrphanConnectAccounts(userId).catch(() => 0);
  } else if (Date.now() - lastExpireStaleConnectAttemptsAt >= EXPIRE_STALE_ON_POLL_MS) {
    await expireStaleConnectAttempts().catch(() => {});
    lastExpireStaleConnectAttemptsAt = Date.now();
  }

  if (accountId) {
    const removed = await query(
      `
      SELECT id, port_slot, LOWER(COALESCE(status, '')) AS status
      FROM vps_system.mt5_accounts
    ...
    const removedSlot = Number(removed.rows?.[0]?.port_slot || 0);
    ...
    if (
      !removedSlot
      && ['checking', 'connecting', 'starting'].includes(removedStatus)
    ) {
      return {
        ok: true,
        connected: false,
        status: 'none',
        message: 'ยังไม่มีรายการเชื่อมต่อ'
      };
    }
  }
  ...
  let where = `a.user_id=$1 AND LOWER(COALESCE(a.status, '')) NOT IN ('deleted', 'cancelled', 'expired')
    AND (
      ...
      OR LOWER(COALESCE(a.status, '')) NOT IN ('checking', 'connecting', 'starting')
    )`;
  if (accountId) {
  ...
  if (row.attempt_id && poll) {
    await pollConnectAttemptOnStatusPoll(row).catch(() => {});
    const refreshedPoll = await refreshConnectStatusRow(row.id).catch(() => null);
    if (refreshedPoll) Object.assign(row, refreshedPoll);
  }

  if (row.attempt_id && !poll) {
    let attemptRow = await getAttemptById(row.attempt_id).catch(() => null);
    if (attemptRow) {
      if (!isAttemptTerminal(attemptRow.status)) {
        attemptRow = await syncAccountWindowProofToAttempt(attemptRow, row).catch(() => attemptRow);
        await ensureAttemptVerificationCommands(attemptRow).catch(() => {});
        await maybeFinalizeAttempt(attemptRow.attempt_id).catch(() => {});
        attemptRow = await getAttemptById(row.attempt_id).catch(() => attemptRow);
      } else if (
        normalizeAttemptStatus(attemptRow.status) === 'connected'
        && clean(attemptRow.purpose_type || 'login_only').toLowerCase() !== 'bot_run'
      ) {
        await ensureAttemptVerificationCommands(attemptRow).catch(() => {});
      }
  ...
  const hasWindowProof = attemptHasWindowLoginProof(
    { mt5_login: row.mt5_login, window_title: row.attempt_window_title, snapshot_verified: row.snapshot_verified, observed_login: row.observed_login },
    row
  );
  const timeoutMs = hasWindowProof || row.snapshot_verified ? WINDOW_PROOF_TIMEOUT_MS : ATTEMPT_TIMEOUT_MS;
