...
async function getAttemptById(attemptId) {
  ...
  const r = await query(
    `
    SELECT at.*, a.current_attempt_id, a.mt5_password
...
async function ensureAttemptVerificationCommands(attempt) {
  ...
  if (attempt.metrics_ready && (attempt.login_verified || attempt.snapshot_verified)) {
    return (await queueLoginExitMt5(attempt)) || false;
  }
...
async function finalizeAttemptConnected(attempt, { message, evidenceSource, observedLogin, balance, equity, processId } = {}) {
  ...
  await queueLoginExitMt5(fresh).catch(() => {});
...
async function maybeFinalizeAttempt(attemptId) {
  ...
  if (
    ...
    && (attempt.port_health_verified || attempt.evidence_source === 'login_command_window')
  ) {
    await finalizeAttemptConnected(attempt, {
      message: attempt.last_message || MT5_SUCCESS_MSG,
      evidenceSource: 'window_title_port_health',
  ...
  if (
    ...
    await finalizeAttemptConnected(attempt, {
      message: attempt.last_message || 'ยืนยันการเชื่อมต่อจาก MT5 บน VPS แล้ว',
...
async function ingestCommandResultEvent(node, { commandId, commandType, payload, ok, result, error, message }) {
  ...
  if (['account_snapshot', 'sync_mt5_account', 'read_account_metrics'].includes(ctype)) {
    ...
    const windowLoginOk =
      isExactLoginMatch(mt5Login, observed)
      || (String(snap.source || result?.source || '').toLowerCase() === 'window_title'
        && observed);
    const snapshotVerified =
      windowLoginOk ||
      portProof.ok ||
      (portProof.ok && (balance !== null || equity !== null));

    await markAttemptProgress(attempt, {
      status: 'checking',
      message: snapshotVerified
        ? `Snapshot ยืนยันเลขบัญชี ${observedFromHealth || observed || mt5Login} แล้ว`
    ...
    if (snapshotVerified && (portProof.ok || balance !== null || equity !== null || windowLoginOk)) {
      return finalizeAttemptConnected(attempt, {
        message: portProof.ok
          ? 'ยืนยันการเชื่อมต่อจาก port health / MT5 แล้ว'
...
async function getConnectStatusForUser(userId, accountId = 0) {
  ...
  if (['connected', 'ready'].includes(accountStatus)) {
    status = 'connected';
  }
  const terminalConnectedProof = Boolean(
    ...
    || (
      row.attempt_terminal && (
        ...
        || (row.snapshot_verified && row.metrics_ready)
      )
    )
  );
