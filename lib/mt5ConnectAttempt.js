...
function getConnectStepMeta(row, status, metricsReady = false) {
  ...
  if (st === 'starting' && !hasWindow) {
    return {
      ...
      detail: message || 'กำลังเปิดหน้าจอ MT5 บน VPS...'
    };
  }
  ...
  if (st === 'starting' || (st === 'checking' && hasWindow && !hasJournal && !hasSnapshot)) {
    return {
      ...
      index: 3,
      total: 4,
      label: 'ขั้นตอน 3/4: เห็นหน้าต่าง MT5 แล้ว',
      detail: message || 'กำลังตรวจเลขบัญชีจากหน้าต่างและ Journal...'
    };
  }
  ...
  if (st === 'checking' && hasSnapshot) {
    return {
      ...
      index: 4,
      total: 4,
      label: 'ขั้นตอน 4/4: ยืนยันจาก MT5 API',
      detail: message || 'กำลังอ่าน Balance / Equity และยืนยันเลขบัญชี...'
    };
  }
  ...
  if (st === 'checking' && hasJournal) {
    return {
      ...
      index: 3,
      total: 4,
      label: 'ขั้นตอน 3/4: กำลังตรวจ Journal',
      detail: message || 'พบหน้าต่าง MT5 แล้ว กำลังรอ Journal ยืนยัน...'
    };
  }
  ...
  return {
    ...
    index: 3,
    total: 4,
    label: 'ขั้นตอน 3/4: กำลังยืนยันการเชื่อมต่อ',
    detail: message || 'กำลังตรวจสอบเลขบัญชี MT5...'
  };
}
...
function canRecoverFailedAttemptAsConnected(attempt, evidenceSource = '') {
  ...
  const trusted = ['journal', 'journal_command', 'snapshot', 'snapshot_port_health', 'connect_result'];
