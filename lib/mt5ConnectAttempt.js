...
async function tryFinalizeWindowVerifiedAttempt(attempt) {
  if (!attempt?.attempt_id) return false;
  if (!(await hasSuccessfulLoginCommand(attempt))) return false;
  if (!attemptHasWindowLoginProof(attempt)) return false;
