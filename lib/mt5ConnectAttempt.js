...
const LOGIN_EQUITY_FETCH_MIN_GAP_MS = Number(process.env.MT5_LOGIN_EQUITY_FETCH_MIN_GAP_MS || 12000);
...
function loginFromWindowTitle(title, expectedLogin = '') {
  const t = clean(title);
  if (!t) return '';
  const m = t.match(/^(\d{6,10})\s*[-:]/);
  const found = m ? m[1] : '';
  if (!found) return '';
  if (expectedLogin && isExactLoginMatch(expectedLogin, found)) return found;
  if (expectedLogin && t.includes(String(expectedLogin))) return exactObservedLogin(expectedLogin);
  return found;
}
...
async function ensureAttemptVerificationCommands(attempt) {
  ...
  if (attempt.folder_path && attempt.mt5_login) {
    const journalPaths = journalLogPathsForFolder(attempt.folder_path);
    const journalPath = journalPaths[0] || `logs\\${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.log`;
