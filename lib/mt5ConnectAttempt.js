...
async function canFinalizeAttemptWithMetrics(attempt, evidenceSource, balance, equity) {
  ...
  if (FINALIZE_EQUITY_SOURCES.has(source)) return true;
  if (source === 'connect_result' || source === 'connect_result_equity') {
    return hasSuccessfulLoginCommand(attempt);
  }
  if (attempt?.journal_verified || attempt?.snapshot_verified) return true;
