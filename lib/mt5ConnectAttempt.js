...
const COMMAND_DEDUP_MS = Number(process.env.MT5_CONNECT_ATTEMPT_COMMAND_DEDUP_MS || 25000);
const LOGIN_EQUITY_FETCH_MAX = Number(process.env.MT5_LOGIN_EQUITY_FETCH_MAX || 8);
const LOGIN_EQUITY_FORCE_CONNECT_AFTER = Number(process.env.MT5_LOGIN_EQUITY_FORCE_CONNECT_AFTER || 3);
const LOGIN_EQUITY_FETCH_MIN_GAP_MS = Number(process.env.MT5_LOGIN_EQUITY_FETCH_MIN_GAP_MS || 6000);
...
function getConnectStepMeta(row, status, metricsReady = false) {
  ...
  if (st === 'connected' && (metricsReady || loginVerified)) {
    return {
      key: 'connected',
      index: 4,
      total: 4,
      label: metricsReady ? 'ขั้นตอน 4/4: ยืนยันสำเร็จ' : 'ขั้นตอน 4/4: ยืนยัน Login แล้ว',
      detail: metricsReady
        ? 'ยืนยันเลขบัญชีและ Equity แล้ว — พร้อมใช้งาน'
        : 'ยืนยัน Login แล้ว — กำลังดึง Equity ต่อในพื้นหลัง'
    };
  }
...
async function queueLoginRetryAfterMismatch(attempt, wrongLogin = '') {
  ...
  await query(
    ...
    `,
    [Number(attempt.vps_id), attempt.port_id ? Number(attempt.port_id) : null, JSON.stringify(common)]
  ).catch(() => {});
  return true;
}
...
/** บัญชีค้าง checking ทั้งที่ attempt จบ connected แล้ว — sync ให้หน้าเว็บใช้งานได้ */
async function syncAccountFromTerminalConnectedAttempt(attempt, opts = {}) {
  ...
  const accRow = await query(
    `SELECT status, login_verified FROM vps_system.mt5_accounts WHERE id=$1 LIMIT 1`,
    [Number(attempt.account_id)]
  ).catch(() => ({ rows: [] }));
  const accStatus = normalizeAttemptStatus(accRow.rows?.[0]?.status);
  if (accStatus === 'connected' && accRow.rows?.[0]?.login_verified) return false;
  ...
  const verifiedOnly = opts.verifiedOnly === true || attemptLoginComplete(attempt);
  ...
  await promoteAccountConnected({
    ...
    requireMetrics: metrics.ready && !verifiedOnly
  }).catch(() => {});
  ...
  await updateAccountForAttempt(attempt, {
    ...
    loginVerified: true,
    metricsReady: metrics.ready || verifiedOnly,
...
async function repairIncompleteConnectAttempt(row) {
  ...
  if (metrics.ready && accountEquity !== null) return row;
  if (row.attempt_terminal && normalizeAttemptStatus(row.attempt_status || '') === 'connected') {
    ...
    if (attempt) {
      await syncAccountFromTerminalConnectedAttempt(attempt, {
        ...
        verifiedOnly: attemptLoginComplete(row, row)
      }).catch(() => {});
    }
    return row;
  }
...
async function finalizeAttemptConnected(attempt, {
  ...
  if (isAttemptTerminal(fresh.status) && !recoverTimedOutFailure) {
    const terminalComplete = attemptMetricsSatisfied(fresh, balance, equity);
    if (normalizeAttemptStatus(fresh.status) === 'connected') {
      await syncAccountFromTerminalConnectedAttempt(fresh, {
        message,
        balance,
        equity,
        verifiedOnly: !terminalComplete || forceWithoutMetrics
      }).catch(() => {});
      if (terminalComplete) {
    ...
    return { ok: true, status: 'connected', attemptId: fresh.attempt_id, attempt: fresh };
  }
  ...
  const verifiedOnly =
    forceWithoutMetrics
    && (fresh.journal_verified || fresh.port_health_verified || fresh.snapshot_verified);
  if (!metrics.ready && !verifiedOnly) {
  ...
  const metricsReady = metrics.ready || verifiedOnly;
...
async function maybeFinalizeAttempt(attemptId) {
  ...
  if (
    ...
    const ageSec = createdAtMs ? Math.floor((Date.now() - createdAtMs) / 1000) : 0;
    if (fetchCount >= LOGIN_EQUITY_FORCE_CONNECT_AFTER || ageSec >= 90) {
      await finalizeAttemptConnected(attempt, {
        message: 'ยืนยัน Login แล้ว — กำลังดึง Equity ต่อในพื้นหลัง',
        evidenceSource: 'verified_without_equity_poll',
        observedLogin: expectedLogin,
        forceWithoutMetrics: true
      }).catch(() => {});
      return getAttemptById(attemptId).catch(() => null);
    }
...
async function ingestCommandResultEvent(node, { commandId, commandType, payload, ok, result, error, message }) {
  ...
  if (ctype === 'login_mt5') {
    ...
    if (verdict === 'success' && (result?.loginVerified === true || result?.login_verified === true)) {
      const loginMetrics = resolveAttemptMetrics(attempt, result?.balance, result?.equity);
  ...
  if ((ctype === 'port_read_file' || ctype === 'read_file') && /verify_journal/i.test(clean(payload?.purpose))) {
    ...
    if (verdict === 'success') {
      const journalMetrics = resolveAttemptMetrics(attempt);
      if (!journalMetrics.ready) {
        await queueLoginEquityFetch(attempt).catch(() => {});
        return { ok: true, pending: true, attemptId: attempt.attempt_id };
      }
      return finalizeAttemptConnected(attempt, {
        ...
        evidenceSource: 'journal',
        observedLogin: mt5Login,
        balance: journalMetrics.balance,
        equity: journalMetrics.equity
      });
    }
    return { ok: true, pending: true, attemptId: attempt.attempt_id };
  }
  ...
  if (['account_snapshot', 'sync_mt5_account', 'read_account_metrics'].includes(ctype)) {
    ...
    if (loginMatch && !hasMetrics) {
      const queued = await queueLoginEquityFetch(attempt).catch(() => false);
      if (!queued) {
        const fetchCount = await countLoginEquityFetchCommands(attempt);
        if (fetchCount >= LOGIN_EQUITY_FORCE_CONNECT_AFTER) {
          if (attempt.journal_verified || attempt.port_health_verified || loginMatch) {
            const fin = await finalizeAttemptConnected(attempt, {
              message: 'ยืนยัน Login แล้ว — จะดึง Equity อีกครั้งหลังเชื่อมต่อ',
              evidenceSource: 'verified_without_equity',
              observedLogin: observedFromHealth || observed || mt5Login,
              forceWithoutMetrics: true
            });
            if (fin?.status === 'connected' || normalizeAttemptStatus(fin?.status) === 'connected') {
          ...
          return finalizeAttemptFailed(
            attempt,
            'ดึง Equity จาก MT5 ไม่สำเร็จหลายครั้ง (อาจมีหลายบัญชี login พร้อมกัน) กรุณาลองใหม่',
...
async function getConnectStatusForUser(userId, accountId = 0) {
  ...
  const loginComplete = attemptLoginComplete(row, row);
  const hasNumericMetrics = balance !== null || equity !== null;
  const metricsReady = hasNumericMetrics || loginComplete;

  let status = row.attempt_id ? attemptDisplayStatus(row.attempt_status || row.status) : normalizeAttemptStatus(row.status);
  const accountStatus = normalizeAttemptStatus(row.status);
  if (['connected', 'ready'].includes(accountStatus) && (metricsReady || loginComplete)) {
    status = 'connected';
  } else if (['connected', 'ready'].includes(accountStatus) && !metricsReady && !loginComplete) {
    status = 'checking';
  }
  const terminalConnectedProof = Boolean(
    row.attempt_terminal && normalizeAttemptStatus(row.attempt_status || '') === 'connected'
  );
  ...
  const connected = status === 'connected' && (metricsReady || loginComplete);
  ...
  const step = getConnectStepMeta(row, status, hasNumericMetrics || loginComplete);
  ...
  return {
    ...
    pending,
    status,
    loginVerified: connected || loginComplete,
    metricsReady,
    equityPending: connected && loginComplete && !hasNumericMetrics,
    message,
    windowTitle: title,
