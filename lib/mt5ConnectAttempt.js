...
const ATTEMPT_TIMEOUT_MS = Number(process.env.MT5_CONNECT_ATTEMPT_TIMEOUT_MS || 120000);
...
async function markAttemptProgress(attempt, {
  ...
  const nextStatus = normalizeAttemptStatus(status || attempt.status);
  const bal = positiveMoney(balance);
  const eq = positiveMoney(equity);
  const metricsReady = Boolean(bal || eq || attempt.metrics_ready);
  ...
  await updateAccountForAttempt(attempt, {
    ...
    lastMessage: message !== undefined ? clean(message) || null : undefined,
    windowTitle: windowTitle !== undefined ? clean(windowTitle) || null : undefined,
    vpsId: attempt.vps_id ? Number(attempt.vps_id) : undefined,
    portId: attempt.port_id ? Number(attempt.port_id) : undefined,
    assignedPortNo: attempt.assigned_port_no ? Number(attempt.assigned_port_no) : undefined,
    windowsPortNo: attempt.assigned_port_no ? Number(attempt.assigned_port_no) : undefined,
    portSlot: attempt.port_slot ? Number(attempt.port_slot) : undefined,
    metricsReady,
    balance: bal !== null ? bal : undefined,
    equity: eq !== null ? eq : undefined
  ...
  if (windowTitle || previewB64 || message) {
    await recordAttemptPreview(attempt, {
      status: nextStatus,
      message,
      windowTitle,
      previewB64
  ...
  return getAttemptById(attempt.attempt_id).catch(() => null);
}
...
async function ingestCommandResultEvent(node, { commandId, commandType, payload, ok, result, error, message }) {
  ...
  if (['account_snapshot', 'sync_mt5_account', 'read_account_metrics'].includes(ctype)) {
    ...
    if (snapshotVerified && portProof.ok) {
      return finalizeAttemptConnected(attempt, {
        message: 'ยืนยันการเชื่อมต่อจาก MT5 API และ port health แล้ว',
        evidenceSource: 'snapshot_port_health',
...
async function getConnectStatusForUser(userId, accountId = 0) {
  ...
  const status = row.attempt_id ? attemptDisplayStatus(row.attempt_status || row.status) : normalizeAttemptStatus(row.status);
