...
async function queuePostConnectExitCommands(attempt) {
  ...
  if (purpose === 'login_only') {
    const { releaseTemporaryLoginFolder } = require('./mt5PortCleanup');
    const { resolveSystemVpsId } = require('./adminVpsBridge');
    const { adminNodeId } = await resolveSystemVpsId(vpsId).catch(() => ({}));
    await releaseTemporaryLoginFolder({
      accountId: Number(attempt.account_id || 0),
      userId: attempt.user_id ? Number(attempt.user_id) : null,
      vpsId,
      portId: portId || null,
      assignedPortNo: portNo,
      adminNodeId: adminNodeId || null,
      folderPath: attempt.folder_path || folderPath,
      mt5Login: attempt.mt5_login,
      reason: 'login_temp_release'
    }).catch(() => {});
  }
...
async function syncAccountFromTerminalConnectedAttempt(attempt, opts = {}) {
  ...
  await promoteAccountConnected({
    ...
    mt5Login: clean(attempt.mt5_login),
    message: finalMsg,
    lockPortAfterLogin: clean(attempt.purpose_type || 'login_only').toLowerCase() !== 'login_only',
    userId: attempt.user_id ? Number(attempt.user_id) : null,
    balance: metrics.balance,
    equity: metrics.equity,
    requireMetrics: true
...
async function finalizeAttemptConnected(attempt, {
  ...
  if (attemptIsCurrent) {
    await promoteAccountConnected({
      ...
      message: finalMsg,
      lockPortAfterLogin: clean(fresh.purpose_type || 'login_only').toLowerCase() !== 'login_only',
