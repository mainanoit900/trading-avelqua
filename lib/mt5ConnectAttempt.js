...
function getConnectStepMeta(row, status, metricsReady = false) {
  ...
  if (st === 'connected' && metricsReady) {
    return {
      ...
      detail: 'ยืนยันเลขบัญชีและ Equity แล้ว — พร้อมใช้งาน'
    };
  }
...
async function repairIncompleteConnectAttempt(row) {
  ...
  const needsRepair =
    ...
    (attemptStatus === 'connected' && row.attempt_terminal) ||
    (accountEquity === null && ['connected', 'checking', 'connecting', 'starting'].includes(accountStatus));
...
async function finalizeAttemptConnected(attempt, {
  ...
  if (isAttemptTerminal(fresh.status) && !recoverTimedOutFailure) {
    const terminalMetrics = resolveAttemptMetrics(fresh, balance, equity);
    if (normalizeAttemptStatus(fresh.status) === 'connected' && !terminalMetrics.ready) {
      await reopenAttemptForMetrics(fresh).catch(() => {});
...
async function maybeFinalizeAttempt(attemptId) {
  ...
  if (isAttemptTerminal(attempt.status)) {
    if (normalizeAttemptStatus(attempt.status) === 'connected' && !terminalMetrics.ready) {
      await reopenAttemptForMetrics(attempt).catch(() => {});
...
async function getConnectStatusForUser(userId, accountId = 0) {
  ...
  const pending = ['starting', 'checking'].includes(status) || (['connected', 'ready'].includes(accountStatus) && !metricsReady);
