...
async function ingestConnectResultEvent(node, body) {
  ...
  if (titleMismatch) {
    return finalizeAttemptFailed(attempt, `บัญชีบน MT5 ไม่ตรง (เห็น ${titleMismatch} แต่กรอก ${mt5Login})`, {
  ...
  if (mt5Login && observedLogin && !isExactLoginMatch(mt5Login, observedLogin)) {
    return finalizeAttemptFailed(attempt, `บัญชีบน MT5 ไม่ตรง (เห็น ${observedLogin} แต่กรอก ${mt5Login})`, {
      evidenceSource: 'observed_login_mismatch',
      windowTitle: windowTitle || undefined
    });
  }
