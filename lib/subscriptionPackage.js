...
function formatSubscriptionDisplayLabel(sub) {
  ...
  const paymentRaw = parsePaymentRawPayload(sub);
  const rawCoupon = paymentRaw.coupon || {};
  ...
  const rawCouponDays = num(rawCoupon.free_days, 0);
  ...
  if (isFreeCoupon || couponDays > 0 || rawCouponDays > 0) {
    // Prefer actual entitlement duration before mutable coupon metadata.
    const days = durationDays || rawCouponDays || couponDays || snapDays;
    const group = tier || sub.coupon_free_package_group || rawCoupon.free_package_group;
...
function parsePaymentRawPayload(payment) {
  const payload = payment?.raw_payload ?? payment?.payment_raw_payload;
  if (!payload) return {};
...
async function applyPaidPackageSubscription({ client, userId, packageRow, sourceChannel = null }) {
  ...
  const lotMin = num(pkg.lot_min, 0);
  const lotMax = num(pkg.lot_max, 0);
  const portsMin = num(pkg.ports_min, 0);
  const portsMax = num(pkg.ports_max, 0);
  const profitMin = pkg.profit_min === null || pkg.profit_min === undefined ? null : num(pkg.profit_min, 0);
  const profitMax = pkg.profit_max === null || pkg.profit_max === undefined ? null : num(pkg.profit_max, 0);
  const profitLabel = String(pkg.profit_label_th || pkg.profit_label_en || pkg.profit_label || '').trim();
