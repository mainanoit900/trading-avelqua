...
async function isUserPortSlotFreeForLogin(userId, slot, totalPorts, mt5Login = '') {
  ...
  const { autoClearPortSlotBeforeLogin } = require('./mt5LoginCommandVerify');
  await autoClearPortSlotBeforeLogin(userId, s, mt5Login).catch(() => ({}));
  const occAfter = await getUserPortSlotOccupant(userId, s);
  if (!occAfter) return { ok: true, autoCleared: true };
  if (
    String(occAfter.mt5_login || '').trim() === String(mt5Login || '').trim() &&
    String(occAfter.status || '').toLowerCase() === 'connected'
  ) {
    return { ok: true, alreadyConnected: true, accountId: occAfter.id, autoCleared: true };
  }

  return {
    ok: false,
    code: 'SLOT_BUSY',
    message: `PORT ${s} ใช้โดย Login ${occAfter.mt5_login} อยู่แล้ว — ลองกดเชื่อมต่ออีกครั้งหรือเลือก PORT อื่น`,
    occupant: occAfter
  };
}
