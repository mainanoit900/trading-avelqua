...
const { FOLDER_POOL_MIN, FOLDER_POOL_MAX } = require('./mt5ReservedPortNo');

/** บัญชีที่ผูก FolderPort ใน pool ร่วม (101–120) — package port_slot แยกจากเลข folder */
...
const STALE_INFLIGHT_NO_BINDING_MINUTES = 3;

/** ยกเลิก checking/connecting ที่ไม่ได้รับ FolderPort จริง (ค้างบล็อก PORT) */
async function releaseStaleInflightAccountsWithoutBinding(maxAgeMinutes = STALE_INFLIGHT_NO_BINDING_MINUTES) {
  const mins = Math.max(1, num(maxAgeMinutes, STALE_INFLIGHT_NO_BINDING_MINUTES));
  const r = await query(
    `
    UPDATE vps_system.mt5_accounts
    SET status = 'failed',
        last_login_message = 'หมดเวลา — ไม่ได้รับ FolderPort',
        updated_at = NOW()
    WHERE LOWER(TRIM(COALESCE(status, ''))) IN ('connecting', 'checking', 'starting')
      AND vps_id IS NULL
      AND assigned_port_no IS NULL
...
/** ล้างเฉพาะบัญชี in-flight ค้างของ user อื่น — ไม่ปลดล็อก FolderPort ที่ user อื่นล็อกไว้แล้ว */
...
async function findFolderPortUsedByOtherUser(systemVpsId, portNo, userId) {
  ...
  const r = await query(
    ...
    LIMIT 1
  `,
    [vpsId, uid, variants, FOLDER_IN_FLIGHT_STATUSES, FOLDER_IDLE_BIND_STATUSES]
  ).catch(() => ({ rows: [] }));

  if (r.rows?.[0]) return r.rows[0];

  const portLock = await query(
    `
    SELECT locked_by_user_id AS user_id, mt5_login, port_no AS assigned_port_no,
           'port_lock' AS conflict_type, status
    ...
    WHERE vps_id = $1
      ...
      AND (
        LOWER(COALESCE(status, '')) IN ('locked', 'running', 'busy')
  ...
  if (portLock.rows?.[0]) return portLock.rows[0];
...
async function assertFolderPortFreeForUser(vpsId, portNo, userId) {
  ...
  const kind =
    conflict.conflict_type === 'bot'
      ? 'บอทกำลังรัน'
      : conflict.conflict_type === 'port_lock'
        ? 'ล็อก FolderPort แล้ว'
        : 'MT5 ใช้งานอยู่';
  const slotHint = num(conflict.port_slot);
  const altHint =
    slotHint > 0
      ? ` — ลองเชื่อมต่อพร้อมกันผ่าน PORT อื่น (เช่น PORT ${slotHint === 1 ? 2 : 1}) ที่คนละ FolderPort`
      : '';
  return {
    ok: false,
    code: 'FOLDER_PORT_BUSY',
    message: `FolderPort ${portLabel} ถูก User อื่นใช้อยู่ (${kind} Login ${login})${altHint}`,
    conflict
  };
}
...
async function clearVpsPortPromotionBlockers(vpsId, portNo, keepAccountId = null) {
  const purged = await purgeMisboundFolderOccupants(vpsId, portNo, keepAccountId).catch(() => 0);
...
async function purgeMisboundFolderOccupants(vpsId, portNo, keepAccountId = null) {
  const vid = num(vpsId);
  const variants = portNoVariants(portNo);
  if (!vid || !variants.length) return 0;

  const params = [vid, variants];
  let sql = `
    UPDATE vps_system.mt5_accounts
  ...
  `;
  const keepId = num(keepAccountId);
  if (keepId > 0) {
    params.push(keepId);
    sql += ` AND id <> $${params.length}`;
  }
