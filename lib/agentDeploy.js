...
const REQUIRED_AGENT_VERSION =
  process.env.AVELQUA_REQUIRED_AGENT_VERSION || '2026-05-29-equity-before-close-v118'
...
async function queueAgentDeploy(nodeId) {
  ...
  const restartCmdId = Number(restartIns.rows?.[0]?.id || 0);
  if (restartCmdId) {
    notifyVpsAgentCommandQueued({
      vpsId,
      commandId: restartCmdId,
