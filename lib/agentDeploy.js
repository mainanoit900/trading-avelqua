...
const REQUIRED_AGENT_VERSION =
  process.env.AVELQUA_REQUIRED_AGENT_VERSION || '2026-06-10-enable-algo-window-wait-v144'
...
function loadAgentSupportFiles() {
  const out = [];
  const mql5Dir = path.join(process.cwd(), 'public', 'agent', 'mql5');
  for (const name of ['BOTClose.mq5', 'AvelquaCloseAll.mq5', 'AvelquaBotClose.mqh']) {
    const fp = path.join(mql5Dir, name);
...
function buildDeployPayload(opts = {}) {
  const payload = {
    ...
    serverUrl: getAgentScriptUrl().replace(/\/agent-script$/, '')
  };
  const agentToken = String(opts.agentToken || '').trim();
  if (agentToken) {
  ...
  if (opts.inlineContent) {
    ...
    payload.supportFiles = loadAgentSupportFiles();
  } else {
...
async function queueAgentDeploy(nodeId) {
  ...
  const tokenRow = await query(
    `SELECT agent_token FROM vps_system.vps_nodes WHERE id=$1 LIMIT 1`,
    [vpsId]
  ).catch(() => ({ rows: [] }));

  const deployIns = await query(
    `
  ...
  `,
    [
      ...
      JSON.stringify(
        buildDeployPayload({
          inlineContent: needsInline,
          agentToken: tokenRow.rows?.[0]?.agent_token || ''
        })
      )
    ]
  );
  const deployCmdId = Number(deployIns.rows?.[0]?.id || 0);
  if (deployCmdId) {
    notifyVpsAgentCommandQueued({
      vpsId,
      commandId: deployCmdId,
      commandType: 'deploy_agent'
    }).catch(() => {});
  }
...
async function deployAgentToAllVpsNodes(opts = {}) {
  const force = opts.force === true || process.env.AGENT_DEPLOY_FORCE === '1';
  ...
  for (const row of nodes.rows || []) {
    const vpsId = Number(row.id || 0);
    if (!vpsId) continue;
    if (force) {
      const q = await queueAgentDeploy(vpsId);
      results.push({
        vpsId,
        node: row.node_code || vpsId,
        queued: !!q.queued,
        reason: q.reason || null
      });
      continue;
    }
    const state = await ensureAgentUpToDate(vpsId).catch(() => ({ action: 'skip' }));
    if (state.action === 'queued' || state.action === 'wait') {
      results.push({ vpsId, node: row.node_code || vpsId, queued: true, reason: state.reason || state.action });
    } else {
      results.push({ vpsId, node: row.node_code || vpsId, queued: false, reason: state.reason || state.action || 'SKIP' });
    }
  }
