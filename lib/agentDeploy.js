...
const REQUIRED_AGENT_VERSION =
  process.env.AVELQUA_REQUIRED_AGENT_VERSION || '2026-05-18-mt5-algo-live-v21';
...
async function queueAgentRestart(vpsId) {
...
async function ensureAgentMaintenance(vpsId, opts = {}) {
  ...
  if (state === 'stuck') {
    ...
    return {
      state: 'stuck',
      notice: messageForUpgradeState('stuck'),
  ...
  if (state === 'needs_restart') {
    const recent = await countRecentMaintenanceSuccess(nid, 45);
    if (recent.restarts >= 3) {
      return {
        state: 'stuck',
        notice: messageForUpgradeState('stuck'),
        maintenancePending: false,
    ...
    const q = await queueAgentDeploy(nid, { force: true }).catch(() => ({ queued: false }));
    if (!q.queued) await queueAgentRestart(nid).catch(() => {});
    return {
      state: 'deploying',
      notice: null,
      maintenancePending: true,
  ...
  if (state === 'legacy') {
    const recent = await countRecentMaintenanceSuccess(nid, AGENT_DEPLOY_MIN_INTERVAL_MINUTES || 45);
    if (recent.deploys >= 1 && !opts.force) {
      return {
        state: 'legacy',
        notice: messageForUpgradeState('stuck'),
        maintenancePending: false,
...
async function resolveLiveDashboardAgentNotice(instances) {
  ...
  for (const vpsId of vpsIds) {
    ...
    if (maint.state === 'stuck' && maint.notice) {
      return { notice: maint.notice, queueDeploy: false, maintenancePending: false };
    }
    ...
    WITH ranked AS (
      ...
      WHERE (vps_id = $1 OR node_id = $1)
        ...
        AND command_type IN (
          ...
          'sync_mt5_account', 'read_account_metrics'
        )
    )
...
function messageForUpgradeState(state) {
  ...
  if (state === 'stuck') {
    return (
      'อัปเดต Agent อัตโนมัติไม่สำเร็จ (Service ยังรันโค้ดเก่า) — บน VPS เปิด PowerShell แอดมินแล้วรัน: ' +
      'Restart-Service AvelquaPythonAgent -Force · ตรวจไฟล์ C:\\avelqua-python-agent\\agent.py ว่ามีบรรทัด 2026-05-18-mt5-algo-live-v21'
    );
  }
