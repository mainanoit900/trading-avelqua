...
async function resolveLiveDashboardAgentNotice(instances) {
  ...
  for (const vpsId of vpsIds) {
    ...
    if (hasRunBotMarker(ver)) continue;

    const state = await getAgentUpgradeState(vpsId);
    ...
    if (state === 'deploying') {
      notice =
        'Agent บน VPS กำลังอัปเดตเป็น v21 (รอ 2–3 นาที) — หลังเสร็จค่า Balance/Equity จะขยับทุก ~4 วินาที';
      continue;
    }
    if (state === 'needs_restart') {
      notice =
        'อัปเดต Agent แล้ว — รอ service บน VPS รีสตาร์ท (1–2 นาที) แล้วค่า Equity จะอัปเดตอัตโนมัติ';
      queueDeploy = true;
      continue;
    }
    if (state === 'legacy' || cmdErr) {
      notice =
        'Agent บน VPS กำลังอัปเดตเป็น v21 (รอ 2–3 นาที) — หลังเสร็จค่า Balance/Equity จะขยับทุก ~4 วินาที';
      queueDeploy = true;
    }
