...
const REQUIRED_AGENT_VERSION =
  process.env.AVELQUA_REQUIRED_AGENT_VERSION || '2026-05-19-equity-dashboard-v33';
...
function hasJournalGateMarker(versionOrBuild) {
  ...
  return (
    ...
    v.includes('agent-perf') ||
    v.includes('connect-fix')
  );
}
...
/** ปล่อยคำสั่ง bot/stop/login ที่ค้าง processing — กัน Agent ไม่ poll คำสั่งใหม่ */
async function expireStuckProcessingCommands(nodeId, maxSec = 90) {
  const vpsId = Number(nodeId || 0);
  if (!vpsId) return { failed: 0 };
  const r = await query(
    `
    UPDATE vps_system.vps_agent_commands
    SET status = 'failed',
        error = COALESCE(error, 'auto-failed: processing timeout'),
        result_message = COALESCE(result_message, 'auto-failed: processing timeout'),
        finished_at = NOW(),
        updated_at = NOW()
    WHERE (vps_id = $1 OR node_id = $1)
      AND command_type IN (
        'stop_mt5_bot', 'run_mt5_bot', 'run_mt5', 'run_bot', 'force_stop_mt5',
        'kill_mt5', 'stop_port', 'login_mt5', 'connect_mt5'
      )
