...
async function tryFastConnectConfirm(account) {
  ...
  if (metricsVerify.ok) {
    const sinceMsMet = await journalSinceMsForVerify(accountId, vpsId, portNo);
    const stashedMet = await getStashedJournalEvidence(accountId);
    if (
      stashedMet &&
      parseMt5JournalOutcome(stashedMet, login, undefined, sinceMsMet) === 'success'
    ) {
