...
async function syncJournalFromLatestCommand(accountId, vpsId, login, folderPath, portNo) {
  ...
  const sinceMs = await accountConnectSinceMs(accountId);
  const verdict = content && login ? parseMt5JournalOutcome(content, login, undefined, sinceMs) : null;
...
async function tryApplyPendingJournalRead(accountId, vpsId) {
  ...
  const sinceRow = await query(
    `SELECT COALESCE(connect_started_at, updated_at) AS since FROM vps_system.mt5_accounts WHERE id=$1 LIMIT 1`,
    [accountId]
  ).catch(() => ({ rows: [] }));
  const sinceTs = sinceRow.rows?.[0]?.since || null;
  ...
  const sql = nodeId
    ...
    WHERE (vps_id=$1 OR node_id=$1)
      ...
      AND ($3::timestamptz IS NULL OR finished_at >= $3::timestamptz - INTERVAL '3 seconds')
    ORDER BY id DESC
    LIMIT 1
  `
    : `
    ...
    WHERE command_type IN ('read_file', 'port_read_file')
      ...
      AND LOWER(COALESCE(status, '')) IN ('success', 'done')
      AND finished_at > NOW() - INTERVAL '15 minutes'
      AND ($2::timestamptz IS NULL OR finished_at >= $2::timestamptz - INTERVAL '3 seconds')
    ORDER BY id DESC
    LIMIT 1
  `;
  const params = nodeId ? [nodeId, String(accountId), sinceTs] : [String(accountId), sinceTs];
...
async function resolveStuckLoginAccount(account) {
  ...
  if (elapsedSec < 3) return { resolved: false };

  if (folderPathEarly && elapsedSec >= 3) {
    await queueJournalReadVerify({
      accountId,
      vpsId,
