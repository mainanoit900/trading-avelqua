...
const { hasOtherLoginWorkOnVps } = require('./mt5MultiPortLogin');
const { positiveMoney, fetchPostLoginEquityOnce, queueSyncMt5Account } = require('./mt5EquitySync');
const { insertPendingAgentCommand } = require('./vpsAgentCommandQueue');

/** Login ขึ้นต้น 8/2 — ปิดค่าเริ่มต้น (reference May-16 ใช้ Journal ทุก login) */
function loginUsesEquityVerify(login) {
  if (String(process.env.MT5_EQUITY_LOGIN_VERIFY || '0') !== '1') return false;
  const s = String(login || '').trim();
  return /^\d/.test(s) && (s.startsWith('8') || s.startsWith('2'));
}
const {
  resolveSystemVpsId,
...
async function findRecentLoginCommand(accountId, vpsId, opts = {}) {
  if (!accountId || !vpsId) return null;
  const sinceMs = Number(opts.sinceMs || opts.since || 0);
  const params = [vpsId, String(accountId)];
  let sinceSql = '';
  if (sinceMs > 0) {
    sinceSql = ` AND created_at >= to_timestamp($${params.length + 1} / 1000.0)`;
    params.push(sinceMs);
  }
  const r = await query(
    `
    SELECT id, status, result, finished_at, created_at, payload, error
    FROM vps_system.vps_agent_commands
    WHERE (vps_id=$1 OR node_id=$1)
...
function isSupersededLoginCancel(cmd) {
  const st = String(cmd?.status || '').toLowerCase();
  const err = String(cmd?.error || '').toLowerCase();
  return st === 'cancelled' && /new login attempt|superseded/i.test(err);
}

/** คำสั่ง login ล่าสุดที่มีผล (ข้าม cancelled จาก login รอบใหม่) */
async function findRecentTerminalLoginCommand(accountId, vpsId, opts = {}) {
  if (!accountId || !vpsId) return null;
  const sinceMs = Number(opts.sinceMs || opts.since || 0) || (await accountConnectSinceMs(accountId));
  const params = [vpsId, String(accountId)];
  let sinceSql = '';
  if (sinceMs > 0) {
    sinceSql = ` AND created_at >= to_timestamp($${params.length + 1} / 1000.0)`;
  ...
  const r = await query(
    `
    SELECT id, status, result, finished_at, created_at, started_at, payload, error, command_type, result_message
    FROM vps_system.vps_agent_commands
    WHERE (vps_id=$1 OR node_id=$1)
      AND command_type IN ('login_mt5', 'connect_mt5')
      AND (payload->>'accountId')::text = $2::text
  ...
  for (const row of r.rows || []) {
    if (isSupersededLoginCancel(row)) continue;
    const st = String(row.status || '').toLowerCase();
    const ctype = String(row.command_type || '').toLowerCase();
    if (
      st === 'cancelled' &&
      ['login_mt5', 'connect_mt5'].includes(ctype) &&
      !String(row.error || row.result_message || '').trim()
    ) {
      row.error = 'cancelled: login superseded';
    }
    return row;
  }
  return null;
...
/** คำสั่ง login ของรอบเชื่อมต่อปัจจุบัน (หลัง connect_started_at) */
async function findLoginCommandForAttempt(accountId, vpsId) {
  const sinceMs = await accountConnectSinceMs(accountId);
  if (!sinceMs) {
    return findRecentLoginCommand(accountId, vpsId);
  }
  return findRecentLoginCommand(accountId, vpsId, { sinceMs });
}
...
function parseJournalRelaxed(content, login, sinceMs) {
  ...
  if (since > 0) {
    const strict = parseMt5JournalOutcome(content, login, undefined, since);
    if (strict === 'failed') return 'failed';
    if (strict === 'success') return 'success';
...
/** running=true แต่ไม่มี PID = stale หลัง force_stop — ห้ามใช้ยืนยัน login */
function portHealthRowHasLiveMt5(row) {
  if (!row || row.running !== true) return false;
  const procId = row.process_id;
  if (procId != null) {
    const s = String(procId).trim();
    if (s && s !== '0' && s !== 'null') return true;
  }
  const pidArr = row.pid;
  if (Array.isArray(pidArr) && pidArr.length > 0) return true;
  if (typeof pidArr === 'string' && pidArr.trim() && pidArr.trim() !== '[]') return true;
  return false;
}

async function isPortMt5Running(vpsId, portNo, opts = {}) {
  if (!vpsId || !portNo) return false;
  const strict = opts.strict !== false;
  const r = await query(
    `
    SELECT running, process_id, pid
  ...
  const row = r.rows?.[0];
  if (!row) return false;
  if (strict) return portHealthRowHasLiveMt5(row);
...
/** ลอง port 2 และ 102 (package slot → system port) */
async function verifyPortLoginWithFallback(vpsId, portNo, mt5Login, opts = {}) {
  const ports = systemPortNosForFail(portNo);
  if (!ports.length && portNo) ports.push(Number(portNo));
  for (const p of ports) {
    const metrics = await verifyPortMetricsLogin(vpsId, p, mt5Login, opts).catch(() => ({
      ok: false
    }));
    if (metrics.ok) return metrics;
    const run = await verifyPortRunningLogin(vpsId, p, mt5Login).catch(() => ({ ok: false }));
    if (run.ok) return run;
  }
  return { ok: false, reason: 'NO_PORT_MATCH' };
}

/** ยกเลิก stop_mt5 ค้างก่อน login — กัน login_mt5 รอคิวไม่ถูก Agent รับ */
async function cancelPendingStopsBeforeLogin(vpsId, portSlot, opts = {}) {
  const nid = Number(vpsId || 0);
  const slot = Number(portSlot || 0);
  if (!nid) return { cancelled: 0 };
  const ports = systemPortNosForFail(slot);
  const portList = ports.length ? ports.map(String) : [String(slot), String(100 + slot)];
  const r = await query(
    `
    UPDATE vps_system.vps_agent_commands
    SET status='cancelled',
        error=COALESCE(error, 'cancelled: login takes priority over stop'),
        finished_at=COALESCE(finished_at, NOW()),
        updated_at=NOW()
    WHERE (vps_id=$1 OR node_id=$1)
      AND command_type IN ('stop_mt5', 'kill_mt5', 'force_stop_mt5')
      AND LOWER(COALESCE(status, '')) = 'pending'
      AND (
        (payload->>'portSlot')::text = ANY($2::text[])
        OR (payload->>'port')::text = ANY($2::text[])
        OR (payload->>'port_no')::text = ANY($2::text[])
        OR (payload->>'portNumber')::text = ANY($2::text[])
      )
    RETURNING id
  `,
...
/** ยืนยัน login — journal หรือ metrics/port health จาก Agent */
async function isAccountLoginJournalVerified(account) {
  ...
  if (String(account.status || '').toLowerCase() === 'connected') {
    return { ok: true, source: 'db_connected' };
  }
  const cmd = await verifyLoginFromCommand({
    accountId,
...
async function probeRecentLoginCommandFailed(account) {
  ...
  const cmd = await findRecentTerminalLoginCommand(accountId, vpsId);
  if (!cmd) return { failed: false };
  const st = String(cmd.status || '').toLowerCase();
  if (!['failed', 'error', 'cancelled'].includes(st)) return { failed: false };
  const cmdErr = String(cmd.error || '').trim();
  const cmdResultMsg = String(cmd.result_message || '').trim();
  const cancelMsg = `${cmdErr} ${cmdResultMsg}`.trim();
  const login = String(account.mt5_login || cmd.payload?.mt5Login || '').trim();
  if (st === 'cancelled') {
    if (/new login attempt|superseded|login takes priority/i.test(cancelMsg)) {
      return { failed: false, transient: true, cmd };
    }
    if (!cancelMsg) {
      return { failed: false, transient: true, cmd };
    }
  }
  if (loginUsesEquityVerify(login) && /MT5_EQUITY|EQUITY_FAILED/i.test(cmdErr)) {
    return {
      failed: true,
  ...
  if (
    st === 'cancelled' &&
    /journal\s+login\s+failed|journal\s+read\s+timeout|cancelled:\s*journal/i.test(cmdErr)
  ) {
    return {
      failed: true,
      message: MT5_LOGIN_TIMEOUT_MSG,
      authFail: false,
      journalVerdict: 'timeout',
      cmd
    };
  }
  const sinceMs = await accountConnectSinceMs(accountId);
  ...
  const timeoutOnly =
    resolved.journalVerdict === 'timeout' &&
    resolved.authFail !== true &&
    /ทันเวลา|timeout|ไม่สามารถยืนยัน/i.test(String(resolved.message || ''));
  const authFail =
    resolved.authFail === true ||
    resolved.journalVerdict === 'failed' ||
    (st === 'failed' && !timeoutOnly);
  return {
    failed: true,
    message: authFail ? MT5_FAIL_USER_MSG : resolved.message,
    authFail,
    journalVerdict: authFail ? 'failed' : resolved.journalVerdict,
    cmd
  };
}
...
function loginCommandNeedsEquityResult(cmd, login) {
  if (!loginUsesEquityVerify(login) || !cmd) return false;
  const st = String(cmd.status || '').toLowerCase();
  if (!['success', 'done'].includes(st)) return false;
  const res = cmd.result && typeof cmd.result === 'object' ? cmd.result : {};
  if (positiveMoney(res.equity) > 0) return false;
  if (res.equityVerified === true || res.equity_verified === true) {
    if (positiveMoney(res.equity) > 0) return false;
    return true;
  }
  if (res.status === 'connected' && (res.loginVerified === true || res.login_verified === true)) {
    return false;
  }
  return !res.status && !res.message && !res.journalEvidence;
}

function loginCommandLooksConnected(cmd) {
  if (!cmd) return false;
  const st = String(cmd.status || '').toLowerCase();
  if (!['success', 'done'].includes(st)) return false;
  const res = cmd.result && typeof cmd.result === 'object' ? cmd.result : {};
  if (String(res.status || '').toLowerCase() === 'connected') return true;
  if (res.loginOnly === true || res.keepMt5Open === true) return true;
  if (res.loginVerified === true || res.login_verified === true) return true;
  if (res.windowVerified === true || res.window_verified === true) return true;
  const msg = String(res.message || cmd.result_message || '').toLowerCase();
  return /connected|เชื่อมต่อ/.test(msg);
}

async function cancelPendingLoginCommands(accountId, vpsId) {
  if (!accountId || !vpsId) return;
  await query(
    `
    UPDATE vps_system.vps_agent_commands
    SET status='cancelled',
        error=COALESCE(error, 'cancelled: duplicate login'),
        finished_at=COALESCE(finished_at, NOW()),
        updated_at=NOW()
    WHERE (vps_id=$1 OR node_id=$1)
      AND command_type IN ('login_mt5', 'connect_mt5')
...
async function finalizeLoginCommandSuccess(accountId, vpsId, resultPatch = {}) {
  if (!accountId || !vpsId) return;
  const login = String(resultPatch.login || '').trim();
  const eq = positiveMoney(resultPatch.equity);
  if (login && loginUsesEquityVerify(login) && eq <= 0) {
    await cancelPendingLoginCommands(accountId, vpsId);
    return;
  }
  const patch = {
    status: 'connected',
  ...
  await query(
    ...
    SET status='success',
        ...
        result_message=COALESCE(NULLIF($4, ''), result_message, 'เชื่อมต่อสำเร็จ'),
        finished_at=COALESCE(finished_at, NOW()),
        error=NULL,
        updated_at=NOW()
    WHERE (vps_id=$1 OR node_id=$1)
      ...
      AND LOWER(COALESCE(status, '')) = 'pending'
  `,
    [vpsId, String(accountId), JSON.stringify(patch), String(patch.message || MT5_SUCCESS_MSG).slice(0, 2000)]
  ).catch(() => {});
  await cancelPendingLoginCommands(accountId, vpsId);
}

/** @deprecated ใช้ finalizeLoginCommandSuccess เมื่อมีผลจริง — อย่า mark success โดยไม่มี equity/result */
async function finishPendingLoginCommands(accountId, vpsId, opts = {}) {
  if (opts?.result && typeof opts.result === 'object') {
    return finalizeLoginCommandSuccess(accountId, vpsId, opts.result);
  }
  return cancelPendingLoginCommands(accountId, vpsId);
}

async function isPortHealthFresh(vpsId, portNo, sinceMs = 0) {
  if (!vpsId || !portNo || sinceMs <= 0) return true;
  const r = await query(
    `
    SELECT updated_at
    FROM vps_system.vps_port_health
  ...
  const upd = r.rows?.[0]?.updated_at ? new Date(r.rows[0].updated_at).getTime() : 0;
  return upd >= sinceMs - 5000;
}

async function queueEquityLoginVerify({ accountId, vpsId, folderPath, mt5Login, portNo }) {
  ...
  const recent = await query(
    ...
    FROM vps_system.vps_agent_commands
    WHERE (vps_id=$1 OR node_id=$1)
      AND command_type IN ('sync_mt5_account', 'account_snapshot', 'dashboard')
      AND (payload->>'accountId')::text = $2::text
      AND COALESCE(payload->>'purpose', '') = 'equity_login_verify'
      AND LOWER(COALESCE(status, '')) IN ('pending', 'processing', 'picked', 'running')
...
async function findJournalFailFast(accountId, vpsId, login, portNo) {
  ...
  if (loginUsesEquityVerify(loginStr)) return null;
  const sinceMs = await accountConnectSinceMs(accountId);
  ...
  const cmd = await findLoginCommandForAttempt(accountId, vpsId);
  if (cmd) {
    const cmdSt = String(cmd.status || '').toLowerCase();
    if (cmdSt === 'success') return null;
    const res = cmd.result && typeof cmd.result === 'object' ? cmd.result : {};
    const blob = [
      res.journalEvidence,
      res.journal_evidence,
    ...
    if (cmdSt === 'failed') {
      const errMsg = String(cmd.error || res.message || cmd.result_message || '').trim();
      if (/MT5_JOURNAL_TIMEOUT|JOURNAL_TIMEOUT|ทันเวลา/i.test(errMsg)) {
        return { source: 'login_cmd_timeout', evidence: errMsg, timeout: true };
      }
      if (
  ...
  if (portNo && vpsId) {
    ...
    const portCmds = await query(
      `
      SELECT result, payload, error, result_message, command_type, status
      FROM vps_system.vps_agent_commands
      WHERE (vps_id=$1 OR node_id=$1)
        ...
        AND (
          command_type NOT IN ('login_mt5', 'connect_mt5')
          OR (payload->>'accountId')::text = $3::text
          OR TRIM(COALESCE(payload->>'mt5Login', payload->>'login', '')) = $4
        )
      ORDER BY id DESC
      LIMIT 6
    `,
      [vpsId, ports, String(accountId), loginStr]
    ).catch(() => ({ rows: [] }));
    for (const row of portCmds.rows || []) {
      const rowSt = String(row.status || '').toLowerCase();
      if (rowSt === 'success') continue;
      const blob = [
...
async function shouldDeferLoginJournalFail(account) {
  ...
  if (loginUsesEquityVerify(login)) {
    if (await hasLoginCommandInProgress(accountId, vpsId).catch(() => false)) return true;
    const st = String(account?.status || '').toLowerCase();
    if (['connecting', 'starting', 'checking'].includes(st)) {
      const cmd = await findLoginCommandInProgress(accountId, vpsId).catch(() => null);
      if (cmd) return true;
    }
  }
  ...
  const elapsedSec = startedAt
    ...
    : 999;
  const deferSec = Number(process.env.MT5_LOGIN_FAIL_DEFER_SEC || 5);
...
async function tryFastJournalFail(account) {
  ...
  const login = String(account?.mt5_login || '').trim();
  const acctSt = String(account?.status || '').toLowerCase();
  if (acctSt === 'connected') return { resolved: false };
  ...
  const failMsg = fail.timeout ? MT5_LOGIN_TIMEOUT_MSG : MT5_FAIL_USER_MSG;
...
/**
 * ถ้า login ค้างเกินกำหนด หรือคำสั่ง VPS ล้มเหลวแล้ว — ปิดสถานะ checking ทันที (ไม่รอ UI timeout)
 */
async function expireStuckLoginVerify(account) {
  const accountId = Number(account?.id || 0);
  if (!accountId) return { expired: false };

  const st = String(account?.status || '').toLowerCase();
  if (!['connecting', 'starting', 'checking'].includes(st)) return { expired: false };

  const cmdFail = await probeRecentLoginCommandFailed(account).catch(() => ({ failed: false }));
  if (cmdFail.failed) {
    const failMsg = cmdFail.message || MT5_LOGIN_TIMEOUT_MSG;
    await failAccountFromJournal(accountId, Number(account.port_id || 0), failMsg, {
      vpsId: account.vps_id,
      portNo: account.assigned_port_no || account.port_slot,
      folderPath: account.folder_path,
    ...
    return { expired: true, message: failMsg };
  }

  const startedMs = account.connect_started_at
    ? new Date(account.connect_started_at).getTime()
    : account.created_at
      ? new Date(account.created_at).getTime()
      : 0;
  const elapsedSec = startedMs ? Math.max(0, Math.floor((Date.now() - startedMs) / 1000)) : 0;
  const stuckSec = Number(process.env.MT5_LOGIN_STUCK_FAIL_SEC || 45);
  const updatedMs = account.updated_at ? new Date(account.updated_at).getTime() : startedMs;
  const staleSec = updatedMs ? Math.max(0, Math.floor((Date.now() - updatedMs) / 1000)) : elapsedSec;

  if (elapsedSec < stuckSec) return { expired: false };

  const failFast = await tryFastJournalFail(account).catch(() => ({ resolved: false }));
  if (failFast.resolved) {
    return { expired: true, message: failFast.message || MT5_FAIL_USER_MSG };
  }

  const cmdBusy = await hasLoginCommandInProgress(accountId, account.vps_id).catch(() => false);
  if (cmdBusy && staleSec < 35) return { expired: false };

  if (!cmdBusy || staleSec >= 35) {
    const failMsg =
      staleSec >= 35 && cmdBusy
        ? 'MT5 Login: ตรวจสอบ Journal ขัดข้อง — ลองใหม่หรือตรวจรหัสผ่าน'
        : MT5_LOGIN_TIMEOUT_MSG;
    await failAccountFromJournal(accountId, Number(account.port_id || 0), failMsg, {
      vpsId: account.vps_id,
      portNo: account.assigned_port_no || account.port_slot,
      folderPath: account.folder_path,
      reason: 'journal_verify_stuck',
      killMt5: true,
      clearPackagePort: true,
      forceFailed: true
    }).catch(() => {});
...
async function tryFastConnectConfirm(account) {
  ...
  const promoteConnected = async (source, msg = MT5_SUCCESS_MSG, metrics = {}) => {
    await promoteAccountConnected({
      accountId,
      portId,
      mt5Login: login,
      message: msg,
      balance: positiveMoney(metrics.balance),
      equity: positiveMoney(metrics.equity)
    });
    return { resolved: true, status: 'connected', source };
  };

  if (loginUsesEquityVerify(login)) {
    const sinceMs = account.connect_started_at
      ? new Date(account.connect_started_at).getTime()
      : await accountConnectSinceMs(accountId);
    const roundCmd = await findRecentTerminalLoginCommand(accountId, vpsId, { sinceMs }).catch(
      () => null
    );
    if (roundCmd) {
      const roundSt = String(roundCmd.status || '').toLowerCase();
      if (['cancelled', 'pending', 'processing', 'picked', 'running'].includes(roundSt)) {
        return { resolved: false };
      }
    }
    ...
    const metricsEq = await verifyPortLoginWithFallback(vpsId, portNo, login, {
      requireLoginMatch: true
    }).catch(() => ({ ok: false }));
    const fresh = metricsEq.ok
      ? await isPortHealthFresh(vpsId, portNo, sinceMs)
      : false;
    if (metricsEq.ok && fresh && positiveMoney(metricsEq.equity) > 0) {
      await query(
        `
        UPDATE vps_system.mt5_accounts
        SET last_equity=COALESCE($2, last_equity),
            last_balance=COALESCE($3, last_balance),
            updated_at=NOW()
        WHERE id=$1
      `,
        [accountId, metricsEq.equity || null, metricsEq.balance || null]
      ).catch(() => {});
      return promoteConnected(metricsEq.source || 'equity_fast', MT5_SUCCESS_MSG, metricsEq);
    }
  }
  ...
  if (winTitle && windowTitleConfirmsLogin(winTitle, login)) {
    const portRun = await verifyPortLoginWithFallback(vpsId, portNo, login).catch(() => ({
      ok: false
    }));
    if (portRun.ok) {
...
async function tryRecoverReadyAccount(account) {
  ...
  if (vpsId) {
    await cancelPendingLoginCommands(accountId, vpsId).catch(() => {});
  }
...
async function reconcileConnectedAccountLive(account, opts = {}) {
  ...
  const login = String(account?.mt5_login || '').trim();
  const st = String(account?.status || '').toLowerCase();
  const updatedMs = account?.updated_at ? new Date(account.updated_at).getTime() : 0;
  const offlineGraceMs = Number(process.env.MT5_CONNECTED_OFFLINE_GRACE_MS || 90000);
  ...
  if (recentLoginOk && (running || (updatedMs > 0 && Date.now() - updatedMs < offlineGraceMs))) {
    return { changed: false, account };
  }
  ...
  if (running && hasRecentAccountMetrics(account)) {
    return { changed: false, account };
  }

  if (running && (await portHealthLoginBindingRecent(vpsId, portNo, login))) {
    return { changed: false, account };
  }

  if (!running) {
    const metricsOk = await verifyPortMetricsLogin(vpsId, portNo, login);
    if (metricsOk.ok && updatedMs > 0 && Date.now() - updatedMs < offlineGraceMs) {
      return { changed: false, account };
    }
    if (!allowDemote) {
...
async function tryRecoverLoginFromPortHealth(account) {
  ...
  const metrics = await verifyPortLoginWithFallback(vpsId, portNo, login, {
    requireLoginMatch: false
  }).catch(() => ({ ok: false }));
  if (metrics.ok && positiveMoney(metrics.equity) > 0) {
    await promoteAccountConnected({
      accountId,
      portId,
      mt5Login: login,
      message: MT5_SUCCESS_MSG,
      balance: positiveMoney(metrics.balance) || undefined,
      equity: positiveMoney(metrics.equity) || undefined
    });
    return {
      resolved: true,
      status: 'connected',
      message: MT5_SUCCESS_MSG,
      source: metrics.source || 'account_metrics_recover'
    };
  }
...
async function verifyPortMetricsLogin(vpsId, portNo, mt5Login, opts = {}) {
  ...
  const r = await query(
    `
    SELECT balance, equity, mt5_login, updated_at, running, process_id, pid
    FROM vps_system.vps_port_health
    WHERE node_id=$1 AND port_number=$2
      AND updated_at > NOW() - INTERVAL '15 minutes'
    ORDER BY updated_at DESC
  ...
  const login = String(mt5Login).trim();
  const reported = String(row.mt5_login || '').trim();
  ...
  if (requireLoginMatch && !portHealthRowHasLiveMt5(row)) {
    return { ok: false, reason: 'MT5_NOT_LIVE' };
  }
  return { ok: true, source: 'port_health_metrics', balance: bal, equity: eq };
}
...
async function hasRecentAgentLoginSuccess(accountId, vpsId, mt5Login) {
  ...
  const resBal = positiveMoney(res.balance);
  const resEq = positiveMoney(res.equity);
  if (resBal > 0 || resEq > 0) return true;

  const portNo = Number(
    res.port || cmd.payload?.portNo || cmd.payload?.port_no || cmd.payload?.port || 0
  );
  if (portNo) {
    const metrics = await verifyPortLoginWithFallback(vpsId, portNo, mt5Login, {
      requireLoginMatch: false
    }).catch(() => ({ ok: false }));
    if (metrics.ok) return true;
  }
...
async function verifyPortRunningLogin(vpsId, portNo, mt5Login) {
  ...
  const r = await query(
    `
    SELECT running, mt5_login, updated_at, process_id, pid
    FROM vps_system.vps_port_health
    WHERE node_id=$1 AND port_number=$2
  ...
  const row = r.rows?.[0];
  if (!row || !portHealthRowHasLiveMt5(row)) return { ok: false, reason: 'PORT_NOT_RUNNING' };
...
async function verifyLoginFromCommand({ accountId, vpsId, mt5Login, portNo }) {
  ...
  if (loginUsesEquityVerify(login)) {
    const resEq = positiveMoney(result.equity);
    const resBal = positiveMoney(result.balance);
    if ((result.equityVerified === true || result.equity_verified === true) && resEq > 0) {
      return { ok: true, source: 'command_equity_flag', equity: resEq, balance: resBal };
    }
    if (resEq > 0 && ['success', 'done'].includes(st)) {
      return { ok: true, source: 'command_equity_snapshot', equity: resEq, balance: resBal };
    }
    const cmdPortNoEq = Number(
      cmd.payload?.portNo || cmd.payload?.port_no || cmd.payload?.port || portNo || 0
    );
    const sinceMs = await accountConnectSinceMs(accountId);
    const metricsEq = await verifyPortLoginWithFallback(vpsId, cmdPortNoEq, login, {
      requireLoginMatch: false
    }).catch(() => ({ ok: false }));
    const fresh = metricsEq.ok ? await isPortHealthFresh(vpsId, cmdPortNoEq, sinceMs) : false;
    if (metricsEq.ok && fresh && positiveMoney(metricsEq.equity) > 0) {
      return { ok: true, source: 'equity_port_metrics', equity: metricsEq.equity };
    }
    if (loginCommandNeedsEquityResult(cmd, login)) {
      return { ok: false, reason: 'EQUITY_PENDING' };
    }
    if (['failed', 'error'].includes(st)) {
      return { ok: false, reason: 'EQUITY_FAILED', status: st };
    }
    if (['success', 'done'].includes(st) && resEq <= 0) {
      return { ok: false, reason: 'EQUITY_REQUIRED' };
    }
  }
  ...
  if (
    ...
    return { ok: true, source: 'command_login_verified_flag' };
  }
  ...
  const runFinal = await verifyPortRunningLogin(vpsId, portNo, login).catch(() => ({ ok: false }));
  if (runFinal.ok) {
    return { ok: true, source: runFinal.source || 'port_running_after_cmd' };
  }

  if (await hasRecentAgentLoginSuccess(accountId, vpsId, login)) {
    return { ok: true, source: 'login_cmd_journal_history' };
  }

  if (['success', 'done'].includes(st)) {
    const resMsg = String(result.message || result.error || '').trim();
    if (
      resMsg &&
      /เชื่อมต่อสำเร็จ|connected|authorized on|journal ok/i.test(resMsg) &&
      !/failed|ผิด|authorization on.*failed/i.test(resMsg)
    ) {
      return { ok: true, source: 'command_success_message' };
    }
    const resBal = positiveMoney(result.balance);
    const resEq = positiveMoney(result.equity);
    if (resBal > 0 || resEq > 0) {
      return { ok: true, source: 'login_cmd_balance_snapshot' };
    }
    const metricsCmd = await verifyPortLoginWithFallback(vpsId, portNo || cmdPortNo2, login, {
      requireLoginMatch: false
    }).catch(() => ({ ok: false }));
    if (metricsCmd.ok) {
      return { ok: true, source: metricsCmd.source || 'login_cmd_port_metrics' };
    }
  }
...
const HARD_FAIL_REASONS = new Set([
  ...
  'equity_login_failed',
  'poll_journal_failed',
  'journal_verify_poll_timeout',
  'connect_poll_failed',
  'package_port_login_fail',
  'connect_fail_cleanup',
  'connect_fail_cleanup_ui',
  'connect_request_error'
]);

async function hasLoginCommandInProgress(accountId, vpsId) {
  if (!accountId && !vpsId) return false;
  const sinceMs = accountId ? await accountConnectSinceMs(accountId) : 0;
  const params = [];
  const parts = [];
  if (accountId) {
  ...
  if (accountId && vpsId) {
    const cmd = await findRecentTerminalLoginCommand(accountId, vpsId);
    const login = String(cmd?.payload?.mt5Login || cmd?.payload?.login || '').trim();
    if (loginCommandNeedsEquityResult(cmd, login)) {
      const finished = cmd.finished_at ? new Date(cmd.finished_at).getTime() : 0;
      if (!finished || Date.now() - finished < 120000) return true;
    }
  }
  return false;
}
...
/** ยกเลิก stop ค้างบน port/folder เดียวกัน — กัน login รอคิวหลัง handoff/release */
async function cancelPendingStopCommandsForSlot(vpsId, portSlot, folderPath = '') {
  const nid = Number(vpsId || 0);
  const slot = Number(portSlot || 0);
  ...
  let sql = `
    UPDATE vps_system.vps_agent_commands
    SET status='cancelled',
        error=COALESCE(error, 'cancelled: superseded by login_mt5'),
        finished_at=COALESCE(finished_at, NOW()),
        updated_at=NOW()
    WHERE (vps_id=$1 OR node_id=$1)
      AND command_type IN ('stop_mt5', 'force_stop_mt5', 'kill_mt5', 'stop_port')
      AND LOWER(COALESCE(status, '')) = 'pending'
  `;
  if (slot > 0 || folder) {
    sql += ` AND (`;
    const parts = [];
    if (slot > 0) {
      params.push(String(slot));
      parts.push(`COALESCE(payload->>'portSlot', payload->>'port_slot', '') = $${params.length}`);
    }
    if (folder) {
      params.push(folder);
      parts.push(`COALESCE(payload->>'vpsFolderPath', payload->>'folderPath', payload->>'folder_path', '') = $${params.length}`);
    }
    sql += parts.join(' OR ') + `)`;
  }
  sql += ` RETURNING id`;
...
async function expireStuckLoginCommands(vpsId, maxSec = 200) {
  const nid = Number(vpsId || 0);
...
async function syncJournalFromLatestCommand(accountId, vpsId, login, folderPath, portNo) {
  if (loginUsesEquityVerify(login)) {
    ...
    return { applied: false, action: 'equity_verify_queued' };
  }
  let nodeId = vpsId;
  if (!nodeId) {
    const restored = await restoreAccountVpsBindingFromLatestLogin(accountId).catch(() => null);
  ...
  if (!row) {
    ...
    return { applied: false, action: loginBusy ? 'login_in_progress' : 'queued' };
  }
...
/** ปิด MT5 เมื่อ login/เชื่อมต่อล้มเหลว — ยกเว้น preserveBinding หรือบัญชี connected จริง */
async function shouldKillMt5OnFail(accountId, opts = {}, journalVerdict = null) {
  if (opts.preserveBinding === true) return false;
  ...
  if (reason === 'login_journal_timeout' && (await accountLooksLoggedIn(accountId))) {
    return false;
  }
  return true;
}
...
async function queueStopMt5ForAccount(accountId, opts = {}) {
  ...
  await query(
    ...
    WHERE (vps_id=$1 OR node_id=$1)
      ...
      AND LOWER(COALESCE(status, '')) IN ('pending', 'processing', 'picked', 'running')
      AND NOT (
        TRIM(COALESCE(payload->>'mt5Login', payload->>'login', '')) ~ '^[82]'
        AND LOWER(COALESCE(status, '')) IN ('processing', 'picked', 'running')
      )
  `,
    [vpsId, String(accountId)]
...
async function failAccountFromJournal(accountId, portId, message, opts = {}) {
  ...
  if (reason === 'connect_poll_timeout') {
    if (opts.agentOffline === true) {
      ...
      opts.clearPackagePort = false;
    } else {
      opts.killMt5 = true;
      opts.clearPackagePort = true;
      opts.forceFailed = true;
    }
  }

  // login ผิด / เชื่อมต่อไม่ได้ → ปิด MT5 + ปล่อย PORT ว่างทันที (ยกเว้น agent offline)
  if (opts.preserveBinding !== true) {
    if (opts.killMt5 !== false) opts.killMt5 = true;
    if (opts.clearPackagePort !== false) opts.clearPackagePort = true;
    if (opts.forceFailed !== false) opts.forceFailed = true;
  }
  ...
  const pkgSlot = Number(acc.port_slot || opts.portSlot || opts.port_slot || 0);
  if (opts.preserveBinding) {
    await query(
      `
      UPDATE vps_system.mt5_accounts
  ...
  if (killMt5) {
    const stopSlot =
      pkgSlot || adminPortNoFromSystem(portNo) || Number(opts.portSlot || opts.port_slot || 0);
    let stopFolder = folderPath;
    if (!stopFolder && stopSlot > 0) {
      stopFolder = folderPathForPackageSlot({ node_name: opts.nodeName || 'VPS-WIN-01' }, stopSlot);
    }
    await queueStopMt5ForAccount(accountId, {
      portId: effectivePortId,
      vpsId,
      portNo: portNo || stopSlot,
      folderPath: stopFolder,
      reason: opts.reason || 'login_failed',
      killMt5: true,
      journalVerdict: opts.journalVerdict || null
    }).catch(() => {});
    if (vpsId && stopFolder && !clearPackagePort) {
  ...
  const nextStatus = killMt5 || forceFailed || clearPackagePort ? 'failed' : 'checking';
  if (killMt5 || forceFailed) {
    ...
    const clearSlot =
      pkgSlot || Number(opts.portSlot || opts.port_slot || 0) || adminPortNoFromSystem(portNo);
    if (clearPackagePort && clearSlot) {
      ...
      if (uid) {
        ...
        await clearOtherAccountsOnPortSlot(query, uid, clearSlot, null).catch(() => {});
      }
    }
    if (clearPackagePort && vpsId) {
      const releaseSlot = pkgSlot || adminPortNoFromSystem(portNo) || Number(opts.portSlot || opts.port_slot || 0);
      let releaseFolder = folderPath;
      if (!releaseFolder && releaseSlot > 0) {
        releaseFolder = folderPathForPackageSlot({ node_name: opts.nodeName || 'VPS-WIN-01' }, releaseSlot);
      }
      const { adminNodeId } = await resolveSystemVpsId(vpsId);
      await releaseUserPortCompletely({
        systemVpsId: vpsId,
        adminNodeId,
        portNo: adminPortNo || releaseSlot,
...
async function promoteAccountConnected({ accountId, portId, mt5Login, message, balance, equity }) {
  ...
  const accRes = await query(
    `
    SELECT a.user_id, a.vps_id, a.assigned_port_no, a.port_slot, a.port_id,
           COALESCE(p.folder_path, '') AS folder_path
    FROM vps_system.mt5_accounts a
    LEFT JOIN vps_system.vps_ports p ON p.id = a.port_id
    WHERE a.id=$1 LIMIT 1
  ...
  await query(`
    UPDATE vps_system.mt5_accounts
    SET status='connected', login_verified=TRUE, last_error=NULL, last_login_message=$2,
        connected_at=NOW(),
  ...
  if (userId) {
    setImmediate(() => {
      fetchPostLoginEquityOnce(accountId, userId, acc).catch((e) => {
        console.warn('[post-login-equity]', accountId, e.message);
      });
    });
  }

  if (acc?.vps_id) {
    await cancelPendingLoginCommands(accountId, acc.vps_id).catch(() => {});
  }
}
...
async function applyLoginMt5CommandFailed(node, payload, opts = {}) {
  ...
  const sinceMs = await accountConnectSinceMs(accountId);
  let failMsg = MT5_LOGIN_TIMEOUT_MSG;
  let reason = 'login_journal_timeout';
  const errRaw = String(opts.error || opts.message || message || '').trim();

  if (loginUsesEquityVerify(login) && /MT5_EQUITY|EQUITY_FAILED|เชื่อมต่อไม่สำเร็จ/i.test(errRaw)) {
    failMsg = 'เชื่อมต่อไม่สำเร็จ';
    reason = 'equity_login_failed';
  } else {
    const resolved = resolveLoginFailUserMessage({
      login,
      sinceMs,
    ...
    failMsg = resolved.message || MT5_LOGIN_TIMEOUT_MSG;
    const authFail =
      resolved.authFail === true || resolved.journalVerdict === 'failed';
    if (authFail) reason = 'login_cmd_failed';
  }
  const authFail = reason === 'login_cmd_failed' || reason === 'equity_login_failed';
  ...
  await failAccountFromJournal(accountId, Number(payload?.portId || payload?.port_id || 0), failMsg, {
    ...
    reason,
    journalVerdict: authFail ? 'failed' : null,
    killMt5: true,
    clearPackagePort: true,
    forceFailed: true
  });
...
async function applyLoginMt5FromCommandResult(node, payload, result) {
  ...
  let verified =
    ...
    result?.journalVerified === true;
  if (
  (result?.equityVerified === true || result?.equity_verified === true) &&
    positiveMoney(result?.equity) > 0
  ) {
    verified = true;
  }
  if (!verified && loginUsesEquityVerify(mt5Login) && positiveMoney(result?.equity) > 0) {
    verified = true;
  }
  if (evidence && mt5Login) {
    const sinceMs = await accountConnectSinceMs(accountId);
  ...
  if (!verified && mt5Login && node?.id) {
    const run = await verifyPortRunningLogin(node.id, portNo, mt5Login).catch(() => ({
      ok: false
    }));
    if (run.ok) verified = true;
  }
  if (!verified && loginUsesEquityVerify(mt5Login) && node?.id) {
    const sinceMs = await accountConnectSinceMs(accountId);
    const metrics = await verifyPortLoginWithFallback(node.id, portNo, mt5Login, {
      requireLoginMatch: false
    }).catch(() => ({ ok: false }));
    const fresh = metrics.ok ? await isPortHealthFresh(node.id, portNo, sinceMs) : false;
    if (metrics.ok && fresh && positiveMoney(metrics.equity) > 0) verified = true;
  }
  if (!verified) {
    const folderPath = payload?.folder_path || payload?.folderPath || payload?.vpsFolderPath || '';
    if (loginUsesEquityVerify(mt5Login)) {
      await query(
        `
        UPDATE vps_system.mt5_accounts
        SET status='checking', last_error=NULL,
            last_login_message='กำลังตรวจ Equity ล่าสุดจาก MT5...', updated_at=NOW()
...
async function resolveStuckLoginAccount(account) {
  ...
  // อย่า fail เร็วเกินไป — ช่วงเปิด MT5 (0–55s) port_health อาจยังไม่อัปเดต
  const loginBusyEarly = await hasLoginCommandInProgress(accountId, vpsId);
  const otherLoginOnVps = await hasOtherLoginWorkOnVps(vpsId, accountId).catch(() => false);
  const lastTouch = account.updated_at ? new Date(account.updated_at).getTime() : 0;
  const agentRecentlyActive =
    lastTouch > 0 && Date.now() - lastTouch < 20 * 1000;
  const offlineGraceSec = otherLoginOnVps || loginBusyEarly ? 120 : 55;

  if (elapsedSec >= offlineGraceSec && vpsId && portNo && !loginBusyEarly && !agentRecentlyActive) {
    const recentLogin = await findRecentTerminalLoginCommand(accountId, vpsId, {
      sinceMs: Math.max(0, startedAt - 30000)
    }).catch(() => null);
    const recentCancelMsg = String(
      [
        recentLogin?.error,
        recentLogin?.result_message,
        recentLogin?.result?.message
      ]
        .filter(Boolean)
        .join(' ')
    ).trim();
    const recentNeverPickedTransient =
      String(recentLogin?.status || '').toLowerCase() === 'cancelled'
      && !recentLogin?.started_at
      && (
        !recentCancelMsg
        || /new login attempt|superseded|login takes priority/i.test(recentCancelMsg)
      );
    if (recentNeverPickedTransient) {
      return { resolved: false };
    }
    const recentFinishedMs = recentLogin?.finished_at
      ? new Date(recentLogin.finished_at).getTime()
      : recentLogin?.created_at
        ? new Date(recentLogin.created_at).getTime()
        : 0;
    const recentConnected =
      loginCommandLooksConnected(recentLogin)
      && recentFinishedMs > 0
      && Date.now() - recentFinishedMs < 3 * 60 * 1000;
    if (recentConnected) {
      const res = recentLogin.result && typeof recentLogin.result === 'object' ? recentLogin.result : {};
      await promoteAccountConnected({
        accountId,
        portId,
        mt5Login: login,
        message: res.message || MT5_SUCCESS_MSG,
        balance: res.balance,
        equity: res.equity
      }).catch(() => {});
      return { resolved: true, status: 'connected', source: 'recent_login_success' };
    }

    let portRunning = false;
    for (const p of systemPortNosForFail(portNo)) {
      if (await isPortMt5Running(vpsId, p)) {
        portRunning = true;
        break;
      }
    }
    if (!portRunning) {
      const metricsDead = await verifyPortLoginWithFallback(vpsId, portNo, login, {
        requireLoginMatch: false
      }).catch(() => ({ ok: false }));
      if (!metricsDead.ok) {
        const fastOk = await tryFastConnectConfirm(account).catch(() => ({
          resolved: false
        }));
        if (fastOk.resolved) return fastOk;

        const uid = Number(account.user_id || 0);
        const slot = Number(account.port_slot || portNo || 0);
        const offlineMsg =
          'MT5 บน VPS ปิดอยู่ — กรุณากดเชื่อมต่อใหม่ (ไม่ต้อง restart Agent)';
        if (uid && slot) {
          await releaseFolderForLoginRetry(uid, slot, {
            message: offlineMsg,
            folderPath: account.folder_path
          }).catch(() => {});
        } else {
          await failAccountFromJournal(accountId, portId, offlineMsg, {
            vpsId,
            portNo,
            folderPath: account.folder_path,
            reason: 'mt5_process_gone'
          }).catch(() => {});
        }
        return { resolved: true, status: 'failed', message: offlineMsg };
      }
    }
  }

  const expired = await expireStuckLoginCommands(
    vpsId,
    loginUsesEquityVerify(login) ? 220 : 150
  ).catch(() => ({ expired: 0, accountIds: [] }));
  if (expired.accountIds?.includes(accountId)) {
    ...
    const stuckFail = await probeRecentLoginCommandFailed(account).catch(() => ({
      failed: true,
      message: MT5_LOGIN_TIMEOUT_MSG
    }));
    const stuckMsg = stuckFail.message || MT5_LOGIN_TIMEOUT_MSG;
    await failAccountFromJournal(accountId, portId, stuckMsg, {
      vpsId,
      portNo,
      folderPath: account.folder_path || '',
      reason: stuckFail.authFail ? 'login_cmd_stuck' : 'login_journal_timeout'
    }).catch(() => {});
    return { resolved: true, status: 'failed', message: stuckMsg };
  }
  ...
  if (elapsedSec >= 8 && !loginUsesEquityVerify(login) && !(await shouldDeferLoginJournalFail(account))) {
    const cmd = await findRecentLoginCommand(accountId, vpsId);
    if (cmd) {
      const st = String(cmd.status || '').toLowerCase();
      const res = cmd.result && typeof cmd.result === 'object' ? cmd.result : {};
      ...
      if (['failed', 'error'].includes(st)) {
        const probed = await probeRecentLoginCommandFailed(account).catch(() => ({
          failed: true,
          message: MT5_LOGIN_TIMEOUT_MSG
        }));
        const cmdFailMsg = probed.message || MT5_LOGIN_TIMEOUT_MSG;
        await failAccountFromJournal(accountId, portId, cmdFailMsg, {
          vpsId,
          portNo,
          folderPath: folderPathEarly,
          reason: probed.authFail ? 'login_cmd_failed' : 'login_journal_timeout',
          killMt5: probed.authFail === true,
          clearPackagePort: probed.authFail === true,
          journalVerdict: probed.authFail ? 'failed' : null,
          forceFailed: probed.authFail === true
        });
        return { resolved: true, status: 'failed', message: cmdFailMsg };
      }
    }
  }
  ...
  if (elapsedSec >= 12 && !loginUsesEquityVerify(login)) {
    const lateFail = await tryFastJournalFail(account);
    if (lateFail.resolved) return lateFail;
  }

  if (folderPathEarly && elapsedSec >= 1 && !loginUsesEquityVerify(login)) {
    ...
    if (appliedEarly) {
      ...
      if (stNowEarly === 'connected') {
        ...
        if (vEarly.ok) {
          await finishPendingLoginCommands(accountId, vpsId).catch(() => {});
          return { resolved: true, status: 'connected', source: 'journal_read_early' };
        }
        const metricsEarly = await verifyPortLoginWithFallback(vpsId, portNo, login, {
          requireLoginMatch: false
        }).catch(() => ({ ok: false }));
        const balEarly = positiveMoney(account.last_balance);
        if (metricsEarly.ok || balEarly > 0) {
          await finishPendingLoginCommands(accountId, vpsId).catch(() => {});
          return { resolved: true, status: 'connected', source: 'metrics_or_balance_early' };
        }
      }
    }
  }
  ...
  if (cmdFailedProbe.failed) {
    const balRecover = positiveMoney(account.last_balance);
    const eqRecover = positiveMoney(account.last_equity);
    const hasVerifiedSnapshot =
      account.login_verified === true ||
      balRecover > 0 ||
      eqRecover > 0;
    if (hasVerifiedSnapshot) {
      const metricsRecover =
        portNo
          ? await verifyPortLoginWithFallback(vpsId, portNo, login, {
              requireLoginMatch: false
            }).catch(() => ({ ok: false }))
          : { ok: false };
      if (metricsRecover.ok || hasVerifiedSnapshot) {
        await promoteAccountConnected({
          accountId,
          portId,
          mt5Login: login,
          message: MT5_SUCCESS_MSG,
          balance: balRecover || undefined,
          equity: eqRecover || undefined
        });
        await finishPendingLoginCommands(accountId, vpsId).catch(() => {});
        return {
          resolved: true,
          status: 'connected',
          source: 'command_failed_recovered',
          message: MT5_SUCCESS_MSG
        };
      }
    }
  ...
  if (applied2) {
    ...
    const metricsAfter = await verifyPortLoginWithFallback(vpsId, portNo, login, {
      requireLoginMatch: false
    }).catch(() => ({ ok: false }));
    if (metricsAfter.ok || positiveMoney(account.last_balance) > 0) {
      await finishPendingLoginCommands(accountId, vpsId).catch(() => {});
      return { resolved: true, status: 'connected', source: 'metrics_after_journal_read' };
    }
  }
  ...
  if (elapsedSec >= 6 && cmdVerify.reason === 'JOURNAL_REQUIRED') {
    const metricsLate = await verifyPortLoginWithFallback(vpsId, portNo, login, {
      requireLoginMatch: false
    }).catch(() => ({ ok: false }));
    const balLate = positiveMoney(account.last_balance);
    const eqLate = positiveMoney(account.last_equity);
    if (metricsLate.ok || balLate > 0 || eqLate > 0) {
      await promoteAccountConnected({
        accountId,
        portId,
        mt5Login: login,
        message: MT5_SUCCESS_MSG,
        balance: balLate || undefined,
        equity: eqLate || undefined
      });
      await finishPendingLoginCommands(accountId, vpsId);
      return {
        resolved: true,
        status: 'connected',
        source: 'port_metrics_late',
        message: MT5_SUCCESS_MSG
      };
    }
    ...
    if (winTitleLate && windowTitleConfirmsLogin(winTitleLate, login)) {
      const portRunLate = await verifyPortLoginWithFallback(vpsId, portNo, login).catch(() => ({
        ok: false
      }));
      if (portRunLate.ok) {
        await promoteAccountConnected({
  ...
  if (elapsedSec >= 45 && cmdVerify.reason === 'JOURNAL_REQUIRED') {
    const portRunMid = await verifyPortRunningLogin(vpsId, portNo, login).catch(() => ({
      ok: false
    }));
    if (portRunMid.ok) {
      await promoteAccountConnected({
      ...
      return {
        resolved: true,
        status: 'connected',
        source: 'port_health_running_mid',
        message: MT5_SUCCESS_MSG
      };
    }
  }

  if (elapsedSec >= 150 && cmdVerify.reason === 'JOURNAL_REQUIRED') {
    const latestAccountRes = await query(
      `
      SELECT status, login_verified, connected_at, last_balance, last_equity
      FROM vps_system.mt5_accounts
      WHERE id = $1
      LIMIT 1
    `,
    ...
    const latestAccount = latestAccountRes.rows?.[0] || {};
    const latestStatus = String(latestAccount.status || '').toLowerCase();
    const latestVerified = latestAccount.login_verified === true;
    const latestConnectedAt = latestAccount.connected_at ? new Date(latestAccount.connected_at).getTime() : 0;
    const latestBal = positiveMoney(latestAccount.last_balance);
    const latestEq = positiveMoney(latestAccount.last_equity);
    if (
      ...
      await promoteAccountConnected({
        ...
        mt5Login: login,
        message: MT5_SUCCESS_MSG,
        balance: latestBal || undefined,
        equity: latestEq || undefined
      });
      await finishPendingLoginCommands(accountId, vpsId).catch(() => {});
      return {
        resolved: true,
        status: 'connected',
        source: 'db_connected_before_timeout_release',
        message: MT5_SUCCESS_MSG
      };
    }

    const portRunLate = await verifyPortRunningLogin(vpsId, portNo, login).catch(() => ({
      ok: false
    }));
    if (portRunLate.ok) {
      await promoteAccountConnected({
      ...
      return {
        resolved: true,
        status: 'connected',
        source: 'port_health_running_late',
        message: MT5_SUCCESS_MSG
      };
    }
    const uid = Number(account.user_id || 0);
    const slot = Number(account.port_slot || portNo || 0);
    if (uid && slot) {
      await releaseUserPackagePortSlot(uid, slot, {
        message: MT5_LOGIN_TIMEOUT_MSG,
...
/** หลัง login/timeout ล้มเหลว — ปิด MT5 + ปล่อย PORT ว่าง (delegate ไป releaseUserPackagePortSlot) */
async function releaseFolderForLoginRetry(userId, portSlot, opts = {}) {
  return releaseUserPackagePortSlot(userId, portSlot, {
    ...opts,
    reason: opts.reason || 'login_fail_auto_release',
    message: opts.message || MT5_LOGIN_TIMEOUT_MSG
  });
}
...
async function releaseUserPackagePortSlot(userId, portSlot, opts = {}) {
  ...
  for (const row of rows.rows || []) {
    ...
    const adminPortNo = Number(row.assigned_port_no || row.windows_port_no || 0);
    if (vpsKey) {
      const key = `${vpsKey}:${adminPortNo || slot}`;
      seenBindings.set(key, {
        vpsId: vpsKey,
        adminPortNo: adminPortNo || slot,
        folderPath: String(row.folder_path || '').trim()
      });
    }
  }
  ...
  for (const binding of seenBindings.values()) {
    const adminSlot = Number(binding.adminPortNo || adminPortNoFromSystem(slot) || slot);
    const bindFolderPath =
      String(binding.folderPath || folderPath || '').trim() ||
      folderPathForPackageSlot({ node_name: 'VPS-WIN-01' }, adminSlot);
    await queueForceStopMt5(binding.vpsId, adminSlot, bindFolderPath, 'package_port_login_fail').catch(() => {});
    for (const pno of systemPortNosForFail(adminSlot)) {
      await clearPortHealthRunning(binding.vpsId, pno).catch(() => {});
    }
    const { adminNodeId } = await resolveSystemVpsId(binding.vpsId).catch(() => ({ adminNodeId: 0 }));
    await releaseUserPortCompletely({
      systemVpsId: binding.vpsId,
      adminNodeId: adminNodeId || binding.vpsId,
      portNo: adminSlot,
      folderPath: bindFolderPath
    }).catch(() => {});
  }

  if (!seenBindings.size) {
    const vpsR = await query(
      `
      SELECT DISTINCT a.vps_id, COALESCE(p.folder_path, '') AS folder_path
      FROM vps_system.mt5_accounts a
      LEFT JOIN vps_system.vps_ports p ON p.id = a.port_id
      WHERE a.user_id = $1
    ...
    const vpsRow = vpsR.rows?.[0];
    const vpsId = Number(vpsRow?.vps_id || 0);
    if (vpsId) {
      if (!folderPath && vpsRow.folder_path) {
        folderPath = String(vpsRow.folder_path).trim();
      }
      if (!folderPath) {
      ...
      for (const pno of systemPortNosForFail(adminSlot)) {
        await clearPortHealthRunning(vpsId, pno).catch(() => {});
      }
...
/** ปิด MT5 บน FolderPort ตามช่องแพ็กเกจ — ใช้เมื่อเว็บว่างแล้วแต่ terminal64 ยังรันบน VPS */
async function forceStopPackagePortSlot(userId, portSlot, opts = {}) {
  const uid = Number(userId || 0);
  const slot = Number(portSlot || 0);
  if (!uid || !slot) return { ok: false, message: 'missing user or port slot' };
  ...
  const accR = await query(
    `
    SELECT a.vps_id, a.assigned_port_no, a.windows_port_no, COALESCE(p.folder_path, '') AS folder_path
    FROM vps_system.mt5_accounts a
    LEFT JOIN vps_system.vps_ports p ON p.id = a.port_id
    WHERE a.user_id=$1
      AND a.port_slot=$2
  ...
  const acc = accR.rows?.[0] || null;

  const { pickFolderPortForSlot, listAllFolderPortsForConnect } = require('./mt5VpsFolderPorts');
  ...
  const adminSlot = Number(acc?.assigned_port_no || acc?.windows_port_no || adminPortNoFromSystem(slot) || slot);
  const portNos = systemPortNosForFail(adminSlot);

  let vpsId = Number(opts.vpsId || acc?.vps_id || 0);
  if (!vpsId && folder?.adminNodeId) {
    const resolved = await resolveSystemVpsId(folder.adminNodeId).catch(() => ({
      systemVpsId: 0
    }));
    vpsId = Number(resolved.systemVpsId || 0);
  }
  if (!vpsId) {
    ...
    vpsId = Number(nodesQ.rows?.[0]?.vps_id || 0);
  }

  let folderPath = String(opts.folderPath || acc?.folder_path || folder?.folderPath || '').trim();
  if (!folderPath && vpsId) {
    folderPath = folderPathForPackageSlot({ node_name: 'VPS-WIN-01' }, adminSlot);
  }
  ...
  if (vpsId) {
    ...
    await releaseUserPortCompletely({
      ...
      portNo: adminSlot,
      folderPath,
      reason: opts.reason || 'ui_force_stop_port'
    }).catch(() => {});
    for (const pno of portNos.length ? portNos : [adminSlot, 100 + slot]) {
      await clearPortHealthRunning(vpsId, pno).catch(() => {});
    }
  }
  ...
  return {
    ...
    vpsId,
    folderPath,
    message: 'ส่งคำสั่งปิด MT5 บน VPS แล้ว — รอ 5–15 วินาที'
  };
}
...
module.exports = {
  ...
  finalizeLoginCommandSuccess,
  cancelPendingLoginCommands,
  ...
  cancelPendingStopCommandsForSlot,
