...
function extractJournalEvidence(...sources) {
  for (const src of sources) {
    ...
    if (
      text.length >= 24
      && /authorized on|authorization on|authorization failed|invalid account|previous successful authorization/i.test(
        text
      )
    ) {
...
/** ตรวจผล login_mt5 ล่าสุด — รวมคำสั่งที่ agent รายงาน failed */
async function probeFailedLoginFromRecentCommand(accountId, vpsId, login) {
  const cmd = await findRecentLoginCommand(accountId, vpsId);
  if (!cmd) return null;

  const result = cmd.result && typeof cmd.result === 'object' ? cmd.result : {};
  ...
  if (evidence && loginStr && parseMt5JournalOutcome(evidence, loginStr) === 'failed') {
    return { evidence };
  }

  const st = String(cmd.status || '').toLowerCase();
  if (['failed', 'error'].includes(st)) {
    const blob = `${evidence} ${result.message || ''} ${cmd.error || ''}`;
    if (
      loginStr
      && parseMt5JournalOutcome(blob, loginStr) === 'failed'
    ) {
      return { evidence: evidence || blob.slice(0, 8000) };
    }
    if (/invalid account|authorization.*failed|ผู้ใช้งานผิด|เชื่อมต่อไม่สำเร็จ/i.test(blob)) {
      return { evidence: evidence || blob.slice(0, 8000) };
    }
  }
...
async function verifyLoginFromCommand({ accountId, vpsId, mt5Login, portNo }) {
  ...
  const st = String(cmd.status || '').toLowerCase();
  if (['failed', 'error'].includes(st)) {
    const failProbe = await probeFailedLoginFromRecentCommand(accountId, vpsId, login);
    if (failProbe) {
      return { ok: false, reason: 'JOURNAL_FAILED', journalEvidence: failProbe.evidence };
    }
...
async function applyLoginMt5CommandFailed(node, payload, opts = {}) {
  ...
  } else if (/invalid account|authorization.*failed|ผู้ใช้งานผิด|เชื่อมต่อไม่สำเร็จ/i.test(`${message} ${evidence}`)) {
    failMsg = MT5_FAIL_USER_MSG;
  } else if (message) {
    failMsg = message;
  }
...
async function applyLoginMt5FromCommandResult(node, payload, result) {
  ...
  if (resultStatus === 'started' && mt5Login) {
    const earlyEvidence = extractJournalEvidence(
      result?.journalEvidence,
      result?.journal_evidence,
      result?.message
    );
    if (earlyEvidence && parseMt5JournalOutcome(earlyEvidence, mt5Login) === 'failed') {
      await failAccountFromJournal(accountId, Number(payload?.portId || payload?.port_id || 0), MT5_FAIL_USER_MSG, {
        vpsId: node.id,
        portNo: payload?.portNumber || payload?.port,
...
async function resolveStuckLoginAccount(account) {
  ...
  const startedAt = account.connect_started_at || account.updated_at;
  ...
  if (elapsedSec < 1) return { resolved: false };

  const cmdFail = await probeFailedLoginFromRecentCommand(accountId, vpsId, login).catch(() => null);
  if (cmdFail) {
    await failAccountFromJournal(accountId, portId, MT5_FAIL_USER_MSG, {
      vpsId,
      portNo,
      folderPath: folderPathEarly,
      reason: 'login_command_journal_failed'
    });
  ...
  if (cmdVerify.reason === 'JOURNAL_FAILED') {
    const ev = cmdVerify.journalEvidence || '';
    const verdict = ev && login ? parseMt5JournalOutcome(ev, login) : null;
    if (verdict === 'failed') {
      await failAccountFromJournal(accountId, portId, MT5_FAIL_USER_MSG);
      return { resolved: true, status: 'failed', message: MT5_FAIL_USER_MSG };
    }
...
module.exports = {
  ...
  probeFailedLoginFromRecentCommand,
