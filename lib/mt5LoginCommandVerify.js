...
async function tryFastJournalFail(account) {
  ...
  await failAccountFromJournal(
    ...
    MT5_FAIL_USER_MSG,
    {
      ...
      reason: `journal_fail_fast_${fail.source || 'unknown'}`
    }
  );
...
async function tryFastConnectConfirm(account) {
  ...
  if (await hasRecentAgentLoginSuccess(accountId, vpsId, login)) {
    return promoteConnected('login_cmd_recent_success', MT5_SUCCESS_MSG);
  }

  const sinceMsEarly = await journalSinceMsForVerify(accountId, vpsId, portNo);
  const stashedEarly = await getStashedJournalEvidence(accountId);
  if (
    stashedEarly &&
    parseMt5JournalOutcome(stashedEarly, login, undefined, sinceMsEarly) === 'success'
  ) {
    return promoteConnected('stashed_journal_early', MT5_SUCCESS_MSG);
  }
  ...
  const portVerify = await verifyPortRunningLogin(vpsId, portNo, login);
  if (portVerify.ok) {
    return promoteConnected('port_health_login', MT5_SUCCESS_MSG);
  }

  const metricsVerify = await verifyPortMetricsLogin(vpsId, portNo, login);
  if (metricsVerify.ok) {
    return promoteConnected(metricsVerify.source || 'port_health_metrics', MT5_SUCCESS_MSG);
  }

  const mt5Running = await isPortMt5Running(vpsId, portNo);
  if (!mt5Running) {
    return { resolved: false };
  }
  ...
  const sinceMs = await journalSinceMsForVerify(accountId, vpsId, portNo);
  const stashed = await getStashedJournalEvidence(accountId);
  if (stashed && parseMt5JournalOutcome(stashed, login, undefined, sinceMs) === 'success') {
...
async function tryRecoverReadyAccount(account) {
  ...
  const recentOk = await hasRecentAgentLoginSuccess(accountId, vpsId, login);
  const metricsOnAccount = hasRecentAccountMetrics(account);
  const metricsOnPort = (await verifyPortMetricsLogin(vpsId, portNo, login).catch(() => ({
    ok: false
  }))).ok;
  if (!recentOk && !metricsOnAccount && !metricsOnPort) {
    return { recovered: false, account };
  }
...
/** ยืนยัน connected จาก VPS จริง (port_health) แม้คำสั่ง DB ยัง pending/processing */
async function tryRecoverLoginFromPortHealth(account) {
  ...
  let verify = await verifyPortRunningLogin(vpsId, portNo, login);
  if (!verify.ok) {
    const cmdVerify = await verifyLoginFromCommand({
      accountId,
      vpsId,
    ...
    if (cmdVerify.ok) {
      ...
      const metricsVerify = await verifyPortMetricsLogin(vpsId, portNo, login);
      if (!metricsVerify.ok) return { resolved: false };
      verify = metricsVerify;
    }
  }
  await promoteAccountConnected({
    accountId,
  ...
  return {
    ...
    status: 'connected',
    message: MT5_SUCCESS_MSG,
    source: verify.source || 'port_health_recover'
  };
}

/** login สำเร็จบน VPS แต่ process scan อาจไม่เห็น terminal64 — ใช้ balance/equity ล่าสุดจาก port_health */
async function verifyPortMetricsLogin(vpsId, portNo, mt5Login) {
...
async function hasRecentAgentLoginSuccess(accountId, vpsId, mt5Login) {
  ...
  return (
    res.loginVerified === true ||
    res.login_verified === true ||
    res.journalVerified === true ||
    String(res.status || '').toLowerCase() === 'connected'
  );
}
...
async function verifyPortRunningLogin(vpsId, portNo, mt5Login) {
  ...
  const metricsFast = await verifyPortMetricsLogin(vpsId, portNo, mt5Login).catch(() => ({
    ok: false
  }));
  if (metricsFast.ok) {
...
async function verifyLoginFromCommand({ accountId, vpsId, mt5Login, portNo }) {
  ...
  if (
    ...
    return { ok: true, source: 'command_login_verified' };
  }
...
async function failAccountFromJournal(accountId, portId, message, opts = {}) {
  ...
  const nextStatus = killMt5 || forceFailed ? 'failed' : 'checking';
...
async function resolveStuckLoginAccount(account) {
  ...
  if (elapsedSec >= 90 && cmdVerify.reason === 'JOURNAL_REQUIRED') {
    const recentOk = await hasRecentAgentLoginSuccess(accountId, vpsId, login).catch(() => false);
    if (recentOk) {
      await promoteAccountConnected({
        accountId,
        portId,
