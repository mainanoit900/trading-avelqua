...
async function failAccountFromJournal(accountId, portId, message, opts = {}) {
  ...
  if (vpsId && portNo) {
    ...
    if (adminNodeId && adminPortNo) {
      await reconcilePortIdleWhenAgentFree(adminNodeId, adminPortNo, folderPath, { accountId }).catch(() => {});
...
async function promoteAccountConnected({
  ...
  if (acc?.vps_id) {
    ...
    const adminPortNo = adminPortNoFromSystem(portNo);
    if (adminNodeId && adminPortNo) {
      await setAdminAllocationStatus(adminNodeId, adminPortNo, 'used').catch(() => {});
    }
  }
}
