...
async function isAccountLoginJournalVerified(account) {
  ...
  const bal = positiveMoney(account.last_balance);
  if (bal > 0 && account.vps_id) {
    const metrics = await verifyPortLoginWithFallback(
      account.vps_id,
      account.assigned_port_no || account.port_slot,
      account.mt5_login,
      { requireLoginMatch: false }
    ).catch(() => ({ ok: false }));
    if (metrics.ok) return { ok: true, source: metrics.source || 'account_balance_metrics' };
    if (bal > 0) return { ok: true, source: 'account_balance_snapshot' };
  }
  return cmd;
}
...
async function tryFastConnectConfirm(account) {
  ...
  const balSnap = positiveMoney(account.last_balance);
  const eqSnap = positiveMoney(account.last_equity);
  if (balSnap > 0 && vpsId) {
    const metricsSnap = await verifyPortLoginWithFallback(vpsId, portNo, login, {
      requireLoginMatch: false
    }).catch(() => ({ ok: false }));
    if (metricsSnap.ok || balSnap > 0) {
      return promoteConnected('account_metrics_fast', MT5_SUCCESS_MSG);
    }
  }
...
async function verifyLoginFromCommand({ accountId, vpsId, mt5Login, portNo }) {
  ...
  if (!['success', 'done'].includes(st)) {
    ...
    const metricsPending = await verifyPortLoginWithFallback(vpsId, cmdPortNo, login, {
      requireLoginMatch: false
    }).catch(() => ({ ok: false }));
    if (metricsPending.ok) {
      return { ok: true, source: metricsPending.source || 'port_metrics_while_processing' };
    }
    return { ok: false, reason: 'COMMAND_NOT_SUCCESS', status: st };
  }
  ...
  if (
    ...
    if (!evidence || !login) {
      const metricsNoJournal = await verifyPortLoginWithFallback(vpsId, cmdPortNo2, login, {
        requireLoginMatch: false
      }).catch(() => ({ ok: false }));
      if (metricsNoJournal.ok) {
        return { ok: true, source: metricsNoJournal.source || 'port_metrics_no_journal_evidence' };
      }
    ...
    const metricsLate = await verifyPortLoginWithFallback(vpsId, cmdPortNo2, login, {
      requireLoginMatch: false
    }).catch(() => ({ ok: false }));
    if (metricsLate.ok) {
      return { ok: true, source: metricsLate.source || 'port_metrics_login_verified' };
    }
    return { ok: false, reason: 'JOURNAL_REQUIRED' };
  }

  const metricsFinal = await verifyPortLoginWithFallback(vpsId, portNo, login, {
    requireLoginMatch: false
  }).catch(() => ({ ok: false }));
  if (metricsFinal.ok) {
    return { ok: true, source: metricsFinal.source || 'port_metrics_final' };
  }
...
async function resolveStuckLoginAccount(account) {
  ...
  if (elapsedSec >= 90 && cmdVerify.reason === 'JOURNAL_REQUIRED') {
    ...
    if (metricsLate.ok) {
      return promoteConnected('port_metrics_late', MT5_SUCCESS_MSG);
    }
