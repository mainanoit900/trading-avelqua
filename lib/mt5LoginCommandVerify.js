...
function loginCommandNeedsEquityResult(cmd, login) {
  ...
  if (res.equityVerified === true || res.equity_verified === true) return false;
...
async function finalizeLoginCommandSuccess(accountId, vpsId, resultPatch = {}) {
  ...
  if (login && loginUsesEquityVerify(login)) {
...
async function verifyLoginFromCommand({ accountId, vpsId, mt5Login, portNo }) {
  ...
  if (loginUsesEquityVerify(login)) {
    ...
    if (result.equityVerified === true || result.equity_verified === true) {
      return { ok: true, source: 'command_equity_flag' };
    }
...
async function promoteAccountConnected({ accountId, portId, mt5Login, message, balance, equity }) {
  ...
  if (acc?.vps_id) {
    await finalizeLoginCommandSuccess(accountId, acc.vps_id, {
      login: mt5Login,
      message: msg,
      balance: bal || undefined,
      equity: eq || undefined,
      loginVerified: true,
      equityVerified: loginUsesEquityVerify(mt5Login)
    }).catch(() => {});
  }
}
...
async function applyLoginMt5FromCommandResult(node, payload, result) {
  ...
  let verified =
    ...
    result?.login_verified === true ||
    result?.journalVerified === true ||
    result?.equityVerified === true ||
    result?.equity_verified === true;
