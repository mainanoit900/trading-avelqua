...
/** เคยยืนยันรหัสสำเร็จแล้ว + รหัสที่กรอกตรงกับที่เก็บ — เปิด MT5 ใหม่ได้โดยไม่ต้องตรวจ journal ซ้ำ */
async function accountCanSkipLoginVerify(account, mt5Password) {
  const accountId = Number(account?.id || 0);
  const login = String(account?.mt5_login || '').trim();
  const pwd = String(mt5Password || '').trim();
  const stored = String(account?.mt5_password || '').trim();
  if (!accountId || !login || !pwd || !stored || pwd !== stored) {
    return false;
  }

  if (account.login_verified === true) return true;

  const journal = String(account?.last_journal_evidence || '').trim();
  if (journal) {
    const sinceMs = await accountConnectSinceMs(accountId).catch(() => 0);
    const jv = parseJournalRelaxed(journal, login, sinceMs);
  ...
  if (cmd.ok && cmd.reason !== 'COMMAND_NOT_SUCCESS') return true;

  if (await hasRecentAgentLoginSuccess(accountId, account.vps_id, login)) {
    return true;
  }
...
/** MT5 รันบน VPS ตรง login/port แล้ว — ไม่ต้องส่ง login_mt5 */
async function mt5LiveForAccount(accountRow) {
  ...
  if (!vpsId || !portNo || !login) return false;
...
async function tryFastConnectConfirm(account) {
  ...
  const metricsFast = await verifyPortLoginWithFallback(vpsId, portNo, login, {
    requireLoginMatch: false
  }).catch(() => ({ ok: false }));
  const balFast = positiveMoney(account.last_balance);
  const eqFast = positiveMoney(account.last_equity);
  const skipVerifyFast = await accountCanSkipLoginVerify(
    account,
    account.mt5_password
  ).catch(() => false);
  if (skipVerifyFast && (metricsFast.ok || balFast > 0 || eqFast > 0)) {
    return promoteConnected(
      metricsFast.source || 'port_metrics_fast',
      MT5_SUCCESS_MSG
    );
  }
...
async function hasRecentAgentLoginSuccess(accountId, vpsId, mt5Login) {
  ...
  if (portNo) {
    const running = await verifyPortRunningLogin(vpsId, portNo, mt5Login).catch(() => ({
      ok: false
    }));
    ...
    if (metrics.ok) {
      const live2 = await verifyPortRunningLogin(vpsId, portNo, mt5Login).catch(() => ({
        ok: false
      }));
      if (live2.ok) return true;
    }
  }
...
async function promoteAccountConnected({ accountId, portId, mt5Login, message, balance, equity }) {
  ...
  if (acc?.vps_id) {
    ...
    const live = await verifyPortRunningLogin(acc.vps_id, portNo, mt5Login).catch(() => ({
      ok: false
    }));
    if (live.ok) {
      await upsertPortHealthRunning(acc.vps_id, portNo, folderPath, mt5Login).catch(() => {});
    } else {
      await clearPortHealthRunning(acc.vps_id, portNo).catch(() => {});
      await query(
        `
        UPDATE vps_system.mt5_accounts
      ...
      `,
        [
          accountId,
          'เชื่อมต่อสำเร็จ — MT5 ปิดบน VPS กดเชื่อมต่อเพื่อเปิด terminal64'
        ]
      ).catch(() => {});
    }
...
async function applyLoginMt5CommandFailed(node, payload, opts = {}) {
  ...
  const timeoutOnly =
    resolved.journalVerdict === 'timeout' &&
    resolved.authFail !== true &&
    /ทันเวลา|timeout|ไม่สามารถยืนยัน/i.test(String(message || resolved.message || ''));
  const authFail =
    resolved.authFail === true ||
    resolved.journalVerdict === 'failed' ||
    !timeoutOnly;
  const failMsg = authFail ? MT5_FAIL_USER_MSG : resolved.message || MT5_LOGIN_TIMEOUT_MSG;
...
async function applyLoginMt5FromCommandResult(node, payload, result) {
  ...
  const runFinal = await verifyPortRunningLogin(node.id, portNo, mt5Login).catch(() => ({
    ok: false
  }));
  if (!runFinal.ok) {
    await query(
      ...
      SET status='checking',
          last_error=NULL,
          last_login_message='เปิด MT5 แล้วแต่ยังไม่เห็น terminal64 — รอ Agent...',
          updated_at=NOW()
      WHERE id=$1
    `,
      [accountId]
...
async function resolveStuckLoginAccount(account) {
  ...
  if (elapsedSec >= 55 && vpsId && portNo && !loginBusyEarly && !agentRecentlyActive) {
  ...
  if (elapsedSec >= 35 && cmdVerify.reason === 'JOURNAL_REQUIRED') {
  ...
  if (elapsedSec >= 120 && cmdVerify.reason === 'JOURNAL_REQUIRED') {
...
/**
 * ก่อนกด Login — เคลียร์ PORT แพ็กเกจอัตโนมัติ:
 * - ปิด MT5 บน FolderPort (VPS)
 * - ถอด login เก่าที่คนละบัญชี / สถานะ failed·ready ออกจากช่อง
 * - ถ้า login เดิม connected และ MT5 ยังรันอยู่ ไม่ปิดทันที
 */
async function autoClearPortSlotBeforeLogin(userId, portSlot, mt5Login, serverName = '') {
  const uid = Number(userId || 0);
  const slot = Number(portSlot || 0);
  const login = String(mt5Login || '').trim();
  const srv = String(serverName || '').trim();
  if (!uid || !slot) return { ok: false, skipped: true };

  const rows = await query(
    `
    SELECT a.id, a.mt5_login, a.status, a.vps_id, a.port_id, a.assigned_port_no,
           a.server_name, a.mt5_server,
           COALESCE(p.folder_path, '') AS folder_path
    FROM vps_system.mt5_accounts a
    LEFT JOIN vps_system.vps_ports p ON p.id = a.port_id
    WHERE a.user_id = $1
  ...
  let skipForceStop = false;

  for (const row of rows.rows || []) {
    const st = String(row.status || '').toLowerCase();
    const rowLogin = String(row.mt5_login || '').trim();
    const rowSrv = String(row.server_name || row.mt5_server || '').trim();
    const sameLogin =
      login &&
      rowLogin === login &&
      (!srv || !rowSrv || srv === rowSrv);

    if (sameLogin && st === 'connected') {
      /* MT5 ปิดอยู่แล้ว — ไม่ส่ง stop ก่อน login (กัน race ที่ login สำเร็จปลอมแต่ terminal64 ไม่รัน) */
      skipForceStop = true;
      continue;
    }
    if (sameLogin && ['connecting', 'starting', 'checking'].includes(st)) {
      continue;
    }
    ...
    if (st === 'connected') {
      await query(
        ...
        WHERE id = $1
      `,
        [row.id, `เคลียร์ PORT ${slot} อัตโนมัติ — รอ Login ${login || 'ใหม่'}`]
      ).catch(() => {});
      await cancelPendingLoginForAccount(Number(row.id), Number(row.vps_id || 0)).catch(
        () => {}
      );
      continue;
    }

    if (!['deleted', 'expired'].includes(st)) {
      await failAccountFromJournal(
        Number(row.id),
        Number(row.port_id || 0),
        `เคลียร์ PORT ${slot} ก่อน Login`,
        {
          vpsId: row.vps_id,
          portNo: row.assigned_port_no || slot,
          folderPath: row.folder_path,
          reason: 'login_auto_clear_port',
          killMt5: false,
          clearPackagePort: true,
          forceFailed: true
        }
  ...
  if (!skipForceStop) {
    await forceStopPackagePortSlot(uid, slot, {
      reason: 'login_auto_clear_port'
    }).catch(() => ({}));
    await waitForVpsPortAgentCommandsIdle(uid, slot, ['stop_mt5', 'kill_mt5', 'force_stop_mt5'], 25000).catch(
      () => false
    );
  }

  return { ok: true, skipForceStop };
}

/** รอคำสั่ง Agent บน PORT จบก่อนส่ง login_mt5 ถัดไป */
async function waitForVpsPortAgentCommandsIdle(vpsId, portSlot, commandTypes, timeoutMs = 20000) {
  const nid = Number(vpsId || 0);
  const slot = Number(portSlot || 0);
  const types = (commandTypes || []).filter(Boolean);
  if (!nid || !types.length) return true;
  const portList = systemPortNosForFail(slot);
  const portTexts = (portList.length ? portList : [slot, 100 + slot]).map(String);
  ...
  while (Date.now() < deadline) {
    const r = await query(
      ...
      WHERE (vps_id=$1 OR node_id=$1)
        ...
        AND LOWER(COALESCE(status, '')) IN ('pending', 'processing', 'picked', 'running')
        AND (
          (payload->>'portSlot')::text = ANY($3::text[])
          OR (payload->>'port')::text = ANY($3::text[])
          OR (payload->>'port_no')::text = ANY($3::text[])
          OR (payload->>'portNumber')::text = ANY($3::text[])
        )
      LIMIT 1
    `,
      [nid, types, portTexts]
...
async function forceStopPackagePortSlot(userId, portSlot, opts = {}) {
  ...
  if (vpsId) {
    for (const pno of portNos.length ? portNos : [adminSlot, 100 + slot]) {
      await queueForceStopMt5(vpsId, pno, folderPath, opts.reason || 'ui_force_stop_port').catch(
        () => {}
      );
