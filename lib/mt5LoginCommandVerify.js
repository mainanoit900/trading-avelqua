...
const { positiveMoney, fetchPostLoginEquityOnce } = require('./mt5EquitySync');
...
async function tryFastConnectConfirm(account) {
  ...
  if (loginUsesEquityVerify(login)) {
    ...
    if (positiveMoney(metricsEq.equity) > 0) {
      await query(
        `
      ...
      return promoteConnected(metricsEq.source || 'equity_fast', MT5_SUCCESS_MSG);
    }
  }
...
async function tryRecoverLoginFromPortHealth(account) {
  ...
  const balAcc = positiveMoney(account.last_balance);
  const eqAcc = positiveMoney(account.last_equity);
  if (metrics.ok || balAcc > 0 || eqAcc > 0) {
    await promoteAccountConnected({
      accountId,
      portId,
      mt5Login: login,
      message: MT5_SUCCESS_MSG,
      balance: balAcc || undefined,
      equity: eqAcc || undefined
    });
    await finishPendingLoginCommands(accountId, vpsId);
    return {
      resolved: true,
