...
async function tryFastConnectConfirm(account) {
  ...
  if (!['connecting', 'starting', 'checking'].includes(status) || !login || !vpsId || !portNo) {
  ...
  if (ph.ok) {
    ...
    return { resolved: true, status: 'connected', source: 'port_health_fast' };
  }
  ...
  if (liveJournal) {
    ...
    if (String(st.rows?.[0]?.status || '').toLowerCase() === 'connected') {
      await finishPendingLoginCommands(accountId, vpsId);
      return { resolved: true, status: 'connected', source: 'journal_live_fast' };
    }
  }
  ...
  if (cmd) {
    ...
    if (evidence && parseMt5JournalOutcome(evidence, login, undefined, 0) === 'success') {
      await promoteAccountConnected({
        accountId,
        portId,
        mt5Login: login,
      ...
      await finishPendingLoginCommands(accountId, vpsId);
      return { resolved: true, status: 'connected', source: 'login_cmd_journal_fast' };
    }
  }
