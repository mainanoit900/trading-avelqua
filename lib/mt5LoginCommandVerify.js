...
async function verifyPortMetricsLogin(vpsId, portNo, mt5Login, opts = {}) {
  ...
  const r = await query(
    `
    SELECT balance, equity, mt5_login, payload, updated_at, running, process_id, pid
    FROM vps_system.vps_port_health
    WHERE node_id=$1 AND port_number=$2
      AND updated_at > NOW() - INTERVAL '15 minutes'
    ORDER BY updated_at DESC
  ...
  const bal = positiveMoney(row?.balance ?? pl.balance);
  const eq = positiveMoney(row?.equity ?? pl.equity);
...
async function verifyPortRunningLogin(vpsId, portNo, mt5Login) {
  ...
  const r = await query(
    `
    SELECT running, mt5_login, payload, updated_at, process_id, pid
    FROM vps_system.vps_port_health
    WHERE node_id=$1 AND port_number=$2
