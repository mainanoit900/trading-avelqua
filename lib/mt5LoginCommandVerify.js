...
async function tryFastConnectConfirm(account) {
  ...
  const portRunFast = await verifyPortRunningLogin(vpsId, portNo, login).catch(() => ({
    ok: false
  }));
  if (portRunFast.ok) {
    return promoteConnected(portRunFast.source || 'port_health_running', MT5_SUCCESS_MSG);
  }
  ...
  if (metricsFast.ok || balFast > 0 || eqFast > 0) {
