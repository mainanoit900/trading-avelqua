...
function extractJournalEvidence(...sources) {
  const { sanitizePgText } = require('./pgSanitize');
  for (const src of sources) {
    const text = sanitizePgText(src).trim();
    if (!text) continue;
...
async function handleLegacyWindowVerifiedConnect({
  ...
  if (folderPath && vpsId) {
    queueJournalReadVerify({ accountId, vpsId, folderPath, mt5Login: login, portNo }).catch(() => {});
  }
...
async function queueStopMt5ForAccount(accountId, opts = {}) {
  ...
  `,
    [vpsId, portId || null, toJsonbParam(pl)]
  ).catch(() => {});
...
/** Agent ส่ง command_result ล้มเหลว (login ผิด / journal fail) */
async function applyLoginMt5CommandFailed(node, payload, opts = {}) {
  ...
  const mt5Login = String(
    payload?.mt5Login || payload?.mt5_login || opts.result?.login || ''
  ).trim();
  const portId = Number(payload?.portId || payload?.port_id || 0);
  const portNo = Number(payload?.portNo || payload?.port_no || payload?.portNumber || 0);
  const rawMsg = String(opts.message || opts.result?.message || '').trim();

  let failMsg = MT5_FAIL_USER_MSG;
  const evidence = extractJournalEvidence(
    opts.result?.journalEvidence,
    opts.result?.journal_evidence,
    opts.result?.message,
    rawMsg
  );
  if (messageIndicatesLoginFailed(evidence || rawMsg, mt5Login)) {
    failMsg = MT5_FAIL_USER_MSG;
  } else if (rawMsg && /เชื่อมต่อไม่สำเร็จ|invalid|authorization|password|login failed/i.test(rawMsg)) {
    failMsg = MT5_FAIL_USER_MSG;
  }
...
async function applyLoginMt5FromCommandResult(node, payload, result) {
  ...
  if (!verified) {
    if (
      ...
      (result.journalVerified === true || result.journal_verified === true)
    ) {
      await promoteAccountConnected({
        accountId,
        portId,
...
async function resolveStuckLoginAccount(account) {
  ...
  const failedCmd = await query(
    `
    SELECT result_message, error, result
    FROM vps_system.vps_agent_commands
    WHERE (vps_id=$1 OR node_id=$1)
      AND command_type='login_mt5'
      AND (payload->>'accountId')::text = $2::text
  ...
  if (failedCmd.rows?.[0]) {
    const row = failedCmd.rows[0];
    const blob = String(
      row.result_message || row.error || row.result?.message || JSON.stringify(row.result || {})
    );
    await failAccountFromJournal(accountId, portId, MT5_FAIL_USER_MSG, {
      vpsId,
      portNo,
      folderPath: account.folder_path,
      reason: 'login_command_failed_row'
    });
    return { resolved: true, status: 'failed', message: MT5_FAIL_USER_MSG, source: 'failed_command' };
  }
  ...
  if (status === 'connected' && messageIndicatesLoginFailed(String(account.last_login_message || account.last_error || ''), login)) {
    await failAccountFromJournal(accountId, portId, MT5_FAIL_USER_MSG, { vpsId, portNo, reason: 'connected_but_journal_failed' });
  ...
  const msgBlob = String(account.last_login_message || account.last_error || '');
  if (messageIndicatesLoginFailed(msgBlob, login)) {
    await failAccountFromJournal(accountId, portId, MT5_FAIL_USER_MSG);
    return {
      resolved: true,
      status: 'failed',
  ...
  if (
    isLegacyWindowVerifiedMessage(msgBlob) &&
    login &&
    (msgBlob.includes(login) || /เชื่อมต่อสำเร็จ|window verified/i.test(msgBlob)) &&
    parseMt5JournalOutcome(msgBlob, login) === 'success'
  ) {
    await promoteAccountConnected({
      accountId,
      portId,
    ...
    return { resolved: true, status: 'connected', source: 'window_message' };
  }
  ...
  if (agentReady && elapsedSec >= 12) {
    ...
    if (!inProg && (!recent || recentSt === 'cancelled')) {
      ...
      if (ar?.mt5_password) {
        ...
        `,
          [vpsId, portId, toJsonbParam(payload)]
        ).catch(() => {});
      }
    }
  }
