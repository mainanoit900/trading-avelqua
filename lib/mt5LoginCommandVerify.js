...
async function probeRecentLoginCommandFailed(account) {
  ...
  let msg = MT5_FAIL_USER_MSG;
  if (evidence && login && parseMt5JournalOutcome(evidence, login, undefined, sinceMs) === 'failed') {
    msg = MT5_FAIL_USER_MSG;
  } else if (/ทันเวลา|timeout|ไม่สามารถยืนยัน/i.test(String(cmd.error || res.message || ''))) {
    msg = MT5_FAIL_USER_MSG;
  }
  return { failed: true, message: msg, cmd };
}
...
async function ensureLoginFailPortReleased(accountId, opts = {}) {
  ...
  const portNo = Number(opts.portNo || acc.assigned_port_no || acc.windows_port_no || acc.port_slot || 0);
  const folderPath = String(opts.folderPath || acc.folder_path || '').trim();
  const slot = Number(acc.port_slot || opts.portSlot || 0);
  ...
  if (Number(acc.vps_id || 0) > 0) {
    const adminSlot = adminPortNoFromSystem(portNo) || slot;
    if (adminSlot) {
      await queueForceStopMt5(
        acc.vps_id,
        adminSlot,
        folderPath,
        'login_auth_failed_cleanup'
      ).catch(() => {});
      const { adminNodeId } = await resolveSystemVpsId(acc.vps_id).catch(() => ({ adminNodeId: 0 }));
      await releaseUserPortCompletely({
        systemVpsId: acc.vps_id,
        adminNodeId: adminNodeId || acc.vps_id,
        portNo: adminSlot,
        folderPath,
...
async function repairFailedAccountsHoldingSlots(userId) {
  ...
  for (const row of rows.rows || []) {
    await ensureLoginFailPortReleased(Number(row.id), {
