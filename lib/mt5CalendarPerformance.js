...
async function ensureConnectBaselineColumn() {
  await query(`
    ALTER TABLE vps_system.mt5_accounts
    ADD COLUMN IF NOT EXISTS connect_baseline_equity NUMERIC
  `);
}

async function ensureCalendarSchema() {
  await ensureDailyPnlTable();
  await ensureConnectBaselineColumn();
}

function pnlFromBaseline(equity, baseline) {
  return roundMoney(num(equity, 0) - num(baseline, 0));
}

async function getConnectBaseline(userId, accountId) {
  ...
  const acc = await query(
    `
    SELECT connect_baseline_equity, last_equity, last_balance
  ...
  let baseline = positiveMoney(acc.rows?.[0]?.connect_baseline_equity);
  if (baseline != null) return baseline;

  const firstLog = await query(
    `
    SELECT el.equity
  ...
  baseline = positiveMoney(firstLog.rows?.[0]?.equity);
  if (baseline != null) {
    await query(
      `
      UPDATE vps_system.mt5_accounts
      SET connect_baseline_equity = $2
      WHERE id = $1
        AND connect_baseline_equity IS NULL
      `,
      [aid, baseline]
    ).catch(() => {});
    return baseline;
  ...
  baseline = positiveMoney(acc.rows?.[0]?.last_equity)
    ?? positiveMoney(acc.rows?.[0]?.last_balance);
  return baseline ?? 0;
}

async function getPrevMonthLastCumulative(userId, accountId, monthFirstDay) {
  const row = await query(
    `
    SELECT closing_equity, pnl
    ...
    LIMIT 1
    `,
    [userId, accountId, monthFirstDay]
  );
  const r = row.rows?.[0];
  if (!r) return null;
  const closing = positiveMoney(r.closing_equity);
  if (closing != null) return roundMoney(r.pnl != null ? r.pnl : closing);
  return roundMoney(r.pnl);
}
...
async function computeDayFromLogs(userId, accountId, dayKey, baseline) {
  const uid = num(userId, 0);
  const aid = num(accountId, 0);
  ...
  const opening = positiveMoney(row.first_equity) ?? base;
  const closing = positiveMoney(row.last_equity) ?? opening;
  return {
    opening_equity: base,
    closing_equity: closing,
    pnl: pnlFromBaseline(closing, base),
...
async function finalizePastDays(userId, accountId, login) {
  ...
  const baseline = await getConnectBaseline(userId, accountId);
  ...
  for (const row of pending.rows || []) {
    ...
    const fromLogs = await computeDayFromLogs(userId, accountId, dayKey, baseline);
    const closing = fromLogs
      ? fromLogs.closing_equity
      : baseline;
    await upsertDailyRow(userId, accountId, login, dayKey, {
      opening_equity: baseline,
      closing_equity: closing,
      pnl: pnlFromBaseline(closing, baseline),
...
async function syncWeekendDay(userId, accountId, login, dayKey) {
  const baseline = await getConnectBaseline(userId, accountId);
  ...
  if (existing.rows?.[0]?.is_finalized) return;

  const refEq = positiveMoney(existing.rows?.[0]?.closing_equity)
    ?? positiveMoney(existing.rows?.[0]?.opening_equity)
    ?? baseline;

  await upsertDailyRow(userId, accountId, login, dayKey, {
    opening_equity: baseline,
    closing_equity: refEq,
    pnl: pnlFromBaseline(refEq, baseline),
    snapshot_count: 0,
    is_finalized: true,
...
async function syncTodayRow(userId, accountId, login) {
  const todayKey = dayKeyBangkok(new Date());
  if (isWeekendDayKey(todayKey)) {
    await syncWeekendDay(userId, accountId, login, todayKey);
    return;
  }
  const baseline = await getConnectBaseline(userId, accountId);
  const existing = await query(
  ...
  const fromLogs = await computeDayFromLogs(userId, accountId, todayKey, baseline);
  const liveEq = (await getLiveEquity(userId, accountId))
    ?? positiveMoney(fromLogs?.closing_equity)
    ?? positiveMoney(existing.rows?.[0]?.closing_equity)
    ?? baseline;

  const closing = liveEq > 0 ? liveEq : (fromLogs?.closing_equity ?? baseline);
...
async function fetchMonthDays(userId, accountId, meta) {
  ...
  for (const row of rows.rows || []) {
    ...
    const isWeekend = isWeekendDayKey(key);
    const isToday = key === dayKeyBangkok(new Date());
    map.set(key, {
      dayKey: key,
      ...
      isFinalized: !!row.is_finalized || isWeekend,
      isToday,
      isWeekend,
      marketClosed: isWeekend,
      live: isToday && !row.is_finalized && !isWeekend
    });
  }
  return map;
}

function buildCalendarGrid(meta, dayMap, todayKey, prevCumulative) {
  ...
  let profitDays = 0;
  let lossDays = 0;
  let prev = prevCumulative;
  let lastCumulative = prevCumulative;
  for (let d = 1; d <= meta.daysInMonth; d += 1) {
    const dayKey = `${meta.monthKey}-${String(d).padStart(2, '0')}`;
    const isWeekend = isWeekendDayKey(dayKey);
    const data = dayMap.get(dayKey);
    const cumulative = isWeekend ? null : (data ? data.pnl : null);
    let dailyChange = null;
    if (cumulative != null && prev != null) {
      dailyChange = roundMoney(cumulative - prev);
    } else if (cumulative != null) {
      dailyChange = cumulative;
    }
    if (dailyChange != null && dailyChange !== 0) {
      if (dailyChange > 0) profitDays += 1;
      if (dailyChange < 0) lossDays += 1;
    }
    if (cumulative != null) {
    ...
    cells.push({
      ...
      pnl: cumulative,
      dailyChange,
      hasData: !!data || isWeekend,
      isToday: dayKey === todayKey,
      isFuture: dayKey > todayKey,
      isWeekend,
      marketClosed: isWeekend,
      live: !!(data && data.live) && !isWeekend,
      finalized: !!(data && data.isFinalized) || isWeekend
    });
  }
  if (lastCumulative != null && prevCumulative != null) {
    monthTotal = roundMoney(lastCumulative - prevCumulative);
  } else if (lastCumulative != null) {
    monthTotal = roundMoney(lastCumulative);
  }
  return { cells, monthTotal, profitDays, lossDays, liveCumulative: lastCumulative };
}
...
async function fetchSnapshotHistory(userId, accountId, dayKey, page) {
  ...
  const marketClosed = isWeekendDayKey(dayKey);
  const baseline = await getConnectBaseline(uid, aid);
  ...
  let rows = (logRows.rows || []).map((row, idx) => {
    ...
    const pnl = pnlFromBaseline(equity, baseline);
    return {
      at: row.created_at,
  ...
  if (isToday && !marketClosed && pg === 1) {
    const liveEq = await getLiveEquity(uid, aid);
    if (liveEq != null) {
      const pnl = pnlFromBaseline(liveEq, baseline);
      const liveRow = {
  ...
  return {
    rows,
    connectBaseline: roundMoney(baseline),
    openingEquity: roundMoney(baseline),
    total,
    page: pg,
    pageSize: lim,
    pageCount,
...
async function fetchCalendarPerformance(userId, { accountId, month, day, historyPage } = {}) {
  ...
  if (!aid) {
    return {
      ...
      marketOpen: isMarketOpenBangkok(),
      refreshEnabled: isMarketOpenBangkok(),
      message: 'no_account'
    };
  }
  ...
  const selectedDay = /^\d{4}-\d{2}-\d{2}$/.test(String(day || '').trim())
    ? String(day).trim()
    : todayKey;
  const connectBaseline = await getConnectBaseline(uid, aid);
  const prevCumulative = await getPrevMonthLastCumulative(uid, aid, meta.firstDay);
  ...
  const calendar = buildCalendarGrid(meta, dayMap, todayKey, prevCumulative);
  ...
  const liveEquity = await getLiveEquity(uid, aid);
  const livePnl = liveEquity != null ? pnlFromBaseline(liveEquity, connectBaseline) : null;

  const marketOpen = isMarketOpenBangkok();
  const selectedIsWeekend = isWeekendDayKey(selectedDay);
  ...
  return {
    ...
    selectedDayPnl: selectedIsWeekend ? null : (selectedDayData ? selectedDayData.pnl : null),
    connectBaseline: roundMoney(connectBaseline),
    liveEquity: liveEquity != null ? roundMoney(liveEquity) : null,
    livePnl,
    marketOpen,
    refreshEnabled: marketOpen,
    selectedMarketClosed: selectedIsWeekend,
    calendar,
    history,
    refreshSec: SNAPSHOT_INTERVAL_SEC
  };
...
async function fetchMt5LoginPortfolio(userId) {
  await ensureCalendarSchema();
  const uid = num(userId, 0);
  if (!uid) {
  ...
  const rows = await query(
    `
    SELECT id, mt5_login, port_slot, last_equity, last_balance, status,
           broker, server_name, mt5_server, connect_baseline_equity,
  ...
  for (const row of rows.rows || []) {
    ...
    let baseline = positiveMoney(row.connect_baseline_equity);
    if (baseline == null) {
      baseline = await getConnectBaseline(uid, accountId);
    }

    let current = await getLiveEquity(uid, accountId);
    if (current == null) {
      current = positiveMoney(row.last_equity) ?? positiveMoney(row.last_balance) ?? baseline;
    }

    const pnl = pnlFromBaseline(current, baseline);
    if (isConnected) {
      activeCount += 1;
      totalPnl += pnl;
      totalCurrentEquity += num(current, 0);
      totalBaselineEquity += num(baseline, 0);
    }

    items.push({
      accountId,
      port: num(row.port_slot, 0) || null,
      mt5Login: String(row.mt5_login || '').trim(),
      broker: String(row.broker || 'MH Markets').trim() || 'MH Markets',
      server: String(row.server_name || row.mt5_server || '').trim() || '-',
      status,
      isConnected,
      baselineEquity: roundMoney(baseline),
  ...
  return {
    ...
    summary: {
      ...
      totalCurrentEquity: roundMoney(totalCurrentEquity),
      totalBaselineEquity: roundMoney(totalBaselineEquity)
    },
    marketOpen,
    refreshEnabled: marketOpen,
    refreshSec: SNAPSHOT_INTERVAL_SEC,
