...
function folderPathForPortNo(portNo, folderPath = '', nodeCode = 'VPS-WIN-01') {
  const fp = String(folderPath || '').trim();
  if (fp) return fp;
  const n = adminPortNoFromSystem(portNo);
  if (!n) return '';
  const code = String(nodeCode || 'VPS-WIN-01').trim() || 'VPS-WIN-01';
  return `C:\\MT5_PORTS\\${code}-PORT-${String(n).padStart(2, '0')}`;
}

function vpsPortNameForNo(portNo, nodeCode = 'VPS-WIN-01') {
  const n = adminPortNoFromSystem(portNo);
  if (!n) return '';
  const code = String(nodeCode || 'VPS-WIN-01').trim() || 'VPS-WIN-01';
  return `${code}-PORT-${String(n).padStart(2, '0')}`;
}
