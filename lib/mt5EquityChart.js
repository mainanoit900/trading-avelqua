...
/** ช่วงบันทึกจุดกราฟ Equity (วินาที) — default 90 วิ */
const SNAPSHOT_INTERVAL_SEC = Math.max(
  30,
  Number(
    process.env.MT5_EQUITY_SNAPSHOT_SEC
      || process.env.MT5_LIVE_METRICS_REFRESH_SEC
      || 90
  )
);
/** @deprecated ใช้ MT5_EQUITY_SNAPSHOT_SEC — รองรับ env เก่า (นาที) */
const SNAPSHOT_INTERVAL_MINUTES_LEGACY = Number(process.env.MT5_EQUITY_SNAPSHOT_MINUTES || 0);
const SNAPSHOT_INTERVAL_SEC_EFFECTIVE =
  SNAPSHOT_INTERVAL_MINUTES_LEGACY > 0
    ? Math.max(30, SNAPSHOT_INTERVAL_MINUTES_LEGACY * 60)
    : SNAPSHOT_INTERVAL_SEC;
/** ค่าเดิม export ให้โค้ดเก่า (นาที) */
const SNAPSHOT_INTERVAL_MINUTES = SNAPSHOT_INTERVAL_SEC_EFFECTIVE / 60;
...
/**
 * บันทึก Equity ทุก SNAPSHOT_INTERVAL_SEC (จุดแรกบันทึกทันทีเมื่อเริ่มรัน)
 */
async function recordEquityLog(instanceId, equity) {
  ...
  if (hasAny) {
    const ins = await query(
      ...
      RETURNING id
      `,
      [iid, eqVal, String(SNAPSHOT_INTERVAL_SEC_EFFECTIVE)]
    ).catch(() => ({ rows: [] }));
    return !!ins.rows?.[0];
  }
...
async function seedInstanceLiveMetrics(instanceId, balance, equity) {
  ...
  await query(
    ...
    SET mt5_balance = COALESCE($2::numeric, mt5_balance),
        ...
        start_equity = CASE
          WHEN start_equity IS NULL AND $3::numeric > 0 THEN $3::numeric
          WHEN start_equity IS NOT NULL AND $3::numeric > start_equity * 5 THEN $3::numeric
          ELSE start_equity
        END,
...
async function fetchEquityChartForInstance(instanceId, userId) {
  ...
  return {
    ...
    currentPnl,
    intervalSeconds: SNAPSHOT_INTERVAL_SEC_EFFECTIVE,
    intervalMinutes: SNAPSHOT_INTERVAL_SEC_EFFECTIVE / 60,
...
module.exports = {
  SNAPSHOT_INTERVAL_SEC: SNAPSHOT_INTERVAL_SEC_EFFECTIVE,
