 ...
 * - VPS1 ก่อน → ถ้าไม่มี FolderPort ว่างค่อย VPS2,3,... ภายใน VPS เลือก FolderPort 1→20
 */
...
/** ลำดับ VPS จากชื่อ (VPS-WIN-01 → 1) — fallback admin_node_id */
function vpsSequenceFromRow(row) {
  const name = String(row?.node_name || row?.node_code || '').trim();
  const m =
    name.match(/(?:VPS[-_]?WIN[-_]?|VPS[-_]?)(\d+)/i) ||
    name.match(/(\d+)\s*$/);
  if (m) return num(m[1], 9999);
  return num(row?.admin_node_id, 9999);
}

/** VPS น้อยไปมาก → FolderPort น้อยไปมาก (1→20) ไม่ข้าม */
function pickBestPortRow(rows, _loginLoadMap = null) {
  const list = (rows || []).filter((r) => !isPortAdminDisabled(r));
  list.sort(
    (a, b) =>
      vpsSequenceFromRow(a) - vpsSequenceFromRow(b) ||
      num(a.admin_node_id) - num(b.admin_node_id) ||
      num(a.port_number) - num(b.port_number) ||
      num(a.allocation_id) - num(b.allocation_id)
  );
  return list[0] || null;
}
 ...
 */
async function findBestAdminPortForLogin(excludeAllocationIds = [], userId = 0) {
  ...
  const r = await query(
    ...
    WHERE COALESCE(pr.used_ports, 0) < COALESCE(pr.max_ports, 20)
      AND COALESCE(pr.port_number, 0) BETWEEN 1 AND 20
      AND COALESCE(TRIM(pr.folder_path), '') <> ''
    ORDER BY pr.admin_node_id ASC, pr.port_number ASC
    `,
    []
  ).catch((e) => {
  ...
  for (const [adminNodeId, rows] of byNode.entries()) {
    ...
    const { systemVpsId } = await resolveSystemVpsId(adminNodeId).catch(() => ({}));
    for (const row of rows) {
      ...
      if (uid && systemVpsId) {
        ...
        const folderGuard = await assertFolderPortFreeForUser(
          systemVpsId,
          adminPortToSystemPortNo(portNo) || portNo,
          uid
        ).catch(() => ({ ok: true }));
        if (!folderGuard.ok) continue;
      }
      available.push(row);
    }
  }
...
async function reserveAdminPortForLogin(userId) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const best = await findBestAdminPortForLogin(tried, userId);
    if (!best) break;
