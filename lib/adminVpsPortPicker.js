...
const { resolveServerForLogin } = require('./mt5Server');
const {
  resolveSystemVpsId,
 ...
 */
async function findBestAdminPortForLogin() {
  ...
  const r = await query(
    ...
    WITH nodes AS (
      SELECT
        ...
        GREATEST(
          COALESCE(sn.last_seen_at, sn.last_heartbeat, sn.updated_at),
  ...
  const [liveMaps, dbUsageMaps] = await Promise.all([
    Promise.all(nodeIds.map((id) => fetchLiveHealthMap(id).catch(() => ({})))),
    Promise.all(nodeIds.map((id) => fetchDbMt5UsageMap(id).catch(() => ({}))))
  ]);
  const liveByNode = Object.fromEntries(nodeIds.map((id, i) => [id, liveMaps[i]]));
  const usageByNode = Object.fromEntries(nodeIds.map((id, i) => [id, dbUsageMaps[i]]));

  for (const [adminNodeId, rows] of byNode.entries()) {
    const liveMap = liveByNode[adminNodeId] || {};
    const dbUsageMap = usageByNode[adminNodeId] || {};
  ...
  return pickBestPortRow(available);
}
...
async function reserveAdminPortForLogin(userId) {
  const best = await findBestAdminPortForLogin();
  if (!best) {
    return {
      ok: false,
      message:
...
function buildMt5LoginPayload({
  ...
  mt5Password,
  serverName,
  journalTimeoutSec = null,
  loginQueueDelaySec = 0
}) {
  ...
  return {
    ...
    serverName: resolveServerForLogin(mt5Login) || serverName,
    useStartupIni: true,
    portable: true,
    showMt5Window: true,
    bringToFront: true,
    showWindowReason: 'login_debug',
    journalTimeoutSec: Number(
      journalTimeoutSec != null && journalTimeoutSec !== ''
        ? journalTimeoutSec
        : process.env.MT5_JOURNAL_TIMEOUT_SEC || 120
    ),
    loginQueueDelaySec: Math.max(0, Number(loginQueueDelaySec || 0)),
