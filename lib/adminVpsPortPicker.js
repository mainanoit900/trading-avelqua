...
/** เลือก PORT ต่ำสุดที่ว่างก่อน (1→20) ตาม /admin/vps/ports — ไม่ข้ามไป PORT 4 ถ้า 3 ว่าง */
function pickBestPortRow(rows) {
...
async function pickBestPortRowSmart(rows) {
  const { pickBestPortForLogin } = require('./mt5AiConnectAdvisor');
  const list = (rows || []).filter((r) => !isPortAdminDisabled(r));
  if (!list.length) return null;
  const picked = await pickBestPortForLogin(list).catch(() => null);
  return picked || pickBestPortRow(list);
}
 ...
 */
async function findBestAdminPortForLogin(preferredPortNo = 0) {
  const preferSlot = num(preferredPortNo, 0);
  ...
  for (const [adminNodeId, rows] of byNode.entries()) {
    ...
    const freshHealth = hasFreshPortHealthSignal(liveMap);
    if (!isVpsNodeEligibleForLogin(rows[0], { freshPortHealth: freshHealth })) continue;
  ...
  if (preferSlot > 0) {
    const matched = available.filter((r) => parsePortNumber(r) === preferSlot);
    if (matched.length) {
      const picked = await pickBestPortRowSmart(matched);
      if (picked) return picked;
    }
    return null;
  }

  return pickBestPortRowSmart(available);
}
...
async function resolveOrCreateSystemPort(systemVpsId, adminPort) {
  ...
  let row = await query(
    ...
    WHERE vps_id = $1
      ...
      AND (
        TRIM(COALESCE(folder_path, '')) = ''
        OR (regexp_match(COALESCE(folder_path, ''), '(?i)PORT[-_ ]*0*([0-9]+)$'))[1]::int = $4
        OR $4 = 0
      )
    ORDER BY
      CASE WHEN TRIM(COALESCE(folder_path, '')) = TRIM($2) THEN 0 ELSE 1 END,
      CASE WHEN LOWER(COALESCE(status, '')) IN ('available', 'free', 'idle') THEN 0 ELSE 1 END,
      id ASC
    LIMIT 1
  `,
    [systemVpsId, folderPath, systemPortNos, portNo]
  ).catch(() => ({ rows: [] }));
...
/**
 * จอง PORT สำหรับ Login MT5 — อ่านจาก admin แล้ว lock ใน vps_system.vps_ports
 */
async function reserveAdminPortForLogin(userId, preferredPortNo = 0) {
  const { assertVpsAgentOnline } = require('./vpsAgentLiveness');
  ...
  const preferSlot = num(preferredPortNo, 0);
  const best = await findBestAdminPortForLogin(preferredPortNo);
  if (!best) {
    return {
      ok: false,
      message:
        preferSlot > 0
          ? `Folder PORT ${String(preferSlot).padStart(2, '0')} ไม่ว่างหรือไม่พร้อม — ตรวจ /admin/vps/…/ports (PORT ${preferSlot} ต้องตรงแพ็กเกจ)`
  ...
  if (!isAdminPortAvailableForLogin(best, liveMap, dbUsageMap)) {
    return {
      ok: false,
      message: `PORT ${String(portNoCheck).padStart(2, '0')} ไม่ว่างแล้ว — กรุณากดเชื่อมต่ออีกครั้ง`
    };
  }
  ...
  const { findFolderPortUsedByOtherUser } = require('./mt5PortSlotGuard');
  const otherUser = await findFolderPortUsedByOtherUser(systemVpsId, portNoCheck, userId).catch(
    () => null
  );
  if (otherUser) {
    return {
      ok: false,
      message: `Folder PORT ${String(portNoCheck).padStart(2, '0')} มีผู้ใช้อื่นอยู่แล้ว — เลือก PORT อื่น`
    };
  }
  ...
  try {
    ...
    if (!lockRes.rows[0]) {
      await client.query('ROLLBACK');
      return { ok: false, message: 'PORT นี้ถูกจองไปแล้ว กรุณาลองใหม่' };
    }
...
/** โฟลเดอร์ VPS ต้องตรง PORT แพ็กเกจ (slot 2 → …-PORT-02) */
function folderPathForPackageSlot(reservedPort, portSlot) {
  const slot = num(portSlot, 0);
  const folder = String(reservedPort?.folder_path || '').trim();
  if (!slot) return folder;
  const m = folder.match(/PORT[-_ ]*0*(\d+)\s*$/i);
  if (m && num(m[1]) === slot) return folder;
  const node = String(reservedPort?.node_name || 'VPS-WIN-01').trim() || 'VPS-WIN-01';
  const portName =
    reservedPort?.port_name || `${node.replace(/\s+/g, '-')}-PORT-${String(slot).padStart(2, '0')}`;
  return `C:\\MT5_PORTS\\${portName}`;
}

/** Payload สำหรับ Agent เปิด MT5 บน FolderPort (portSlot = แพ็กเกจ, folderPath = VPS) */
function buildMt5LoginPayload({
  ...
  skipLoginVerify = false
}) {
  const folderPortNo = num(portSlot) || num(parsePortNumber(reservedPort));
  const folder = folderPathForPackageSlot(reservedPort, folderPortNo);
  const portNo = folderPortNo;
  return {
    action: 'login_mt5',
    accountId,
    userId,
    ...
    ...(skipLoginVerify
      ? { skipLoginVerify: true, skip_login_verify: true, reopenOnly: true, reopen_only: true }
      : {})
  };
}
