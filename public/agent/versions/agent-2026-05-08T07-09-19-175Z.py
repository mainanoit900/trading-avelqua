...
AGENT_VERSION = os.getenv("AVELQUA_AGENT_VERSION", "2026-05-08-final-login-fix")
...
"""
    ...
    while time.time() < deadline:
        ...
        latest, text = latest_log_text(port_dir)
        raw_text = text or ""
        check_text = raw_text.lower()

        if raw_text:
            tail = raw_text[-900:].replace("\\n", " | ")
        ...
        # FINAL FIX:
        # ถ้า MT5 log มีคำยืนยัน authorization/synchronized/enabled ให้ถือว่า login ผ่านทันที
        # ไม่บังคับให้เจอเลข login ทุกบรรทัด เพราะ MT5 บาง build ตัด log เป็น tail ทำให้เลข login หายได้
        auth_ok = any(word in check_text for word in success_words)
