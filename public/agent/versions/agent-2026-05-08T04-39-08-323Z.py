...
def mt5_login_success(text: str, login: str) -> bool:
    ...
    success_found = any(x in text_lower for x in success_patterns)

    if not success_found:
        return False

    if login not in text and f"'{login}'" not in text and f'"{login}"' not in text:
        return False
...
"""
    ...
    while time.time() < deadline:
        ...
        if mt5_has_login_failure_text(text or ""):
            ...
            send_connect_result(
                payload,
                "failed",
                "MT5 login failed: user/password ไม่ถูกต้อง",
                port_no
            )
            raise RuntimeError("MT5 login failed: user/password ไม่ถูกต้อง")

        if mt5_login_success(text or "", login):
            send_connect_result(
                payload,
                "connected",
                "MT5 login success: authorized verified by log",
                port_no
            )
        ...
        if proc.poll() is not None:
            ...
            send_connect_result(
                payload,
                "failed",
                "MT5 terminal ปิดตัวก่อน login สำเร็จ",
                port_no
            )
            raise RuntimeError("MT5 terminal exited before authorized")
    ...
    send_connect_result(
        payload,
        "failed",
        "MT5 login timeout: no authorized login in log",
        port_no
    )

    raise RuntimeError("MT5 login timeout: no authorized login in log")
