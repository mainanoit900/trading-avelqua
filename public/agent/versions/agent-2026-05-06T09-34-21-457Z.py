...
def send_port_health() -> None:
    try:
        ...
        api("POST", "/port-health", body, timeout=20)
...
def _popen_hidden(args: List[str], cwd: Optional[str] = None) -> subprocess.Popen:
    ...
    if os.name == "nt":
        ...
        startupinfo.wShowWindow = 6  # SW_MINIMIZE (safer for MT5 than fully hidden)
...
def start_mt5_bot(payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    PRO MT5 Engine V2:
    - no-log primary verification
    - exact PORT isolation
    - window-title login verification
    - socket detect as secondary signal
    - auto recover once if terminal exits during login
    """
...
"""
    ...
    def result_ok(message: str) -> Dict[str, Any]:
        send_connect_result(payload, "connected", message, port)
    ...
    def launch_mt5(reason: str) -> None:
        try:
    ...
    while time.time() < deadline:
        ...
        # 1) Primary truth: MT5 window title contains the exact account login.
        verified, title_info = mt5_login_verified_by_window(port, payload)
        if time.time() - last_title_log >= 8:
            log(f"MT5 V2 TITLE CHECK PORT={port} LOGIN={login} OK={verified} TITLE={title_info[:220]}")
            last_title_log = time.time()
        if verified:
            return result_ok("MT5 login success (window verified)")

        # 2) Secondary signal: terminal has established broker socket, then wait briefly for title.
        socket_ok, socket_info = mt5_socket_established(port, payload)
        if time.time() - last_socket_log >= 8:
            log(f"MT5 V2 SOCKET CHECK PORT={port} OK={socket_ok} INFO={socket_info[:220]}")
            last_socket_log = time.time()
        if socket_ok and elapsed >= 18:
            # ห้ามถือว่า login ผ่านจาก socket อย่างเดียว
            # ต้องรอ window title มีเลข login จริงเท่านั้น
            latest, text = latest_log_text(port_dir)
            if mt5_has_login_failure_text(text or ""):
                stop_mt5_port_only(port, payload)
                send_connect_result(payload, "failed", "MT5 login failed (auth rejected)", port)
                raise RuntimeError("MT5 login failed (auth rejected)")

        # 3) Failure text is only a negative signal. Never require success text from log.
        latest, text = latest_log_text(port_dir)
        if latest:
            log(f"MT5 V2 LOG WATCH PORT={port} LOG={latest}")
            if mt5_has_login_failure_text(text or ""):
                stop_mt5_port_only(port, payload)
                send_connect_result(payload, "failed", "MT5 login failed (auth rejected)", port)
                raise RuntimeError("MT5 login failed (auth rejected)")

        # 4) Auto recover once if terminal closes/crashes during login.
        if elapsed >= 10 and not procs and recover_count < 1:
            recover_count += 1
            log(f"MT5 V2 AUTO RECOVER PORT={port} LOGIN={login} reason=process_exited")
            launch_mt5("auto_recover_process_exited")
            started_at = time.time()
            continue

        log(f"MT5 V2 WAIT PORT={port} SECONDS={elapsed} PROCS={len(procs)}")

    stop_mt5_port_only(port, payload)
    send_connect_result(payload, "failed", "MT5 login timeout (no verified window/socket)", port)
    raise RuntimeError("MT5 login timeout (no verified window/socket)")
