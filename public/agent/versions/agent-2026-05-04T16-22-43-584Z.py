...
def handle_command(cmd: Dict[str, Any]) -> None:
    ...
    try:
        if ctype in ("status", "service_status"):
            command_result(cmd_id, True, {"service_name": SERVICE_NAME, "status": "Running", "agent_type": "python", "agent_version": AGENT_VERSION})
    ...
    for i in range(1, 21):
        p = MT5_ROOT / f"VPS-WIN-01-PORT-{i:02d}"
        if p.exists():
            result["ports_found"].append({
                "port": i,
                "path": str(p),
                "terminal_exists": (p / "terminal64.exe").exists()
            })
