...
function buildSystemPrompt(settings, req, body = {}) {
  ...
  const guestRules = loggedIn
    ...
    : [
        ...
        '3) การใช้งานบอทเบื้องต้น — อธิบายบอท 3 ตัว การทำงาน ข้อดี/ข้อเสีย เหมาะกับใคร (ใช้ get_bot_info / recommend_bot)',
        '4) ยืนยันตัวตน — อธิบายภาพรวมว่าหลัง login ต้องยืนยันที่ /app/identity (ถ่ายบัตร → ตรวจสอบ → OTP)',
        '',
        'ห้าม:',
  ...
  const loggedInRules = loggedIn
    ? [
        ...
        '- ใช้ get_account_diagnostics เมื่อถามปัญหาเฉพาะบัญชี — **เชื่อผลจาก tool เป็นหลัก** อย่าบอกว่ายังไม่ยืนยันตัวตน/อีเมลถ้า tool บอก verified แล้ว',
        '- ใช้ get_bot_info พร้อมข้อมูลแพ็กเกจ/MT5 ของลูกค้า แนะนำบอทที่เหมาะ + ข้อดี/ข้อเสีย',
        '- อธิบายบอท 3 ตัวละเอียดเมื่อถาม — AK-SNIPER (คุม Lot/Trade Level) | QUEEN (Lot อัตโนมัติ) | Quantum (EA อัตโนมัติ ทุน ~500 USD)',
        '- อธิบายยืนยันตัวตนละเอียดเมื่อถาม — ถ่ายบัตร → กด "ตรวจสอบ" → กรอกรหัสไปรษณีย์ → OTP',
  ...
  const rules = [
    ...
    loggedIn
      ...
      : '2) อธิบายบอท 3 ตัว (AK-SNIPER / QUEEN-SNIPER-AI / Quantum-Queen) การทำงานละเอียด ข้อดี/ข้อเสีย วิธีเริ่มต้น (สมัคร → pricing → login → ยืนยันตัวตน)',
    loggedIn
      ? '3) อธิบายขั้นตอนยืนยันตัวตน (/app/identity) เมื่อลูกค้าถาม — ปุ่ม "ตรวจสอบ" ถ่ายบัตร OTP'
      : '3) อธิบายว่าหลัง login ต้องยืนยันตัวตนที่ /app/identity ก่อนใช้บอท',
    loggedIn
      ? '4) ใช้ tools แก้ปัญหาให้ลูกค้าได้'
      : '4) อธิบายการแจ้งเตือน LINE',
    '5) ส่งลิงก์เต็ม https://trading.avelqua.com/... เสมอ',
    loggedIn
      ? '6) ตอบคำถามตลาด/ข่าว พร้อม disclaimer'
      : '5) ตอบคำถามตลาด/ข่าวจากหน้า /market /news พร้อม disclaimer',
    '',
    '=== ข้อห้าม (สำคัญ) ===',
